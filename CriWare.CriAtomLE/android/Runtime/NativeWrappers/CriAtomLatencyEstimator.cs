...
#pragma warning disable 0465
	...
	public static partial class CriAtomLatencyEstimator
	{
		...
		///  本関数を呼び出すと、Atomライブラリ内部で遅延推測器が生成、起動します。 起動された遅延推測器のスレッドは、他スレッドの要求等を待たず遅延推測処理を開始します。 遅延推測を終了するには、<see cref="CriAtomLatencyEstimator.FinalizeANDROID"/> 関数を呼ぶ必要があります。 
		///  また、本関数は完了復帰です。多重呼び出しを許容しますが、Atomライブラリ内では 本関数の呼び出し回数をカウントしています。 実際の初期化処理が実行されるのは、最初の呼び出しの時だけになります。 なお、この呼び出し回数カウンタは、<see cref="CriAtomLatencyEstimator.FinalizeANDROID"/> 関数を呼び出す度に デクリメントされます。 
		/// </para>
		/// <para>
		/// 注意:
		...
		///  また、本関数は完了復帰です。多重呼び出しを許容します。 本関数を呼び出すと、<see cref="CriAtomLatencyEstimator.InitializeANDROID"/> 関数の呼び出しカウンタが デクリメントされます。本関数と<see cref="CriAtomLatencyEstimator.InitializeANDROID"/> 関数の呼び出し回数が 同数になるよう注意してください。 
		/// </para>
		/// <para>
		/// 注意:
		/// criAtom_Initialize_ANDROID 関数実行前に本関数を実行しないでください。
		...
		/// <returns><see cref="CriAtomLatencyEstimator.Info"/> 遅延推測器の情報 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 遅延推測器の現在の情報を取得します。
		///  取得できる情報は「遅延推測器の状態」「推測遅延時間(ミリ秒)」の２つです。
		/// <see cref="CriAtomLatencyEstimator.InitializeANDROID"/> 関数を呼び出した後、本関数を呼ぶことで 現在の遅延推測器の情報(状態、推測遅延時間)を取得することができます。 状態が <see cref="CriAtomLatencyEstimator.Status.Done"/> である時、推測遅延時間は0でない数値になります。 それ以外の状態では、推測遅延時間は0を返します。 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriBool 初期化済みかどうか </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		public enum Status
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>初期状態/停止状態 (実行中スレッドなし) </para>
			/// </remarks>
			Stop = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>遅延時間を推測中 (実行中スレッドあり) </para>
			/// </remarks>
			Processing = 1,
			/// <summary></summary>
			/// <remarks>
			/// <para>遅延時間の推測完了 (実行中スレッドなし) </para>
			/// </remarks>
			Done = 2,
			/// <summary></summary>
			/// <remarks>
			/// <para>エラー (実行中スレッドなし) </para>
			/// </remarks>
			Error = 3,
		}
	}
