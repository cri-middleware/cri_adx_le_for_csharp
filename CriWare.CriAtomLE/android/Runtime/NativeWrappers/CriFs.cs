...
#pragma warning disable 0465
	...
	public static partial class CriFs
	{
		...
		///  プライオリティはナイス値で指定してください。範囲は-20(最高)から19(最低)です。 アプリケーションのメインスレッドよりも高いプライオリティを指定してください。
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetFileAccessThreadPriority_ANDROID(prio);
		}
		...
		public static unsafe CriErr.Error GetFileAccessThreadPriorityANDROID(out Int32 prio)
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetFileAccessThreadPriority_ANDROID(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetMemoryFileSystemThreadPriority_ANDROID(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetMemoryFileSystemThreadPriority_ANDROID(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetDataDecompressionThreadPriority_ANDROID(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetDataDecompressionThreadPriority_ANDROID(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_EnableAssetsAccess_ANDROID(vm, jobj);
		}

		/// <summary>Androidプロジェクト内のassetsフォルダーに対するアクセスの有効化(プレフィックス指定) </summary>
		/// <param name="vm">JavaVMオブジェクトへの参照 </param>
		/// <param name="jobj">android.content.Contextオブジェクト </param>
		/// <param name="prefix">assetsフォルダーへのアクセスを示すプレフィックス </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 特定のプレフィックスから始まるファイルパスに対してassetフォルダーへのアクセスを有効化します。
		///  本関数の呼び出し以降は、ファイルパスにprefix文字列が含まれる場合に、prefixを除いた文字列がassetsフォルダー内でのパスとして認識されます。
		///  アクセスが終了した場合、 <see cref="CriFs.DisableAssetsAccessANDROID"/> 関数 を呼び出してください。 
		/// </para>
		/// <para>
		/// 注意:
		...
		{
			return (CriErr.Error)NativeMethods.criFs_EnableAssetsAccessForPrefix_ANDROID(vm, jobj, prefix.GetPointer(stackalloc byte[prefix.BufferSize]));
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_DisableAssetsAccess_ANDROID();
		}
