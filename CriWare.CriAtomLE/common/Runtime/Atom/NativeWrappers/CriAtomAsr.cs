...
#pragma warning disable 0465
	...
	public static partial class CriAtomAsr
	{
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomAsr_SetDefaultConfig_(CriAtomAsrConfig *p_config)"/>
		/// </remarks>
		...
		/// <see cref="CriAtom.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomAsr.Initialize"/> 関数でASRの初期化を行う場合、 本関数で計算したサイズ分のメモリをワーク領域として渡す必要があります。
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。
		/// </para>
		/// <para>
		/// 備考:
		/// ASRの初期化に必要なワークメモリのサイズは、ASR初期化用コンフィグ 構造体（ <see cref="CriAtomAsr.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomAsr.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		public static unsafe void Initialize(in CriAtomAsr.Config config)
		{
			fixed (CriAtomAsr.Config* configPtr = &config)
				NativeMethods.criAtomAsr_Initialize(configPtr, default, default);
		}
		...
		/// <see cref="CriAtom.SetUserAllocator"/> メソッドを使用してアロケーターを登録済みの場合、 ASR初期化時に確保されたメモリ領域が解放されます。
