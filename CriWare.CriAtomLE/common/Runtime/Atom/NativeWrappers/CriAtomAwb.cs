...
namespace CriWare
{
	...
	public partial class CriAtomAwb : IDisposable
	{
		...
		/// <returns><see cref="CriAtomAwb"/> AWBオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 音声再生の音源として使用するAWBファイルのTOC情報をロードします。
		///  ロード完了まで本関数は復帰しませんので、シーンの切り替えや、 負荷変動が許容できるタイミングで実行してください。
		///  ロードに成功すると、戻り値に有効なAWBオブジェクトが返りますので、 <see cref="CriAtomPlayer.SetWaveId"/> 関数に指定して使用してください。
		...
		/// <see cref="CriAtomAwb.GetStatus"/> 関数でAWBオブジェクトのステータスを定期的に取得し、 ロードの完了を確認してください。
		///  ロードの完了を待っている間はAWBオブジェクトのステータス更新のために、 定期的に<see cref="CriAtom.ExecuteMain"/> 関数を実行する必要があります。
		...
		/// <returns><see cref="CriAtomAwb"/> オンメモリAWBオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns><see cref="CriAtomAwb.Type"/> AWBオブジェクトの種別を示す値 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		{
			return (CriAtomAwb.Type)NativeMethods.criAtomAwb_GetType(NativeHandle);
		}
		...
		/// <para>
		/// 説明:
		/// AWBオブジェクトがTOC情報のみのオブジェクトなのか、メモリ上にロードされたAWBオブジェクトなのかを示す値です。 <see cref="CriAtomAwb.GetAwbType"/> 関数で取得します。
		/// </para>
		/// </remarks>
		...
		public enum Type
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>TOC情報のみのAWBハンドル </para>
			/// </remarks>
			Toc = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>オンメモリAWBハンドル </para>
			/// </remarks>
			Onmemory = 1,
			/// <summary></summary>
			/// <remarks>
			/// <para>無効なAWBハンドル </para>
			/// </remarks>
			Error = 2,
		}
		...
		/// <summary>AWBハンドルを介してAWBファイルに含まれるコンテンツ数を取得 </summary>
		/// <returns>コンテンツファイル数（0～65535） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
...
#pragma warning restore 1591
		...
		/// <summary>AWBハンドルが即時解放可能かどうかのチェック </summary>
		/// <returns>CriBool AWBの状態（true = 即時解放可能、false = 再生中のプレーヤーあり） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// AWBオブジェクトを即座に解放可能かどうかをチェックします。
		///  本関数が false を返すタイミングで <see cref="CriAtomAwb.Dispose"/> 関数を実行すると、 AWBオブジェクトを参照しているプレーヤーに対する停止処理が行われます。
		///  （ストリーム再生用のAWBオブジェクトの場合、ファイル読み込み完了を待つため、 <see cref="CriAtomAwb.Dispose"/> 関数内で長時間処理がブロックされる可能性があります。）
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行すると、指定したAWBデータを参照しているAtomプレーヤーが存在しないか、 ライブラリ内で検索処理が行われます。
		...
		/// <summary>AWBハンドルのステータス取得 </summary>
		/// <returns><see cref="CriAtomAwb.Status"/> AWBオブジェクトの状態を示す値 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		{
			return (CriAtomAwb.Status)NativeMethods.criAtomAwb_GetStatus(NativeHandle);
		}
		...
		/// <summary>オンメモリAWBハンドルの作成に必要なワーク領域サイズ </summary>
		/// <remarks>
		/// <para>
		/// 説明:
