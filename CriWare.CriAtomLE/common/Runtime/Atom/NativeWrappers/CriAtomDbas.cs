...
namespace CriWare
{
	...
	public partial class CriAtomDbas : IDisposable
	{
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomDbas.CriAtomDbas"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomDbas.Config"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomDbas_SetDefaultConfig_(CriAtomDbasConfig *p_config)"/>
		/// </remarks>
		...
		/// <param name="config">D-BAS作成用コンフィグ構造体へのポインタ </param>
		/// <returns><see cref="CriAtomDbas"/> D-BAS管理用ID </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <seealso cref="CriAtomDbas.CalculateWorkSize"/>
		/// <seealso cref="CriAtomDbas.Dispose"/>
		public unsafe CriAtomDbas(in CriAtomDbas.Config config)
		{
			fixed (CriAtomDbas.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomDbas_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomDbas()
		{
...
#pragma warning restore 1591
		...
		/// <para>
		/// 説明:
		/// D-BAS作成時に指定可能なバッファー保証数の下限値です。 途切れずにストリーミング再生するためには、 最低でも2バッファー（ダブルバッファリング）必要なため、 2未満の値に設定する事はできません。 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomDbas.CriAtomDbas"/>
		/// <seealso cref="CriAtomDbas.CalculateWorkSize"/>
