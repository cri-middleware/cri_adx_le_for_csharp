...
#pragma warning disable 0465
	...
	public static partial class CriAtomEx
	{
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomEx.InitializeForUserPcmOutput"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomEx.ConfigForUserPcmOutput"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_SetDefaultConfigForUserPcmOutput_(CriAtomExConfigForUserPcmOutput *p_config)"/>
		/// </remarks>
		...
		///  本関数を実行後、 <see cref="CriAtomEx.GetPerformanceInfo"/> 関数を実行することで、 サーバー処理の負荷や、サーバー処理の実行間隔等、ライブラリのパフォーマンス情報を 取得することが可能です。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_AttachPerformanceMonitor_()"/>
		/// </remarks>
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_DetachPerformanceMonitor_()"/>
		/// </remarks>
		...
		/// 説明:
		/// 現在までの計測結果を破棄します。
		///  パフォーマンスモニターは、 <see cref="CriAtomEx.AttachPerformanceMonitor"/> 関数実行直後 からパフォーマンス情報の取得を開始し、計測結果を累積します。
		///  以前の計測結果を以降の計測結果に含めたくない場合には、 本関数を実行し、累積された計測結果を一旦破棄する必要があります。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_ResetPerformanceMonitor_()"/>
		/// </remarks>
		...
		/// 説明:
		/// パフォーマンス情報を取得します。
		///  本関数は、 <see cref="CriAtomEx.AttachPerformanceMonitor"/> 関数実行後から <see cref="CriAtomEx.DetachPerformanceMonitor"/> 関数を実行するまでの間、利用可能です。
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_GetPerformanceInfo_(CriAtomPerformanceInfo *p_info)"/>
		/// </remarks>
		...
		///  これに対し、<see cref="CriAtomEx.SetChannelMapping"/>(5, 1); を実行した場合、 5ch音声データの各チャンネルが L, R, LFE, Ls, Rs の順で出力されるようになります。
		/// </para>
		/// <para>
		/// 備考:
		/// 現状、本関数は5ch音声データのマッピングパターン変更にしか対応していません。
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_SetChannelMapping_(CriSint32 nch, CriUint32 type)"/>
		/// </remarks>
		...
		/// </para>
		/// <nativeinfo declaration="CriSint32 criAtomEx_CalculateAdxBitrate_(CriSint32 num_channels, CriSint32 sampling_rate)"/>
		/// </remarks>
		...
		/// </para>
		/// <nativeinfo declaration="CriSint32 criAtomEx_CalculateHcaBitrate_(CriSint32 num_channels, CriSint32 sampling_rate, CriAtomEncodeQuality quality)"/>
		/// </remarks>
		...
		/// qualityにはCRI Atom CraftまたはCRI Atom Encoderで設定したエンコード品質を指定します。 
		/// </para>
		/// <nativeinfo declaration="CriSint32 criAtomEx_CalculateHcaMxBitrate_(CriSint32 num_channels, CriSint32 sampling_rate, CriAtomEncodeQuality quality)"/>
		/// </remarks>
		...
		/// </para>
		/// <nativeinfo declaration="CriBool criAtomEx_GetStreamingInfo_(CriAtomStreamingInfo *streaming_info)"/>
		/// </remarks>
		...
		/// Atomサーバー内の処理と一部排他制御しているため、 優先度逆転によりAtomサーバーを止めてしまわないように注意してください。 
		/// </para>
		/// <nativeinfo declaration="CriBool criAtomEx_SetFreeTimeBufferingFlagForDefaultDevice_(CriBool flag)"/>
		/// </remarks>
		...
		///  その結果、ワーク領域を必要とする関数（ <see cref="CriAtomExPlayer.CriAtomExPlayer"/> 関数等）に対し、 個別にワーク領域をセットする処理を省略することが可能になります。
		...
		///  本メソッドは内部的に <see cref="CriAtom.SetUserAllocator"/> メソッドや <see cref="CriAtom.SetUserMallocFunction"/> 関数、 <see cref="CriAtom.SetUserFreeFunction"/> 関数を呼び出します。
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_SetUserAllocator_(CriAtomMallocFunc p_malloc_func, CriAtomFreeFunc p_free_func, void *p_obj)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomEx.Initialize"/> 関数に設定するコンフィグ構造体（ <see cref="CriAtomEx.Config"/> ）に、 デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomEx_SetDefaultConfig_(CriAtomExConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>ユーザPCM出力方式用ワーク領域サイズ計算 </summary>
		...
		/// <para>
		/// 備考:
		/// ライブラリが必要とするワーク領域のサイズは、ライブラリ初期化用コンフィグ 構造体（ <see cref="CriAtomEx.ConfigForUserPcmOutput"/> ）の内容によって変化します。
		...
		public static unsafe Int32 CalculateWorkSizeForUserPcmOutput(in CriAtomEx.ConfigForUserPcmOutput config)
		{
			fixed (CriAtomEx.ConfigForUserPcmOutput* configPtr = &config)
				return NativeMethods.criAtomEx_CalculateWorkSizeForUserPcmOutput(configPtr);
		}
		...
		/// <para>
		/// 備考:
		/// ライブラリが必要とするワーク領域のサイズは、ライブラリ初期化用コンフィグ 構造体（ <see cref="CriAtomEx.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		public unsafe partial struct Config
		{
			...
			///  スレッドモデルが <see cref="CriAtomEx.ThreadModel.Multi"/> の場合、サーバー処理は CRI Atomライブラリが作成するスレッドで、定期的に実行されます。
			///  スレッドモデルが <see cref="CriAtomEx.ThreadModel.Single"/> や <see cref="CriAtomEx.ThreadModel.UserMulti"/> の場合、サーバー処理は <see cref="CriAtomEx.ExecuteMain"/> 関数内で実行されます。
			///  server_frequency には、サーバー処理を実行する頻度を指定します。
			///  スレッドモデルが <see cref="CriAtomEx.ThreadModel.Multi"/> の場合、CRI Atomライブラリは指定された 頻度でサーバー処理が実行されるよう、サーバー処理の呼び出し間隔を調節します。
			///  スレッドモデルが <see cref="CriAtomEx.ThreadModel.Single"/> や <see cref="CriAtomEx.ThreadModel.UserMulti"/> の場合、ユーザは <see cref="CriAtomEx.ExecuteMain"/> 関数を server_frequency で指定した頻度以上 で実行する必要があります。
			///  アプリケーションのフレームレートの変動が大きく、サーバー処理を実行する頻度にバラツキ ができてしまう場合には、最悪のフレームレートを想定して server_frequency の値を指定 するか、またはスレッドモデルに <see cref="CriAtomEx.ThreadModel.Multi"/> を指定してください。 
			/// </para>
			/// <para>
			/// 備考:
			/// Atomライブラリのサーバー処理では、以下のような処理が行われます。
			/// <list type="bullet">
			/// <item><description>発音リクエストの処理（ボイスの取得等）</description></item>
			/// <item><description>パラメーターの更新（ボリュームやパン、ピッチ等の変更の適用）</description></item>
			/// <item><description>音声データのデコードと出力</description></item>
			/// </list>
			/// </para>
			/// <para>
			/// 注意:
			/// スレッドモデルに <see cref="CriAtomEx.ThreadModel.Single"/> や <see cref="CriAtomEx.ThreadModel.UserMulti"/> を指定したにもかかわらず、 <see cref="CriAtomEx.ExecuteMain"/> 関数が server_frequency で 指定した値以下の頻度でしか実行されなかった場合、再生中の音が途切れる等の問題が 発生する可能性がありますので、ご注意ください。
			/// </para>
			/// <para>
			...
			///  CRI Atom Library以外の音声出力ライブラリを用いる場合は、本フラグにtrueを指定してから、<see cref="CriAtomEx.Initialize"/>を 実行するようにしてください。 
			/// </para>
			/// <para>
			/// 注意:
			/// 本フラグを切り替える際には、Atomライブラリを<see cref="CriAtomEx.Finalize"/>で終了してから、フラグの内容を変更し、再度 <see cref="CriAtomEx.Initialize"/>にて初期化処理を行うようにしてください。
			/// </para>
			/// </remarks>
			...
			///  <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定して AtomEx プレーヤーを作成する場合、 上記よりもさらに多くのバーチャルボイスを必要とする可能性があります。
			/// </para>
			/// </remarks>
			...
			///  キュー、トラック、カテゴリの AISAC が対象となります。 ミキサー AISAC は本設定値の対象ではありません。 
			/// </para>
			/// </remarks>
			...
			/// <para>
			/// 説明:
			/// AtomExPlayerに<see cref="CriAtomExPlayer.SetCategoryById"/>関数や<see cref="CriAtomExPlayer.SetCategoryByName"/>関数 を用いてカテゴリをセットした場合にキューのカテゴリ設定を上書きする機能を有効化します。 
			/// </para>
			/// <para>
			/// 備考：
			/// CRI Atom Ver.2.20.31未満のライブラリは、プレーヤーに対してカテゴリ設定を行うと、 キューに設定されていたカテゴリが上書きにより無効になっていました。 Ver.2.20.31未満の挙動に戻す必要がある場合には本フラグにfalseを設定してください。 
			/// </para>
			/// </remarks>
			...
			/// <para>
			/// 説明:
			/// 初期化時にACFの登録を行う際にACF情報へのポインタを指定します。 nullを指定した場合、初期化時にACFの登録は行われません。 本メンバを設定して初期化処理内でACFの登録を行う場合、 <see cref="CriAtomEx.Config"/> 構造体の max_voice_limit_groups, max_categories, categories_per_playbackの各メンバ値は ACF設定値と比較して大きい方の値が初期化用設定値として使用されます。 CriAtomEx初期化時にACFの登録を行った場合、環境によっては CriAtomExAsr, CriAtomExHcaMx 等のモジュール初期化にも一部ACF内の設定値が使用されます。
		...
		/// <param name="config">初期化用コンフィグ構造体 </param>
		/// <returns>CriBool 初期化できたかどうか？（できた：true／できない：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ライブラリを初期化します。
		///  ライブラリの機能を利用するには、必ずこの関数を実行する必要があります。
		///  （ライブラリの機能は、本関数を実行後、 <see cref="CriAtomEx.Finalize"/> 関数を実行するまでの間、 利用可能です。）
		...
		///  初期化処理の前に <see cref="CriAtomEx.CalculateWorkSize"/> 関数で取得したサイズ分のメモリを予め 確保しておき、本関数に設定してください。
		///  尚、Fixed Memory方式を用いた場合、ワーク領域はライブラリの終了処理（ <see cref="CriAtomEx.Finalize"/> 関数） を行うまでの間、ライブラリ内で利用され続けます。
		...
		/// <list type="number">
		/// <item><description>初期化処理実行前に、 <see cref="CriAtomEx.SetUserAllocator"/> 関数を用いてメモリ確保／解放関数を登録する。
		/// </description></item>
		/// <item><description>初期化用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description><see cref="CriAtomEx.Initialize"/> 関数で初期化処理を行う。
		///  （workにはnull、work_sizeには0を指定する。）
		/// </description></item>
		/// <item><description>アプリケーション終了時に <see cref="CriAtomEx.Finalize"/> 関数で終了処理を行う。
		/// </description></item>
		/// </list>
		/// </para>
		/// <list>
		...
		/// <item><description>初期化用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description>ライブラリの初期化に必要なワーク領域のサイズを、 <see cref="CriAtomEx.CalculateWorkSize"/> 関数を使って計算する。
		/// </description></item>
		...
		{
			fixed (CriAtomEx.Config* configPtr = &config)
				return NativeMethods.criAtomEx_Initialize(configPtr, default, default);
		}
		...
		///  ライブラリ初期化時（ <see cref="CriAtomEx.Initialize"/> 関数 ）に <see cref="CriAtomEx.Config"/> 構造体にて指定します。 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomEx.Initialize"/>
		/// <seealso cref="CriAtomEx.Config"/>
		public enum ThreadModel
		{
			...
			///  本機能に未対応のプラットフォームでは、ライブラリ初期化時に <see cref="CriAtomEx.ThreadModel.MultiWithSonicsync"/>が指定された場合でも、 <see cref="CriAtomEx.ThreadModel.Multi"/>指定時と同様の動作となります。
			/// </para>
			/// <para>
			/// 注意:
			...
			/// <list type="bullet">
			/// <item><description><see cref="CriAtomEx.GetPerformanceInfo"/></description></item>
			/// <item><description><see cref="CriAtomExAsrRack.GetPerformanceInfo"/> </description></item>
			/// </list>
			/// </para>
			/// </remarks>
			...
			/// ライブラリは内部でスレッドを作成し、マルチスレッドにて動作します。
			///  スレッドは <see cref="CriAtomEx.Initialize"/> 関数呼び出し時に作成されます。
			///  サーバー処理自体は作成されたスレッド上で実行されますが、 <see cref="CriAtomEx.ThreadModel.Multi"/> とは異なり、自動的には実行されません。
		...
		public unsafe partial struct RngInterface
		{
			...
			/// </para>
			/// </remarks>
			[System.NonSerialized] public delegate* unmanaged[Cdecl]<Int32> CalculateWorkSize;

			/// <summary>疑似乱数生成器の作成 </summary>
			/// <returns><see cref="IntPtr"/> 疑似乱数生成器オブジェクト </returns>
			/// <remarks>
			/// <para>
			/// 説明:
			/// 疑似乱数生成器を作成します。
			...
			/// </para>
			/// </remarks>
			[System.NonSerialized] public delegate* unmanaged[Cdecl]<IntPtr, Int32, IntPtr> Create;
			...
			/// </para>
			/// </remarks>
			[System.NonSerialized] public delegate* unmanaged[Cdecl]<IntPtr, void> Destroy;

			/// <summary>疑似乱数の生成 </summary>
			/// <returns>CriSint32 疑似乱数 </returns>
			/// <remarks>
			/// <para>
			/// 説明:
			/// 新しい疑似乱数を生成します。
			...
			/// </para>
			/// </remarks>
			[System.NonSerialized] public delegate* unmanaged[Cdecl]<IntPtr, Int32, Int32, Int32> Generate;
			...
			/// </para>
			/// </remarks>
			[System.NonSerialized] public delegate* unmanaged[Cdecl]<IntPtr, UInt32, void> SetSeed;

		}
		/// <summary>ユーザPCM出力用ライブラリ初期化 </summary>
		/// <param name="config">初期化用コンフィグ構造体 </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  ユーザは <see cref="CriAtomExAsr.GetPcmDataFloat32"/> 関数を使用してライブラリから定期的にPCMデータを取得し、 アプリケーション側で音声出力を行う必要があります。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数と以下の関数は併用できません。
		...
		/// <seealso cref="CriAtomEx.CalculateWorkSizeForUserPcmOutput"/>
		public static unsafe void InitializeForUserPcmOutput(in CriAtomEx.ConfigForUserPcmOutput config)
		{
			fixed (CriAtomEx.ConfigForUserPcmOutput* configPtr = &config)
				NativeMethods.criAtomEx_InitializeForUserPcmOutput(configPtr, default, default);
		}

		/// <summary>ユーザPCM出力用ライブラリ終了処理 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		{
			NativeMethods.criAtomEx_FinalizeForUserPcmOutput();
		}
		...
		///  ライブラリ初期化時にスレッドモデルを <see cref="CriAtomEx.ThreadModel.Multi"/> に設定した場合、 リアルタイム性の要求される処理は全てCRI Atomライブラリ内で定期的に自動実行されるため、 本関数の呼び出し頻度は少なくても問題は発生しません。
		///  （最低でも毎秒1回程度実行されていれば、音切れ等の問題が発生することはありません。）
		///  ライブラリ初期化時にスレッドモデルを <see cref="CriAtomEx.ThreadModel.Single"/> や <see cref="CriAtomEx.ThreadModel.UserMulti"/> に設定した場合、ファイルの読み込み管理や、 データのデコード、音声の出力等、音声再生に必要な処理のほぼ全てが本関数内で実行されます。
		///  また、音声再生処理に同期して、CRI File Systemライブラリのファイルアクセスとデータ展開処理を実行します。
		///  そのため、ライブラリ初期化時に指定したサーバー処理の実行頻度（ <see cref="CriAtomEx.Config"/> 構造体の server_frequency ）を下回る頻度で本関数を実行した場合や、 大きいデータの読み込み、圧縮ファイルの読み込み等を行う場合、 音切れ等の問題が発生する可能性があるので注意してください。
		/// </para>
		/// <para>
		/// 備考:
		/// ライブラリ初期化時にスレッドモデルを <see cref="CriAtomEx.ThreadModel.Multi"/> に設定した場合でも、 本関数を実行する必要があります。
		///  （スレッドモデルを <see cref="CriAtomEx.ThreadModel.Multi"/> の場合、ステータス更新等、ごく一部の 処理のみを行うため、本関数内で長時間処理がブロックされることはありません。） 
		...
		///  スレッドモデルが<see cref="CriAtomEx.ThreadModel.UserMulti"/>の場合、 アプリケーションは、この関数を定期的に実行する必要があります。
		///  ファイルの読み込み管理や、データのデコード、音声の出力等、 音声再生に必要な処理のほぼ全てが本関数内で実行されます。
		///  そのため、ライブラリ初期化時に指定したサーバー処理の実行頻度（ <see cref="CriAtomEx.Config"/> 構造体の server_frequency ）を下回る頻度で本関数を実行した場合、音切れ等の問題が発生する可能性 があります。
		...
		/// <list type="bullet">
		/// <item><description>ストリーム再生時のデータ供給に伴う発音遅延
		/// </description></item>
		/// <item><description>発音リソースの奪い取りに伴う発音遅延
		///  再生同期を行いたい場合は、<see cref="CriAtomExPlayer.Prepare"/> 関数を使用して再生準備を行い、 準備完了後に割り込み防止区間で再生を開始してください。
		...
		/// <returns>CriUint64 時刻（マイクロ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="sw">CRI_TRUE=タイマー一時停止、CRI_FALSE=タイマー再開 </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 説明:
		/// ワーク領域サイズの計算用に、ライブラリ初期化用コンフィグ構造体 （ <see cref="CriAtomEx.Config"/> 構造体）を仮登録します。
		///  ACFの登録やボイスプールの作成に必要なワーク領域のサイズは、 ライブラリ初期化時（ <see cref="CriAtomEx.Initialize"/> 関数実行時） に設定する構造体のパラメーターによって変化します。
		...
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfData"/> 関数</description></item>
		/// <item><description>ボイスプール作成用ワーク領域サイズの計算
		///  （ <see cref="CriAtomExVoicePool.CalculateWorkSizeForStandardVoicePool"/> 関数等）</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		/// 引数（ config ）に null を指定した場合、デフォルト設定 （ <see cref="CriAtomEx.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。
		...
		///  （本関数で登録する構造体のパラメーターと、 ライブラリの初期化に使用する構造体のパラメーターが異なる場合、 ワーク領域サイズが不足し、オブジェクトの作成に失敗する恐れがあります。）
		...
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomEx.RegisterAcfData"/> 関数でACF情報を登録する際には、 本関数が返すサイズ分のメモリをワーク領域として渡す必要があります。
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。
		/// </para>
		/// <para>
		...
		/// <param name="acfData">ACFデータアドレス </param>
		/// <param name="acfDataSize">ACFデータサイズ </param>
		/// <returns>CriBool データの登録に成功したか </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// メモリ上に配置されたACFデータをライブラリに取り込みます。
		///  ACF情報の登録に必要なワーク領域のサイズは、 <see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfData"/> 関数で計算します。
		///  ACFファイルの登録に成功すると、本関数は戻り値として true を返します。
		///  データ不正などの理由等によりACFファイルの読み込みに失敗した場合、本関数は戻り値 としてfalse を返します。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <list type="number">
		/// <item><description>AISAC</description></item>
		/// <item><description>AISACコントロール値</description></item>
		/// <item><description>カテゴリ</description></item>
		/// <item><description>セレクターラベル</description></item>
		/// <item><description>バスセンド</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomEx.UnregisterAcf"/></see>
		/// <see><see cref="CriAtomEx.UnregisterAcf"/></see>
		/// <para>
		///  本関数にセットしたデータ領域とワーク領域は、 
		///  関数を実行するまでの間、 アプリケーションで保持する必要があります。
		///  （ 
		...
		///  本関数は、 <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録の有無によって 計算に使用する要素が異なります。<list type="bullet">
		/// <item><description>アロケーター登録時：ACFファイルを一時的に読み込み、ACF内に記録されているカテゴリ数、 再生単位でのカテゴリ参照数、REACT数を使用したサイズ計算が行われます。
		/// </description></item>
		/// <item><description>アロケーター未登録時：ライブラリ初期化時に指定した <see cref="CriAtomEx.Config"/> 構造体の max_categoriesメンバ、categories_per_playbackメンバを使用したサイズ計算が行われます。
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。
		/// </description></item>
		/// </list>
		...
		///  ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfFile"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		/// <param name="binder">バインダーハンドル </param>
		/// <param name="path">ファイルパス </param>
		/// <returns>CriBool ファイル読み込み結果 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ACFファイルをロードし、ライブラリに取り込みます。
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドを使用してアロケーターを登録済みの場合、 本関数にワーク領域を指定する必要はありません。
		///  （ work に null 、 work_size に 0 を指定することで、登録済みのアロケーター から必要なワーク領域サイズ分のメモリが動的に確保されます。） ワーク領域を指定して本関数を使用する場合、 <see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfFile"/> 関数 を使用してワークサイズを計算してください。
		///  ACFファイルの登録に成功すると、本関数は戻り値として true を返します。
		///  リードエラー等によりACFファイルの読み込みに失敗した場合、本関数は戻り値として false を返します。
		/// </para>
		/// <para>
		/// 備考:
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomEx.SetUserAllocator"/></see>
		/// <see><see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfFile"/></see>
		/// <see><see cref="CriAtomEx.Config"/></see>
		/// <list>
		/// <list type="bullet">
		/// <item><description>max_categories：ACF内のカテゴリ数、REACT数と同値以上
		/// </description></item>
		/// <item><description>categories_per_playback：ACF内の再生単位でのカテゴリ参照数と同値以上
		///  本関数にセットしたワーク領域は、 <see cref="CriAtomEx.UnregisterAcf"/> 関数を実行するまでの間、 アプリケーションで保持する必要があります。
		///  （ <see cref="CriAtomEx.UnregisterAcf"/> 関数実行前に、ワーク領域のメモリを解放しないでください。） <see cref="CriAtomEx.UnregisterAcf"/></description></item>
		/// </list>
		/// </list>
		/// <para>
		///  マクロによるアロケーター登録を行わずに 
		///  関数によって計算したワークサイズ分の ワーク領域を指定した本関数の呼び出しでCRI_FALSEが返された場合、ワーク領域不足が要因 の可能性があります。
		///  ライブラリ初期化時に指定する 
		...
		{
			return NativeMethods.criAtomEx_RegisterAcfFile(binder?.NativeHandle ?? default, path.GetPointer(stackalloc byte[path.BufferSize]), default, default);
		}
		...
		/// 説明:
		/// ACFファイルをロードし、ライブラリに取り込みます。
		///  ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomEx.RegisterAcfFile"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		{
			return NativeMethods.criAtomEx_RegisterAcfFileById(binder?.NativeHandle ?? default, id, default, default);
		}
		...
		/// <returns>CriUint32 ACFフォーマットバージョン </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="path">ファイルパス </param>
		/// <param name="flag">レジスト可能フラグ </param>
		/// <returns>CriUint2 ACFフォーマットバージョン </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ACFファイルをロードし、ACFデータのフォーマットバージョンを取得します。
		///  ACF情報の登録に必要なワーク領域のサイズは、 <see cref="CriAtomEx.CalculateWorkSizeForRegisterAcfFile"/> 関数で計算します。
		...
		{
			fixed (NativeBool* flagPtr = &flag)
				return NativeMethods.criAtomEx_GetAcfVersionFromFile(binder?.NativeHandle ?? default, path.GetPointer(stackalloc byte[path.BufferSize]), default, default, flagPtr);
		}
		...
		/// 説明:
		/// ACFファイルをロードし、ACFデータのフォーマットバージョンを取得します。
		///  ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomEx.GetAcfVersionFromFile"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		{
			fixed (NativeBool* flagPtr = &flag)
				return NativeMethods.criAtomEx_GetAcfVersionFromFileById(binder?.NativeHandle ?? default, id, default, default, flagPtr);
		}
		...
		/// <returns>CriBool フォーマット情報が取得できたかどうか？（取得できた：true／取得できない：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// メモリにロードされた音声データのフォーマットを解析します。
		///  解析に成功すると、本関数は true を返し、音声データのフォーマット情報を 第3引数（ info ）に格納します。
		...
		/// <para>
		/// 備考:
		/// メモリ上に配置された音声データについては、 <see cref="CriAtomEx.AnalyzeAudioHeader"/> 関数を実行することで音声データのフォーマット情報を取得可能です。
		///  再生中の音声データのフォーマットについては <see cref="CriAtomExPlayback.GetFormatInfo"/> 関数で取得可能です。
		/// </para>
		/// </remarks>
		...
		/// <summary>ACBハンドルが即時解放可能かどうかのチェック </summary>
		/// <param name="buffer">バッファー </param>
		/// <param name="size">バッファーサイズ </param>
		/// <returns>CriBool 再生中かどうか（true = 再生中のプレーヤーあり、false = 再生中のプレーヤーなし） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  （ <see cref="CriAtomExPlayer.StopWithoutReleaseTime"/> 関数で停止処理を行った場合や、 ボイスの奪い取りが発生した場合、AtomExプレーヤーからボイスは切り離されますが、 ボイスが完全に停止するまでの間、データは参照される可能性があります。）
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行すると、指定したデータ領域を参照しているAtomプレーヤーの存在を 検索する処理が動作します。
		...
		/// <returns>CriSint32 必要ワーク領域サイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// 説明:
		/// DSPバス設定からDSPバスを構築するのに必要なワーク領域サイズを計算します。
		/// <see cref="CriAtomEx.CalculateWorkSizeForDspBusSetting"/> 関数と違い、 ACF情報を登録する前でも本関数は実行可能です。
		///  （ただし、ACFデータを事前にメモリにロードし、 <see cref="CriAtomExAsr.SetConfigForWorkSizeCalculation"/> 関数でASR初期化用コンフィグ構造体を仮登録しておく必要があります。）
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、 エラーコールバックのメッセージで確認可能です。
		/// </para>
		/// <para>
		...
		/// <list type="bullet">
		/// <item><description>もとのチャンネル数、サンプリングレート、機種固有設定などを上書きします。
		/// </description></item>
		/// <item><description>アタッチ時に再生中の音声をすべて停止します
		/// </description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		{
			NativeMethods.criAtomEx_AttachDspBusSetting(setting.GetPointer(stackalloc byte[setting.BufferSize]), default, default);
		}
		...
		/// <returns>const CriChar8* スナップショット名文字列へのポインタ。 スナップショットが適用されていない場合や取得に失敗した場合はCRI_NULLが返ります。 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  取得した情報を元に、<see cref="CriAtomExAcb.GetCueInfoByIndex"/> 関数等を利用することで、 より詳細な情報を取得することができます。 
		/// </para>
		/// <para>
		/// 備考
		...
		public enum SourceType
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>サウンドジェネレータID </para>
			/// </remarks>
			SoundGeneratorId = 9,
			/// <summary></summary>
			/// <remarks>
			/// <para>RawPcmFloatID </para>
			/// </remarks>
			RawPcmFloatId = 10,
			/// <summary></summary>
			/// <remarks>
		...
		/// <param name="sw">スイッチ（CRI_FALSE = 無効、CRI_TRUE = 有効） </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// パン3Dや3Dポジショニングの計算時にバーチャルスピーカーの設定を使用する機能のON/OFFを設定します。
		///  この設定を有効にすると、マルチチャンネルサウンドは <see cref="CriAtomEx.SetVirtualSpeakerAngleArray"/> 関数にて設定した バーチャルスピーカー角度からそれぞれ再生されます。 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriSint32 ゲーム変数の総数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriFloat32 ゲーム変数値 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// ゲーム変数の値は以下のサウンド制御において参照されます。<list type="bullet">
		/// <item><description>スイッチキューによる再生トラックの切り替え</description></item>
		/// <item><description>AISAC によるパラメーターの制御 </description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <param name="sw">スイッチ（CRI_FALSE = チェック無効、CRI_TRUE = チェック有効） </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <summary>トラックトランジションバイセレクターコールバック </summary>
		/// <returns>
		/// 未使用 AtomExライブラリのトラックトランジションバイセレクターコールバック関数型です。
		///  コールバック関数の登録には <see cref="CriAtomEx.SetTrackTransitionBySelectorCallback"/> 関数を使用します。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		public unsafe partial struct ResourceUsage
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>対象リソースの現在の使用数 </para>
			/// </remarks>
			public UInt32 useCount;

			/// <summary></summary>
			/// <remarks>
			/// <para>対象リソースの制限数 </para>
			/// </remarks>
			public UInt32 limit;
		...
		/// <para>
		/// 注意:
		/// typeが<see cref="CriAtomExAcf.AisacType.AutoModulation"/> の場合、 control_idは内部的に使用されるインデックス値となります。 
		/// </para>
		/// </remarks>
		...
		public enum AisacGraphType
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>バンドパスフィルターの高域カットオフ周波数 </para>
			/// </remarks>
			BandpassHi = 3,
			/// <summary></summary>
			/// <remarks>
			/// <para>バンドパスフィルターの低域カットオフ周波数 </para>
			/// </remarks>
			BandpassLow = 4,
			/// <summary></summary>
			/// <remarks>
			/// <para>バイクアッドフィルターの周波数 </para>
			/// </remarks>
			BiquadFreq = 5,
			/// <summary></summary>
			/// <remarks>
			/// <para>バイクアッドフィルターのQ値 </para>
			/// </remarks>
			BiquadQ = 6,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>パンニング3D方位角度 </para>
			/// </remarks>
			Pan3dAngle = 15,
			/// <summary></summary>
			/// <remarks>
			/// <para>パンニング3Dボリューム </para>
			/// </remarks>
			Pan3dVolume = 16,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>ACB Ver.0.11.00以降では使用しない </para>
			/// </remarks>
			Pan3dCenter = 18,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 0 </para>
			/// </remarks>
			Aisac0 = 20,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 1 </para>
			/// </remarks>
			Aisac1 = 21,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 2 </para>
			/// </remarks>
			Aisac2 = 22,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 3 </para>
			/// </remarks>
			Aisac3 = 23,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 4 </para>
			/// </remarks>
			Aisac4 = 24,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 5 </para>
			/// </remarks>
			Aisac5 = 25,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 6 </para>
			/// </remarks>
			Aisac6 = 26,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 7 </para>
			/// </remarks>
			Aisac7 = 27,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 8 </para>
			/// </remarks>
			Aisac8 = 28,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 9 </para>
			/// </remarks>
			Aisac9 = 29,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 10 </para>
			/// </remarks>
			Aisac10 = 30,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 11 </para>
			/// </remarks>
			Aisac11 = 31,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 12 </para>
			/// </remarks>
			Aisac12 = 32,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 13 </para>
			/// </remarks>
			Aisac13 = 33,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 14 </para>
			/// </remarks>
			Aisac14 = 34,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロールID 15 </para>
			/// </remarks>
			Aisac15 = 35,
			/// <summary></summary>
			/// <remarks>
			/// <para>ボイスプライオリティ </para>
			/// </remarks>
			Priority = 36,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>バイクアッドフィルターのゲイン </para>
			/// </remarks>
			BiquadGain = 38,
			/// <summary></summary>
			/// <remarks>
			/// <para>パンニング3D センターレベル </para>
			/// </remarks>
			Pan3dMixdownCenter = 39,
			/// <summary></summary>
			/// <remarks>
			/// <para>パンニング3D LFEレベル </para>
			/// </remarks>
			Pan3dMixdownLfe = 40,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープ アタック </para>
			/// </remarks>
			EgAttack = 41,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープ リリース </para>
			/// </remarks>
			EgRelease = 42,
			/// <summary></summary>
			/// <remarks>
			/// <para>シーケンス再生レシオ </para>
			/// </remarks>
			PlaybackRatio = 43,
			/// <summary></summary>
			/// <remarks>
			/// <para>L chドライセンド </para>
			/// </remarks>
			DrySendL = 44,
			/// <summary></summary>
			/// <remarks>
			/// <para>R chドライセンド </para>
			/// </remarks>
			DrySendR = 45,
			/// <summary></summary>
			/// <remarks>
			/// <para>Center chドライセンド </para>
			/// </remarks>
			DrySendCenter = 46,
			/// <summary></summary>
			/// <remarks>
			/// <para>LFE chドライセンド </para>
			/// </remarks>
			DrySendLfe = 47,
			/// <summary></summary>
			/// <remarks>
			/// <para>Surround L chドライセンド </para>
			/// </remarks>
			DrySendSl = 48,
			/// <summary></summary>
			/// <remarks>
			/// <para>Surround R chドライセンド </para>
			/// </remarks>
			DrySendSr = 49,
			/// <summary></summary>
			/// <remarks>
			/// <para>Ex1 chドライセンド </para>
			/// </remarks>
			DrySendEx1 = 50,
			/// <summary></summary>
			/// <remarks>
			/// <para>Ex2 chドライセンド </para>
			/// </remarks>
			DrySendEx2 = 51,
			/// <summary></summary>
			/// <remarks>
			/// <para>マルチチャンネル音源の広がり </para>
			/// </remarks>
			Wideness = 52,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>任意AISACコントロールID </para>
			/// </remarks>
			ArbitraryAisacControl = 55,
			/// <summary></summary>
			/// <remarks>
			/// <para>パンニング3D仰俯角度 </para>
			/// </remarks>
		...
		/// <see cref="CriAtomExAcf.GetSelectorInfoByIndex"/> 関数または <see cref="CriAtomExAcf.GetSelectorInfoByName"/> 関数に 引数として渡します。
		/// </para>
		/// <para>注意:</para>
		/// </remarks>
		...
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomExAcb.GetWaveformInfoById"/>
		/// <seealso cref="CriAtomExAcb.GetWaveformInfoByName"/>
		...
		public unsafe partial struct CueInfo
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>セレクターインデックス（スイッチ変数のみ） </para>
			/// </remarks>
		...
		public enum PanType
		{
			...
			/// <para>
			/// 説明:
			/// パンタイプが判明していない状態です。 <see cref="CriAtomExAcb.GetCueInfoByName"/> 関数などにて、 ACB Ver.1.35.00 未満の ACB データ内のキューの情報（ <see cref="CriAtomEx.CueInfo"/> ）を取得した場合に得られます。 
			/// </para>
			/// <para>
			/// 注意:
		...
		public unsafe partial struct CuePos3dInfo
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>Zero距離InteriorPan適用距離 </para>
			/// </remarks>
			public Single sourceRadius;

			/// <summary></summary>
			/// <remarks>
			/// <para>InteriorPan適用境界距離 </para>
			/// </remarks>
		...
		/// <list type="bullet">
		/// <item><description>ボリューム</description></item>
		/// <item><description>3Dパンニングの演算結果によるボリューム</description></item>
		/// <item><description>3Dポジショニングの演算結果によるボリューム</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		public enum SilentMode
		{
			...
			/// <para>
			/// 説明:
			/// 無音となった際は自動的に一時停止復帰型でバーチャル化します。 復帰する際はバーチャル化した位置から再生されます。 
			/// </para>
			/// </remarks>
			VirtualResume = 4,
		}
		...
		///  プールできるボイス数の最大数は <see cref="CriAtomEx.MaxVoicesPerPool"/> で、 最小数は <see cref="CriAtomEx.MinVoicesPerPool"/> です。
		/// </para>
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForStandardVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForAdxVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForHcaVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForWaveVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForAiffVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForRawPcmVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForInstrumentVoicePool"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		public unsafe partial struct AisacInfo
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>デフォルトコントロール値が設定されているか </para>
			/// </remarks>
			public NativeBool defaultControlFlag;

			/// <summary></summary>
			/// <remarks>
			/// <para>デフォルトAISACコントロール値 </para>
			/// </remarks>
		...
		public enum ReactDuckerTargetType
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>ボリュームのダッカー </para>
			/// </remarks>
			Volume = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>AISACコントロール値のダッカー </para>
			/// </remarks>
			AisacControlValue = 1,
		}
		...
		public unsafe partial struct ReactFadeParameter
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>変化曲線の強さ（0.0f ～ 2.0f） </para>
			/// </remarks>
			public Single curveStrength;

			/// <summary></summary>
			/// <remarks>
			/// <para>フェード時間（ミリ秒） </para>
			/// </remarks>
		...
		public enum ReactHoldType
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>再生中にホールドを行う </para>
			/// </remarks>
			WhilePlaying = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>固定時間でホールドを行う </para>
			/// </remarks>
			FixedTime = 1,
		}
		...
		public enum VoiceAllocationMethod
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>ボイスの確保は1回限り </para>
			/// </remarks>
			AllocateVoiceOnce = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>ボイスを繰り返し確保する </para>
			/// </remarks>
		...
		/// <see cref="CriAtomExPlayer.Resume"/> 関数、および <see cref="CriAtomExPlayback.Resume"/> 関数の引数として使用します。 
		/// </para>
		/// </remarks>
		...
		public enum ResumeMode
		{
			/// <summary></summary>
			/// <remarks>
			/// <para>一時停止方法に関係なく再生を再開 </para>
			/// </remarks>
			AllPlayback = 0,
			/// <summary></summary>
			/// <remarks>
			/// <para>Pause 関数でポーズをかけた音声のみ再生を再開 </para>
			/// </remarks>
			PausedPlayback = 1,
			/// <summary></summary>
			/// <remarks>
			/// <para>Prepare 関数で再生準備を指示した音声の再生を開始 </para>
			/// </remarks>
			PreparedPlayback = 2,
			ModeReserved = 3,
		}
		...
		public enum ParameterId
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>パンスピーカータイプ </para>
			/// </remarks>
			PanSpeakerType = 6,
			/// <summary></summary>
			/// <remarks>
			/// <para>2Dパン（チャンネル0） </para>
			/// </remarks>
			PanCh0 = 7,
			/// <summary></summary>
			/// <remarks>
			/// <para>2Dパン（チャンネル1） </para>
			/// </remarks>
			PanCh1 = 8,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>バイクアッドフィルターのフィルタータイプ </para>
			/// </remarks>
			BiquadFilterType = 19,
			/// <summary></summary>
			/// <remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープのアタックタイム </para>
			/// </remarks>
			EnvelopeAttackTime = 23,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープのホールドタイム </para>
			/// </remarks>
			EnvelopeHoldTime = 24,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープのディケイタイム </para>
			/// </remarks>
			EnvelopeDecayTime = 25,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープのリリースタイム </para>
			/// </remarks>
			EnvelopeReleaseTime = 26,
			/// <summary></summary>
			/// <remarks>
			/// <para>エンベロープのサスティンレベル </para>
			/// </remarks>
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター0 </para>
			/// </remarks>
			DspParameter0 = 33,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター1 </para>
			/// </remarks>
			DspParameter1 = 34,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター2 </para>
			/// </remarks>
			DspParameter2 = 35,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター3 </para>
			/// </remarks>
			DspParameter3 = 36,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター4 </para>
			/// </remarks>
			DspParameter4 = 37,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター5 </para>
			/// </remarks>
			DspParameter5 = 38,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター6 </para>
			/// </remarks>
			DspParameter6 = 39,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター7 </para>
			/// </remarks>
			DspParameter7 = 40,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター8 </para>
			/// </remarks>
			DspParameter8 = 41,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター9 </para>
			/// </remarks>
			DspParameter9 = 42,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのパラメーター10 </para>
			/// </remarks>
			DspParameter10 = 43,
			/// <summary></summary>
			/// <remarks>
			/// <para>インサーションDSPのバイパスフラグ </para>
			/// </remarks>
			DspBypassFlag = 44,
		}
		...
		public enum BiquadFilterType
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>ローシェルフフィルター </para>
			/// </remarks>
			Lowshelf = 4,
			/// <summary></summary>
			/// <remarks>
			/// <para>ハイシェルフフィルター </para>
			/// </remarks>
			Highshelf = 5,
			/// <summary></summary>
			/// <remarks>
			/// <para>ピーキングフフィルター </para>
			/// </remarks>
			Peaking = 6,
		}
		...
		/// <param name="flag">AISACコントロール値を適用するか？（CRI_TRUE:する、CRI_FALSE:しない） </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForDspPitchShifter"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExVoicePool.SetDefaultConfigForDspTimeStretch"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		...
		///  コールバック関数のその他の引数については、 別途 <see cref="CriAtomEx.VoiceEventCbFunc"/> の説明をご参照ください。
		///  funcにnullを指定することで登録済み関数の登録解除が行えます。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <see cref="CriAtomEx.SetVoiceEventCallback"/> 関数に本関数型のコールバック関数を登録することで、 ボイスイベント発生時にコールバックを受け取ることが可能となります。
		...
		///  ボイス取得前にキューリミット制御で発音が棄却された場合、 本コールバックに<see cref="CriAtomEx.VoiceEvent.Reject"/>等の情報は返されません。
		///  （カテゴリキュープライオリティによる制御に関する情報は、現状取得できません。）
		///  ボイスイベントコールバック時点では、 request->atom_player には再生すべき音声データがまだセットされていません。
		///  そのため、再生する音声データの情報等については、 atom_player に問い合わせず、 <see cref="CriAtomEx.VoiceInfoDetail"/> 構造体のメンバ値を使用してください。
		///  本コールバック関数内で長時間処理をブロックすると、音切れ等の問題が発生しますので、 ご注意ください。
		/// </para>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		public enum VoiceEvent
		{
			...
			/// 再生中のボイスが奪い取られたことを示す値です。
			///  再生中のボイスが1つ停止され、そのボイスが別の音声の再生に再利用されました。
			/// <see cref="CriAtomEx.VoiceEvent.AllocateAndRemove"/> と異なり、 停止される波形データと新規に再生する波形データとが、 同一のボイスリミットグループに所属する場合に本イベントが発生します。
			/// </para>
			/// </remarks>
		...
		public unsafe partial struct VoiceInfoDetail
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>ストリーム再生かどうか </para>
			/// </remarks>
			public NativeBool streamingFlag;

			/// <summary></summary>
			/// <remarks>
			/// <para>発音に使用するプレーヤー </para>
			/// </remarks>
		...
		///  コールバック関数には、再生中のボイスに関する詳細情報が <see cref="CriAtomEx.VoiceInfoDetail"/> 構造体として渡されます。
		/// </para>
		/// <para>
		/// 備考:
		/// 第 2 引数（ obj ）にセットした値は、コールバック関数の引数として渡されます。
		///  コールバック関数のその他の引数については、 別途 <see cref="CriAtomEx.VoiceInfoCbFunc"/> の説明をご参照ください。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <see cref="CriAtomEx.EnumerateVoiceInfos"/> 関数に本関数型のコールバック関数を登録することで、 再生中のボイスの情報をコールバックで受け取ることが可能となります。
		/// </para>
		/// <para>
		/// 注意:
		/// 本コールバック関数内で長時間処理をブロックすると、音切れ等の問題が発生しますので、 ご注意ください。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  コールバック関数には、停止ボイスに関する情報が <see cref="CriAtomEx.MonitoringVoiceStopInfo"/> 構造体として渡されます。
		/// </para>
		/// <para>
		/// 備考:
		/// 第 2 引数（ obj ）にセットした値は、コールバック関数の引数として渡されます。
		///  コールバック関数のその他の引数については、 別途 <see cref="CriAtomEx.MonitoringVoiceStopCbFunc"/> の説明をご参照ください。
		/// <see cref="CriAtomEx.SetMonitoringVoiceStopPlaybackId"/> 関数にて設定した監視再生IDにて再生中のキューが、 以下の構造・設定を持つ場合にコールバック関数にて通知される再生IDが監視再生IDとは異なることがあります。
		///  これは再生内部処理ににて別途再生IDが割り振られるためとなります。
		/// <list type="bullet">
		/// <item><description>キューリンクを使用している
		/// </description></item>
		/// <item><description>シーケンスタイプがトラック遷移タイプのキュー
		/// </description></item>
		/// </list>
		/// </para>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  ボイスプール作成時にデフォルト値設定を使用すると、 <see cref="CriAtomEx.DefaultVoicesPerPool"/> 数分のボイスが確保されます。
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomExVoicePool.SetDefaultConfigForStandardVoicePool"/>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で音声の再生を開始した際、 ボイスリミットコントロール等によりボイスが確保できなかった場合に返される、 無効な再生IDです。
		/// </para>
		/// <para>
		/// 備考:
		/// 再生 ID を指定する API に対して本 ID をセットした場合でも、 エラーコールバックは発生しません。
		///  （何もされずに関数からリターンします。）
		///  そのため、 <see cref="CriAtomExPlayer.Start"/> 関数の結果にかかわらず、 再生 ID を使用した処理を常時行っても、特に問題ありません。
		/// </para>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayback.GetCurrentBlockIndex"/> 関数で再生中の音声のカレントブロック インデックスを取得した際、再生中の音声がブロックシーケンスではない場合に 返される無効なインデックスです。
		/// </para>
		/// </remarks>
		...
		/// <see cref="CriAtomExPlayer.SetFadeOutTime"/> 関数の第2引数に本パラメーターをセットすることで、 フェードアウト処理を無効化することが可能です。
		/// </para>
		/// </remarks>
