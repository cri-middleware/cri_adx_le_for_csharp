...
namespace CriWare
{
	...
	{
		/// <summary>3Dリスナーハンドル作成用コンフィグ構造体にデフォルト値をセット </summary>
		/// <param name="pConfig">3Dリスナーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリスナーオブジェクト作成用コンフィグ構造体（ <see cref="CriAtomEx3dListener.Config"/> ）に、デフォルトの値をセットします。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dListener_SetDefaultConfig_(CriAtomEx3dListenerConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>3Dリスナーハンドルの作成に必要なワーク領域サイズの計算 </summary>
		/// <param name="config">3Dリスナーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>CriSint32 3Dリスナーオブジェクト作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリスナーオブジェクトを作成するために必要な、ワーク領域のサイズを取得します。
		///  アロケーターを登録せずに3Dリスナーオブジェクトを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomEx3dListener.CriAtomEx3dListener"/> 関数にセットする必要があります。
		///  3Dリスナーオブジェクトの作成に必要なワークメモリのサイズは、3Dリスナーオブジェクト作成用コンフィグ 構造体（ <see cref="CriAtomEx3dListener.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx3dListener.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <summary>3Dリスナーハンドル作成用コンフィグ構造体 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリスナーオブジェクトを作成する場合に使用する構造体です。
		///  現状指定可能なパラメーターはありませんが、将来パラメーターが追加される可能性があるため、 本構造体を使用する際には <see cref="CriAtomEx3dListener.SetDefaultConfig"/> メソッドを使用し、 構造体の初期化を行ってください。 
		/// </para>
		/// </remarks>
		...
		/// <summary>3Dリスナーハンドルの作成 </summary>
		/// <param name="config">3Dリスナーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns><see cref="CriAtomEx3dListener"/> 3Dリスナーオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  必要なメモリのサイズは、 <see cref="CriAtomEx3dListener.CalculateWorkSize"/> 関数で計算します。
		...
		public unsafe CriAtomEx3dListener(in CriAtomEx3dListener.Config config)
		{
			fixed (CriAtomEx3dListener.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomEx3dListener_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomEx3dListener()
		{
			CriAtomEx3dListener.Config* configPtr = null;
			NativeHandle = NativeMethods.criAtomEx3dListener_Create(configPtr, default, default);
		}

		/// <summary>3Dリスナーハンドルの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリスナーオブジェクトを破棄します。
...
#pragma warning restore 1591
		...
		/// <summary>3Dリスナーハンドルに対する3Dリージョンハンドルの設定 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
