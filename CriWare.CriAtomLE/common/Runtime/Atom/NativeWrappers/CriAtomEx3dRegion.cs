...
namespace CriWare
{
	...
	{
		/// <summary>3Dリージョンハンドル作成用コンフィグ構造体にデフォルト値をセット </summary>
		/// <param name="pConfig">3Dリージョンハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリージョンオブジェクト作成用コンフィグ構造体（ <see cref="CriAtomEx3dRegion.Config"/> ）に、デフォルトの値をセットします。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dRegion_SetDefaultConfig_(CriAtomEx3dRegionConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>3Dリージョンハンドルの作成に必要なワーク領域サイズの計算 </summary>
		/// <param name="config">3Dリージョンハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>CriSint32 3Dリージョンオブジェクト作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリージョンオブジェクトを作成するために必要な、ワーク領域のサイズを取得します。
		///  アロケーターを登録せずに3Dリージョンオブジェクトを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomEx3dRegion.CriAtomEx3dRegion"/> 関数にセットする必要があります。
		///  3Dリージョンオブジェクトの作成に必要なワークメモリのサイズは、3Dリージョンオブジェクト作成用コンフィグ 構造体（ <see cref="CriAtomEx3dRegion.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx3dRegion.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <summary>3Dリージョンハンドル作成用コンフィグ構造体 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリージョンオブジェクトを作成する場合に使用する構造体です。
		///  現状指定可能なパラメーターはありませんが、将来パラメーターが追加される可能性があるため、 本構造体を使用する際には <see cref="CriAtomEx3dRegion.SetDefaultConfig"/> メソッドを使用し、 構造体の初期化を行ってください。 
		/// </para>
		/// </remarks>
		...
		/// <summary>3Dリージョンハンドルの作成 </summary>
		/// <param name="config">3Dリージョンハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>criAtomEx3dRegionHn 3Dリージョンオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  必要なメモリのサイズは、 <see cref="CriAtomEx3dRegion.CalculateWorkSize"/> 関数で計算します。
		...
		public unsafe CriAtomEx3dRegion(in CriAtomEx3dRegion.Config config)
		{
			fixed (CriAtomEx3dRegion.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomEx3dRegion_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomEx3dRegion()
		{
			CriAtomEx3dRegion.Config* configPtr = null;
			NativeHandle = NativeMethods.criAtomEx3dRegion_Create(configPtr, default, default);
		}

		/// <summary>3Dリージョンハンドルの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dリージョンオブジェクトを破棄します。
