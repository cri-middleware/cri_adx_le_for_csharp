...
namespace CriWare
{
	...
	public partial class CriAtomEx3dSource : IDisposable, Interfaces.IPositionable
	{
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dSource_SetDefaultConfigForRandomPosition_(CriAtomEx3dSourceRandomPositionConfig *p_config)"/>
		/// </remarks>
		...
		/// <list type="bullet">
		/// <item><description>最小距離：0.0f</description></item>
		/// <item><description>最大距離：0.0f</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dSource_SetMinMaxDistance_(CriAtomEx3dSourceHn ex_3d_source, CriFloat32 min_attenuation_distance, CriFloat32 max_attenuation_distance)"/>
		/// </remarks>
		...
		///  本関数によってデフォルト値を変更すると、以降に作成する3D音源オブジェクト（ <see cref="CriAtomEx3dSource"/> ）の 最小距離／最大距離の初期値が本関数で設定した値となります。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description>ツールのプロパティにて、最小距離／最大距離の初期値設定を0.0以外に設定している</description></item>
		/// <item><description>インゲームプレビュー用にビルドしている</description></item>
		/// </list>
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dSource_ChangeDefaultMinMaxDistance_(CriFloat32 min_attenuation_distance, CriFloat32 max_attenuation_distance)"/>
		/// </remarks>
		...
		/// <summary>3D音源ハンドル作成用コンフィグ構造体にデフォルト値をセット </summary>
		/// <param name="pConfig">3D音源ハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクト作成用コンフィグ構造体（ <see cref="CriAtomEx3dSource.Config"/> ）に、デフォルトの値をセットします。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dSource_SetDefaultConfig_(CriAtomEx3dSourceConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>3D音源ハンドルの作成に必要なワーク領域サイズの計算 </summary>
		/// <param name="config">3D音源ハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>CriSint32 3D音源オブジェクト作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトを作成するために必要な、ワーク領域のサイズを取得します。
		///  アロケーターを登録せずに3D音源オブジェクトを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomEx3dSource.CriAtomEx3dSource"/> 関数にセットする必要があります。
		///  3D音源オブジェクトの作成に必要なワークメモリのサイズは、3D音源オブジェクト作成用コンフィグ 構造体（ <see cref="CriAtomEx3dSource.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx3dSource.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <summary>3D音源ハンドル作成用コンフィグ構造体 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトを作成する場合に使用する構造体です。
		///  将来パラメーターが追加される可能性があるため、 本構造体を使用する際には <see cref="CriAtomEx3dSource.SetDefaultConfig"/> メソッドを使用し、 構造体の初期化を行ってください。 
		/// </para>
		/// </remarks>
		...
		public unsafe partial struct Config
		{
			...
			///  本パラメーターをtrueに設定して3D音源オブジェクトを作成すると、その3D音源オブジェクトで発音される 3D音のボイスプライオリティは、リスナーとの距離によって減衰を受けるようになります。
			...
			/// <list type="bullet">
			/// <item><description>データに設定されている値</description></item>
			/// <item><description><see cref="CriAtomExPlayer.SetVoicePriority"/> 関数による設定値</description></item>
			/// <item><description>距離によるボイスプライオリティ減衰値</description></item>
			/// </list>
			/// </para>
			/// <para>本パラメーターのデフォルト値はCRI_FALSE（距離によるボイスプライオリティ無効）です。 </para>
			/// </remarks>
			...
			/// <para>
			/// 備考:
			/// 3D音源における位置のランダム化を使用しない場合や <see cref="CriAtomEx3dSource.RandomPositionConfig"/> 構造体の変数 calculation_type に 対して、 <see cref="CriAtomEx3dSource.RandomPositionCalculationType.List"/> を指定 しない場合、本設定値は 0 を指定してください。 
			/// </para>
			/// </remarks>
			/// <seealso cref="CriAtomEx3dSource.RandomPositionConfig"/>
		...
		/// <summary>3D音源ハンドルの作成 </summary>
		/// <param name="config">3D音源ハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns><see cref="CriAtomEx3dSource"/> 3D音源オブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  必要なメモリのサイズは、 <see cref="CriAtomEx3dSource.CalculateWorkSize"/> 関数で計算します。
		...
		public unsafe CriAtomEx3dSource(in CriAtomEx3dSource.Config config)
		{
			fixed (CriAtomEx3dSource.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomEx3dSource_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomEx3dSource()
		{
			CriAtomEx3dSource.Config* configPtr = null;
			NativeHandle = NativeMethods.criAtomEx3dSource_Create(configPtr, default, default);
		}

		/// <summary>3D音源ハンドルの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトを破棄します。
...
#pragma warning restore 1591
		...
		///  デフォルト値は以下のとおりです。
		/// <list type="bullet">
		/// <item><description>前方ベクトル：(0.0f, 0.0f, 1.0f)</description></item>
		/// <item><description>上方ベクトル：(0.0f, 1.0f, 0.0f)</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <list type="bullet">
		/// <item><description>インサイドアングル：360.0f</description></item>
		/// <item><description>アウトサイドアングル：360.0f</description></item>
		/// <item><description>アウトサイドボリューム：0.0f</description></item>
		/// </list>
		/// </para>
		/// <list>
		/// <list type="bullet">
		/// <item><description>インサイドアングル：加算</description></item>
		/// <item><description>アウトサイドアングル：加算</description></item>
		/// <item><description>アウトサイドボリューム：乗算</description></item>
		/// </list>
		/// </list>
		/// <para>
		...
		///  本関数によってデフォルト値を変更すると、以降に作成する3D音源オブジェクト（ <see cref="CriAtomEx3dSource"/> ）の サウンドコーンパラメーターの初期値が本関数で設定した値となります。
		...
		///  ライブラリ初期化時のデフォルト値は以下のとおりです。
		/// <list type="bullet">
		/// <item><description>3D音源の半径：0.0f</description></item>
		/// <item><description>インテリア距離：0.0f（3D音源の最小距離に依存）</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomEx3dSource.ChangeDefaultInteriorPanField"/></see>
		/// <para>
		/// 注意:
		/// 設定したパラメーターを実際に適用するには、<see cref="CriAtomEx3dSource.Update"/> 関数を呼び出す必要があります。 
		/// </para>
		...
		///  本関数によってデフォルト値を変更すると、以降に作成する3D音源オブジェクト（ <see cref="CriAtomEx3dSource"/> ）の ドップラー係数の初期値が本関数で設定した値となります。
		...
		///  本関数によってデフォルト値を変更すると、以降に作成する3D音源オブジェクト（ <see cref="CriAtomEx3dSource"/> ）の ボリュームの初期値が本関数で設定した値となります。
		...
		/// 注意:
		/// 設定したパラメーターを実際に適用するには、<see cref="CriAtomEx3dSource.Update"/> 関数を呼び出す必要があります。
		///  本関数で指定した AISAC コントロール ID に対するコントロール値は、 以下関数のいずれかを使用してプレーヤーに設定したコントロール値より、優先して適用されます。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetAisacControlById"/> 関数
		/// </description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetAisacControlByName"/> 関数
		///  再生中の 3D 音源に対して AISAC コントロール ID の変更を行った場合、 変更前のコントロール ID に対する最終コントロール値が適用され続けます。
		...
		/// <summary>3D音源ハンドルに対する3Dリージョンハンドルの設定 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトに対して位置のランダム化の設定をします。 本関数を実行すると、再生時に音声の位置が元の位置情報および設定値に従って ランダムに変化します。 
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomEx3dSource.SetDefaultConfigForRandomPosition"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		...
		public unsafe partial struct RandomPositionConfig
		{
			...
			///  calculation_type に設定した座標の算出方法に対するパラメーター配列に 対する各要素の設定は以下の通りです。<list type="bullet">
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Rectangle"/><list type="bullet">
			/// <item><description>0: 左右幅(x軸), 1: 前後幅(z軸), 2: 0.0f</description></item>
			/// </list></description></item>
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Cuboid"/><list type="bullet">
			/// <item><description>0: 左右幅(x軸), 1: 前後幅(z軸), 2: 上下幅(y軸)</description></item>
			/// </list></description></item>
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Circle"/><list type="bullet">
			/// <item><description>0: 半径, 1: 0.0f, 2: 0.0f</description></item>
			/// </list></description></item>
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Cylinder"/><list type="bullet">
			/// <item><description>0: 半径, 1: 上下幅(y軸), 2: 0.0f</description></item>
			/// </list></description></item>
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Sphere"/><list type="bullet">
			/// <item><description>0: 半径, 1: 0.0f, 2: 0.0f</description></item>
			/// </list></description></item>
			/// </list>
			...
			/// <para>
			/// 備考:
			/// 以下の座標の算出方法では、本パラメーターは無視されます。<list type="bullet">
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.Callback"/></description></item>
			/// <item><description><see cref="CriAtomEx3dSource.RandomPositionCalculationType.List"/></description></item>
			/// </list>
			/// </para>
			/// </remarks>
			/// <seealso cref="CriAtomEx3dSource.RandomPositionCalculationType"/>
		...
		public enum RandomPositionCalculationType
		{
			...
			/// 注意:
			/// 本定義は情報取得用にのみ用いられます。
			///  そのため、本設定値を <see cref="CriAtomEx3dSource.RandomPositionConfig"/> 構造体に 指定して <see cref="CriAtomEx3dSource.SetRandomPositionConfig"/> 関数を実行した場合、 エラーが発生します。 
			/// </para>
			/// </remarks>
			None = -1,
			/// <summary>矩形 </summary>
		...
		///  登録したコールバック関数は、3D音源の位置のランダム化が有効な場合、 座標算出を行うタイミングで実行されます。 
		/// </para>
		/// <para>
		/// 備考:
		/// 3D音源の位置のランダム化に関するコンフィグ構造体である <see cref="CriAtomEx3dSource.RandomPositionConfig"/> の変数 calculation_type に対して <see cref="CriAtomEx3dSource.RandomPositionCalculationType.Callback"/> を設定している場合のみ、座標算出時に指定したコールバック関数を実行します。 
		/// </para>
		/// <para>
		/// 注意:
		/// コールバック登録の適用には、<see cref="CriAtomEx3dSource.Update"/> 関数を呼び出す必要があります。
		///  サーバー処理への割り込みを考慮しないAPIを実行した場合、 エラーが発生したり、デッドロックが発生する可能性があります。
		...
		/// <see cref="CriAtomEx3dSource.SetRandomPositionCalculationCallback"/> 関数を実行することで、 コールバック関数の登録が可能です。
		///  本コールバック関数は、3D音源の位置のランダム化における位置座標を算出する際に 呼び出されます。
		///  アプリケーション側で位置座標を決定したい場合にご利用ください。 
		/// </para>
		/// <para>
		/// 注意:
		/// 本コールバック関数内で長時間処理をブロックすると、音切れ等の問題が発生しますので、 ご注意ください。
		///  本コールバック関数は <see cref="CriAtomEx3dSource.RandomPositionConfig"/> 構造体に <see cref="CriAtomEx3dSource.RandomPositionCalculationType.Callback"/> を指定したときのみ呼び出されます。 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomEx3dSource.RandomPositionConfig"/>
		/// <seealso cref="CriAtomEx3dSource.SetRandomPositionCalculationCallback"/>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <para>
		/// 備考:
		/// 位置座標の算出方法として <see cref="CriAtomEx3dSource.RandomPositionCalculationType.List"/> を設定している場合のみ、本パラメーターを参照します。
		///  その他の算出方法を設定している場合、本パラメーターは無視されます。
		/// </para>
		/// <para>
		/// 注意:
		/// 設定したパラメーターを実際に適用するには、<see cref="CriAtomEx3dSource.Update"/> 関数を呼び出す必要があります。
		///  本関数に設定した領域は、 <see cref="CriAtomEx3dSource.SetRandomPositionList"/> 関数 の実行完了後は、内部からは参照されません。
		...
		///  登録したコールバック関数は、3D音源の位置のランダム化が有効な場合、 座標算出が行われた後のタイミングで実行されます。 
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <see cref="CriAtomEx3dSource.SetRandomPositionResultCallback"/> 関数を実行することで、 コールバック関数の登録が可能です。
		///  本コールバック関数は、3D音源の位置のランダム化が有効な場合、位置座標の算出が行われた後に 呼び出されます。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		public unsafe partial struct RandomPositionResultInfoDetail
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>元の3D音源の位置に対するオフセット座標 </para>
			/// </remarks>
