...
namespace CriWare
{
	...
	{
		/// <summary>3D音源ハンドルリスト作成用コンフィグ構造体にデフォルト値をセット </summary>
		/// <param name="pConfig">3D音源ハンドルリスト作成用コンフィグ構造体へのポインタ </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリスト作成用コンフィグ構造体（ <see cref="CriAtomEx3dSourceList.Config"/> ）に、デフォルトの値をセットします。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dSourceList_SetDefaultConfig_(CriAtomEx3dSourceListConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>3D音源ハンドルリストの作成に必要なワーク領域サイズの計算 </summary>
		/// <param name="config">3D音源ハンドルリスト作成用コンフィグ構造体へのポインタ </param>
		/// <returns>CriSint32 3D音源オブジェクトリスト作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストを作成するために必要なワーク領域のサイズを取得します。
		///  アロケーターを登録せずに3D音源オブジェクトリストを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomEx3dSourceList.CriAtomEx3dSourceList"/> 関数にセットする必要があります。
		///  3D音源オブジェクトリストの作成に必要なワークメモリのサイズは、3D音源オブジェクトリスト作成用コンフィグ 構造体（ <see cref="CriAtomEx3dSourceList.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx3dSourceList.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <summary>3D音源ハンドルリスト作成用コンフィグ構造体 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストを作成する場合に使用する構造体です。
		///  現状指定可能なパラメーターはありませんが、将来パラメーターが追加される可能性があるため、 本構造体を使用する際には <see cref="CriAtomEx3dSourceList.SetDefaultConfig"/> メソッドを使用し、 構造体の初期化を行ってください。 
		/// </para>
		/// </remarks>
		...
		/// <summary>3D音源ハンドルリストの作成 </summary>
		/// <param name="config">3D音源ハンドルリスト作成用コンフィグ構造体へのポインタ </param>
		/// <returns><see cref="CriAtomEx3dSourceList"/> 3D音源オブジェクトリスト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  必要なメモリのサイズは、 <see cref="CriAtomEx3dSourceList.CalculateWorkSize"/> 関数で計算します。
		...
		public unsafe CriAtomEx3dSourceList(in CriAtomEx3dSourceList.Config config)
		{
			fixed (CriAtomEx3dSourceList.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomEx3dSourceList_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomEx3dSourceList()
		{
			CriAtomEx3dSourceList.Config* configPtr = null;
			NativeHandle = NativeMethods.criAtomEx3dSourceList_Create(configPtr, default, default);
		}

		/// <summary>3D音源ハンドルリストの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストを破棄します。
		...
		///  3D音源オブジェクトリストをセットしたAtomExプレーヤーで再生している音声がある場合、 本関数を実行する前に、それらの音声を停止するか、そのAtomExプレーヤーを破棄してください。 
		/// </para>
		/// <para>
		/// 備考:
...
#pragma warning restore 1591
		...
		/// <summary>3D音源ハンドルリストへの3D音源ハンドルの追加 </summary>
		/// <param name="ex3dSource">3D音源ハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストに3D音源オブジェクトを追加します。
		///  追加したAtomExプレーヤーは3D音源オブジェクトリストと関連付けられ、 マルチポジショニング再生が可能となります。
		///  追加した3D音源オブジェクトを3D音源オブジェクトリストから削除する場合は、 <see cref="CriAtomEx3dSourceList.Remove"/> 関数または <see cref="CriAtomEx3dSourceList.RemoveAll"/> 関数を呼び出してください。
		/// </para>
		/// <para>
		/// 備考:
		/// 以下の条件に当てはまる3D音源オブジェクトは3D音源オブジェクトリストに追加することはできません。
		/// <list type="bullet">
		/// <item><description>既にAtomExプレーヤーに設定されている</description></item>
		/// <item><description>既に他の3D音源オブジェクトリストに追加されている 
		...
		/// <summary>3D音源ハンドルリストから3D音源ハンドルの削除 </summary>
		/// <param name="ex3dSource">3D音源ハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストから、指定した3D音源オブジェクトを削除します。
		///  3D音源オブジェクトリストから全ての3D音源オブジェクトを削除したい場合は、 <see cref="CriAtomEx3dSourceList.RemoveAll"/> 関数を 呼び出してください。
		...
		/// <summary>3D音源ハンドルリストから3D音源ハンドルの全削除 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3D音源オブジェクトリストから追加されている全ての3D音源オブジェクトを削除します。
		///  3D音源オブジェクトリストから特定の3D音源オブジェクトを削除したい場合は、 <see cref="CriAtomEx3dSourceList.Remove"/> 関数を 呼び出してください。
		///  3D音源オブジェクトリストに3D音源オブジェクトを追加したい場合は、 <see cref="CriAtomEx3dSourceList.Add"/> 関数を呼び出してください。 
		/// </para>
		/// <para>
		/// 備考:
