...
namespace CriWare
{
	...
	{
		/// <summary>3Dトランシーバーハンドル作成用コンフィグ構造体にデフォルト値をセット </summary>
		/// <param name="pConfig">3Dトランシーバーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dトランシーバーオブジェクト作成用コンフィグ構造体（ <see cref="CriAtomEx3dTransceiver.Config"/> ）に、デフォルトの値をセットします。 
		/// </para>
		/// <nativeinfo declaration="void criAtomEx3dTransceiver_SetDefaultConfig_(CriAtomEx3dTransceiverConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>3Dトランシーバーハンドルの作成に必要なワーク領域サイズの計算 </summary>
		/// <param name="config">3Dトランシーバーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>CriSint32 3Dトランシーバーオブジェクト作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dトランシーバーオブジェクトを作成するために必要な、ワーク領域のサイズを取得します。
		///  アロケーターを登録せずに3Dトランシーバーオブジェクトを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomEx3dTransceiver.CriAtomEx3dTransceiver"/> 関数にセットする必要があります。
		///  3Dトランシーバーオブジェクトの作成に必要なワークメモリのサイズは、3Dトランシーバーオブジェクト作成用コンフィグ 構造体（ <see cref="CriAtomEx3dTransceiver.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomEx3dTransceiver.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <summary>3Dトランシーバーハンドル作成用コンフィグ構造体 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dトランシーバーオブジェクトを作成する場合に使用する構造体です。
		///  現状指定可能なパラメーターはありませんが、将来パラメーターが追加される可能性があるため、 本構造体を使用する際には <see cref="CriAtomEx3dTransceiver.SetDefaultConfig"/> メソッドを使用し、 構造体の初期化を行ってください。 
		/// </para>
		/// </remarks>
		...
		/// <summary>3Dトランシーバーハンドルの作成 </summary>
		/// <param name="config">3Dトランシーバーハンドル作成用コンフィグ構造体へのポインタ </param>
		/// <returns>criAtomEx3dTransceiverHn 3Dトランシーバーオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  必要なメモリのサイズは、 <see cref="CriAtomEx3dTransceiver.CalculateWorkSize"/> 関数で計算します。
		...
		public unsafe CriAtomEx3dTransceiver(in CriAtomEx3dTransceiver.Config config)
		{
			fixed (CriAtomEx3dTransceiver.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomEx3dTransceiver_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomEx3dTransceiver()
		{
			CriAtomEx3dTransceiver.Config* configPtr = null;
			NativeHandle = NativeMethods.criAtomEx3dTransceiver_Create(configPtr, default, default);
		}

		/// <summary>3Dトランシーバーハンドルの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
...
#pragma warning restore 1591
		...
		///  ライブラリ初期化時のデフォルト値は以下のとおりです。
		/// <list type="bullet">
		/// <item><description>3Dトランシーバーの半径：0.0f</description></item>
		/// <item><description>インテリア距離：0.0f（3Dトランシーバーの最小距離に依存）</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description>直接音領域の半径：0.0f</description></item>
		/// <item><description>クロスフェード距離：0.0f</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <summary>3Dトランシーバーハンドルに対する3Dリージョンハンドルの設定 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
