...
namespace CriWare
{
	...
	public partial class CriAtomExAcb : IDisposable
	{
		...
		/// <param name="awbBinder">AWBファイルを含むバインダーのハンドル </param>
		/// <param name="awbPath">AWBファイルのパス </param>
		/// <returns>CriSint32 ワーク領域サイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAcb.LoadAcbData"/> 関数の実行に必要なワーク領域サイズを計算します。 <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomExAcb.LoadAcbData"/> 関数でAWBデータをロードする際には、 本関数が返すサイズ分のメモリをワーク領域として渡す必要があります。
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAcb.LoadAcbDataById"/> 関数の実行に必要なワーク領域サイズを計算します。 ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomExAcb.CalculateWorkSizeForLoadAcbData"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		/// <param name="awbBinder">AWBファイルを含むバインダーのハンドル </param>
		/// <param name="awbPath">AWBファイルのパス </param>
		/// <returns><see cref="CriAtomExAcb"/> ACBオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ACBデータをロードし、キュー再生に必要な情報を取り込みます。
		///  ACBデータのロードに必要なワーク領域のサイズは、 <see cref="CriAtomExAcb.CalculateWorkSizeForLoadAcbData"/> 関数で計算します。
		...
		///  ACBデータをロードすると、ACBデータにアクセスするためのACBオブジェクト （ <see cref="CriAtomExAcb"/> ）が返されます。
		///  AtomExプレーヤーに対し、 <see cref="CriAtomExPlayer.SetCueId"/> 関数でACBオブジェクト、および再生する キューのIDをセットすることで、ACBデータ内のキューを再生することが可能です。
		...
		///  本関数は即時復帰関数です。
		///  ACBファイルを事前にメモリにロードしてから本関数を実行することで、 ACBオブジェクト作成時に処理がブロックされるのを回避可能です。 
		///  データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数にてセットしたデータ領域やワーク領域のメモリ内容はACBオブジェクト破棄時 までアプリケーション中で保持し続ける必要があります。
		...
		/// 説明:
		/// ACBデータをロードし、キュー再生に必要な情報を取り込みます。
		///  ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomExAcb.LoadAcbData"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		/// <param name="acbBinder">ACBファイルを含むバインダーのハンドル </param>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAcb.LoadAcbFile"/> 関数の実行に必要なワーク領域サイズを計算します。 <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomExAcb.LoadAcbFile"/> 関数でACBファイルをロードする際には、 本関数が返すサイズ分のメモリをワーク領域として渡す必要があります。
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。
		/// </para>
		/// <para>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAcb.LoadAcbFileById"/> 関数の実行に必要なワーク領域サイズを計算します。 ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomExAcb.CalculateWorkSizeForLoadAcbFile"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		///  ACBファイルのロードに必要なワーク領域のサイズは、 <see cref="CriAtomExAcb.CalculateWorkSizeForLoadAcbFile"/> 関数で計算します。
		///  第3引数の awb_binder 、および第4引数の awb_path には、ストリーム再生用 のAWBファイルを指定します。
		///  （オンメモリ再生のみのACBデータをロードする場合、 awb_binder および awb_path にセットした値は無視されます。）
		///  ACBファイルをロードすると、ACBデータにアクセスするためのACBオブジェクト （ <see cref="CriAtomExAcb"/> ）が返されます。
		///  AtomExプレーヤーに対し、 <see cref="CriAtomExPlayer.SetCueId"/> 関数でACBオブジェクト、および再生する キューのIDをセットすることで、ACBファイル内のキューを再生することが可能です。
		///  ACBファイルのロードに成功すると、本関数は戻り値として ACB オブジェクトを返します。
		///  リードエラー等によりACBファイルのロードに失敗した場合、本関数は戻り値として CRI_NULL を返します。
		/// </para>
		/// <para>
		...
		/// 注意:
		/// 本関数を実行する前に、ライブラリを初期化しておく必要があります。
		///  本関数にワーク領域をセットした場合、セットした領域のメモリをACBオブジェクト破棄時 までアプリケーション中で保持し続ける必要があります。
		///  （セット済みのワーク領域に値を書き込んだり、メモリ解放したりしてはいけません。）
		///  本関数は完了復帰型の関数です。
		///  ACBファイルのロードにかかる時間は、プラットフォームによって異なります。
		///  ゲームループ等の画面更新が必要なタイミングで本関数を実行するとミリ秒単位で 処理がブロックされ、フレーム落ちが発生する恐れがあります。
		...
		/// 説明:
		/// ACBファイルをロードし、キュー再生に必要な情報を取り込みます。
		///  ファイルパスの代わりにCPKコンテンツIDを指定する点を除けば、 <see cref="CriAtomExAcb.LoadAcbFile"/> 関数と機能は同じです。
		/// </para>
		/// <para>
		/// 備考：
		/// データがCPKにパックされていない場合、引数binderにはnullを指定してください。
		...
		/// 説明:
		/// ACBオブジェクトを解放します。
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドを使用してアロケーターを登録済みの場合、 ACBオブジェクト作成時に確保されたメモリ領域が解放されます。
		///  （ACBオブジェクト作成時にワーク領域を渡した場合、本関数実行後であれば ワーク領域を解放可能です。）
		/// </para>
		/// <para>
		/// 備考:
		/// 本関数でACBオブジェクトを破棄する際には、 当該ACBオブジェクトを参照しているキューは全て停止されます。
		///  （本関数実行後に、ACBオブジェクトの作成に使用したワーク領域や、 ACBデータが配置されていた領域が参照されることはありません。）
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行すると、破棄しようとしているACBデータを参照している Atomプレーヤーの存在を検索する処理が動作します。
...
#pragma warning restore 1591
		...
		/// <returns>CriBool ACBの状態（true = 即時解放可能、false = 再生中のプレーヤーあり） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ACBオブジェクトを即座に解放可能かどうかをチェックします。
		///  本関数が false を返すタイミングで <see cref="CriAtomExAcb.Dispose"/> 関数を実行すると、 ACBオブジェクトを参照しているプレーヤーに対する停止処理が行われます。
		///  （ストリーム再生用のACBオブジェクトの場合、ファイル読み込み完了を待つため、 <see cref="CriAtomExAcb.Dispose"/> 関数内で長時間処理がブロックされる可能性があります。）
		/// </para>
		/// <para>
		/// 備考:
		/// ACBオブジェクトを再生していたプレーヤーを全て停止させた場合でも、 ライブラリ内では当該ACBオブジェクトを参照しているボイスが存在する可能性があります。
		///  （ <see cref="CriAtomExPlayer.StopWithoutReleaseTime"/> 関数で停止処理を行った場合や、 ボイスの奪い取りが発生した場合、プレーヤーからボイスは切り離されますが、 その後もボイス側でファイルの読み込み完了待ちを行うケースがあります。）
		/// <see cref="CriAtomExAcb.Dispose"/> 関数内で処理がブロックされるのを避ける必要がある場合には、 本関数が true を返すまで、<see cref="CriAtomExAcb.Dispose"/> 関数を実行しないでください。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行すると、指定したACBデータを参照しているAtomプレーヤーの存在を 検索する処理が動作します。
		///  そのため、本関数実行中に他スレッドでAtomプレーヤーの作成／破棄を行うと、 アクセス違反やデッドロック等の重大な不具合を誘発する恐れがあります。
		///  本関数実行時にAtomプレーヤーの作成／破棄を他スレッドで行う必要がある場合、 Atomプレーヤーの作成／破棄を <see cref="CriAtomEx.Lock"/> 関数でロックしてから実行ください。
		/// </para>
		...
		/// <summary>全てのACBハンドルをリリース </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <summary>ACBハンドルの列挙 </summary>
		/// <param name="func">ACBハンドルコールバック関数 </param>
		...
		/// 説明:
		/// ACBオブジェクトを列挙します。
		///  本関数を実行すると、第 1 引数（ func ） でセットされたコールバック関数がACBオブジェクトの数分だけ呼び出されます。
		///  コールバック関数には、ACBオブジェクトが引数として渡されます。
		/// </para>
		/// <para>
		/// 備考:
		/// 第 2 引数（ obj ）にセットした値は、コールバック関数の引数として渡されます。
		///  コールバック関数のその他の引数については、 別途 <see cref="CriAtomExAcb.HandleCbFunc"/> の説明をご参照ください。
		...
		/// <summary>ACBハンドルコールバック関数型 </summary>
		/// <returns>CriBool 列挙を続けるかどうか（true：継続、false：中止） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ACBオブジェクトの通知に使用される、コールバック関数の型です。
		/// <see cref="CriAtomExAcb.EnumerateHandles"/> 関数に本関数型のコールバック関数を登録することで、 ACBオブジェクトをコールバック経由で受け取ることが可能です。
		/// </para>
		/// <para>
		/// 注意:
		/// ACBオブジェクトをコールバック関数内で破棄してはいけません。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <returns>CriUint2 ACBフォーマットバージョン </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  ACB情報の登録に必要なワーク領域のサイズは、 <see cref="CriAtomExAcb.CalculateWorkSizeForLoadAcbFile"/> 関数で計算します。
		...
		public static unsafe UInt32 GetVersionFromFile(CriFsBinder acbBinder, ArgString acbPath, out NativeBool flag)
		{
			fixed (NativeBool* flagPtr = &flag)
				return NativeMethods.criAtomExAcb_GetVersionFromFile(acbBinder?.NativeHandle ?? default, acbPath.GetPointer(stackalloc byte[acbPath.BufferSize]), default, default, flagPtr);
		}
		...
		/// <returns>CriSint32 キュー数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriBool キューが存在するかどうか（存在する：true／存在しない：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="index">キューインデックス </param>
		/// <returns><see cref="Int32"/> キューID </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキューインデックスに 合致するデータの存在を検索する処理が動作します。
		...
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキュー名に 合致するデータの存在を検索する処理が動作します。
		...
		/// <param name="index">キューインデックス </param>
		/// <returns>const CriChar8* キュー名 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキューIDに 合致するデータの存在を検索する処理が動作します。
		...
		/// <param name="id">キューID </param>
		/// <returns><see cref="Int32"/> キューインデックス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="id">キューID </param>
		/// <returns>const CriChar8 * ユーザデータ文字列 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="id">キューID </param>
		/// <returns>CriSint64 キューの長さ（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="id">キューID </param>
		/// <returns>CriSint32 AISAC Controlの個数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriBool 取得に成功したかどうか（成功：true、失敗：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriBool コントロール可能かどうか（可能：true、不可能：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="id">キューID </param>
		/// <returns>CriSint32 プライオリティ（取得に失敗した場合-1が帰ります） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  そのキューで再生される音声波形が複数ある場合、初めのトラックで最初に再生される音声波形の情報が取得されます。 指定したキューIDのキューが存在しない場合、falseが返ります。
		/// </para>
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキューIDに 合致するデータの存在を検索する処理が動作します。
		...
		///  そのキューで再生される音声波形が複数ある場合、初めのトラックで最初に再生される音声波形の情報が取得されます。 指定したキュー名のキューが存在しない場合、falseが返ります。
		/// </para>
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキュー名に 合致するデータの存在を検索する処理が動作します。
		...
		/// <summary>オンメモリ再生用 AWB ハンドルの取得 </summary>
		/// <returns><see cref="CriAtomAwb"/> AWB オブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  ACB オブジェクトを作成する際、 Atom ライブラリはオンメモリ再生用に AWB データを読み込み、再生用のオブジェクト（ AWB オブジェクト）を作成します。
		///  本関数を使用することで、 Atom ライブラリが内部的に作成した AWB オブジェクトを 取得することが可能です。
		///  取得した AWB オブジェクトを使用することで、 ACB オブジェクト内のオンメモリ波形データを、 アプリケーション側から <see cref="CriAtomExPlayer.SetWaveId"/> 関数を使用して再生することが可能になります。
		...
		///  先頭以外のスロットにある AWB オブジェクトを取得する際は以下の関数を使用してください。<list type="bullet">
		/// <item><description><see cref="CriAtomExAcb.GetStreamingAwbHandleBySlotName"/></description></item>
		/// <item><description><see cref="CriAtomExAcb.GetStreamingAwbHandleBySlotIndex"/></description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		/// ACB データ内には、ストリーム再生用の AWB ファイルが関連付けられています。
		///  ACB オブジェクトを作成する際、 Atom ライブラリはストリーム再生用に AWB データを読み込み、再生用のオブジェクト（ AWB オブジェクト）を作成します。
		///  本関数を使用することで、 Atom ライブラリが内部的に作成した AWB オブジェクトを 取得することが可能です。
		///  取得した AWB オブジェクトを使用することで、 ストリーム再生用の波形データを、 アプリケーション側から <see cref="CriAtomExPlayer.SetWaveId"/> 関数を使用して再生することが可能になります。
		...
		public enum CueType
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>ランダム非連続（前回再生した音以外をランダムに鳴らす） </para>
			/// </remarks>
			RandomNoRepeat = 4,
			/// <summary></summary>
			/// <remarks>
			/// <para>スイッチ再生（ゲーム変数を参照して再生トラックの切り替える） </para>
			/// </remarks>
			SwitchGameVariable = 5,
			/// <summary></summary>
			/// <remarks>
			/// <para>コンボシーケンシャル（「コンボ時間」内に連続コンボが決まるとシーケンシャル、最後までいくと「コンボループバック」地点に戻る） </para>
			/// </remarks>
			ComboSequential = 6,
			/// <summary></summary>
			/// <remarks>
			/// <para>スイッチ再生（セレクターを参照して再生トラックを切り替える） </para>
			/// </remarks>
			SwitchSelector = 7,
			/// <summary></summary>
			/// <remarks>
			/// <para>トラックトランジション再生（セレクターを参照して再生トラックを切り替える） </para>
			/// </remarks>
		...
		/// <param name="name">キュー名 </param>
		/// <returns>CriSint32 発音数（キューリミットが設定されていないキューを指定した場合-1が帰ります） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns><see cref="Int32"/> ブロックインデックス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// キューインデックスとブロック名からブロックインデックスを取得します。
		///  指定したキューインデックスのキューが存在しない場合やブロック名が存在しない場合は、 <see cref="CriAtomEx.InvalidBlockIndex"/> が返ります。 
		/// </para>
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキューインデックスに 合致するデータの存在を検索する処理が動作します。
		...
		///  指定したキューIDのキューが存在しない場合やブロック名が存在しない場合は、 <see cref="CriAtomEx.InvalidBlockIndex"/> が返ります。 
		/// </para>
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキューIDに 合致するデータの存在を検索する処理が動作します。
		...
		///  指定したキュー名のキューが存在しない場合やブロック名が存在しない場合は、 <see cref="CriAtomEx.InvalidBlockIndex"/> が返ります。 
		/// </para>
		/// <para>
		/// 備考:
		/// 第1引数（ ach_hn ）に null を指定した場合、全てのACBデータを対象に、指定したキュー名に 合致するデータの存在を検索する処理が動作します。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <para>
		/// 説明:
		/// ACBオブジェクトに対してストリーム用のAWBファイルをアタッチします。 第2引数の awb_binder 、および第3引数の awb_path には、ストリーム再生用 のAWBファイルを指定します。
		///  第5引数の awb_name はAWBをアタッチするスロットを指定するために使用します。 このため、AtomCraftが出力したAWB名（ファイル名から拡張子を取り除いた部分）を変更している場合 はオリジナルのAWB名を指定してください。
		///  AWBファイルのアタッチを行うには、ライブラリが内部で利用するためのメモリ領域 （ワーク領域）を確保する必要があります。
		...
		///  アタッチ時に確保されたメモリは、デタッチ時（ <see cref="CriAtomExAcb.DetachAwbFile"/> 関数実行時）か、ACBオブジェクトリリース時（ <see cref="CriAtomExAcb.Dispose"/> 関数実行時）に解放されます。
		...
		public void AttachAwbFile(CriFsBinder awbBinder, ArgString awbPath, ArgString awbName)
		{
			NativeMethods.criAtomExAcb_AttachAwbFile(NativeHandle, awbBinder?.NativeHandle ?? default, awbPath.GetPointer(stackalloc byte[awbPath.BufferSize]), awbName.GetPointer(stackalloc byte[awbName.BufferSize]), default, default);
		}
		...
		/// <returns>CriSint32 ストリームAWBスロット数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>const CriChar8* ストリームAWBポート名 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
