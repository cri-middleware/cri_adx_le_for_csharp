...
#pragma warning disable 0465
	...
	{
		/// <summary>バイノーラライザーのボリューム設定 </summary>
		/// <param name="volume">ボリューム値 </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// バイノーラライザーのボリュームを指定します。
		///  本関数を使用することで、バイノーラル処理を経由する音声の音量のみを制御することが可能です。
		///  ボリューム値は音声データの振幅に対する倍率です（単位はデシベルではありません）。
		///  例えば、1.0fを指定した場合、出力音声はデフォルトの音量で出力されます。
		///  0.5fを指定した場合、音声波形の振幅を半分にしたデータと同じ音量（-6dB）で音声が出力されます。
		...
		{
			NativeMethods.criAtomExAsr_SetBinauralizerVolume(volume);
		}
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAsr.AttachBusAnalyzerByName"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExAsr.BusAnalyzerConfig"/> ）に、デフォルト値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExAsr_SetDefaultConfigForBusAnalyzer_(CriAtomExAsrBusAnalyzerConfig *p_config)"/>
		/// </remarks>
		...
		/// </para>
		/// <para>（多重音声のミキシングや前段のエフェクトによっては上記範囲を超えた値が出る可能性があります。）</para>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  異なるバイノーラライザー設定の ACF を登録した場合は以下の挙動になります。<list type="bullet">
		/// <item><description>バイノーラライザー設定が存在しない古い ACF : バイノーラル結果が無音になる。</description></item>
		/// <item><description>バイノーラライザー設定が CRI Binauralizer の ACF : バイノーラル結果が無音になる。 </description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExAsr.Initialize"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExAsr.Config"/> ）に、デフォルト値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExAsr_SetDefaultConfig_(CriAtomExAsrConfig *p_config)"/>
		/// </remarks>
		...
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomExAsr.Initialize"/> 関数でASRの初期化を行う場合、 本関数で計算したサイズ分のメモリをワーク領域として渡す必要があります。
		...
		/// <para>
		/// 説明:
		/// ワーク領域サイズの計算用に、ASR初期化用コンフィグ構造体 （ <see cref="CriAtomExAsr.Config"/> 構造体）を仮登録します。
		///  エフェクトのアタッチに必要なワーク領域のサイズは、 ASR初期化時（ <see cref="CriAtomExAsr.Initialize"/> 関数実行時） に設定する構造体のパラメーターによって変化します。
		...
		///  （ <see cref="CriAtomEx.CalculateWorkSizeForDspBusSettingFromAcfData"/> 関数が実行可能となります。）
		/// </para>
		/// <para>
		/// 備考:
		/// 引数（ config ）に null を指定した場合、デフォルト設定 （ <see cref="CriAtomExAsr.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。
		...
		///  （本関数で登録する構造体のパラメーターと、 ASRの初期化に使用する構造体のパラメーターが異なる場合、 ワーク領域サイズが不足し、オブジェクトの作成に失敗する恐れがあります。）
		...
		public static unsafe void Initialize(in CriAtomExAsr.Config config)
		{
			fixed (CriAtomExAsr.Config* configPtr = &config)
				NativeMethods.criAtomExAsr_Initialize(configPtr, default, default);
		}
		...
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドを使用してアロケーターを登録済みの場合、 ASR初期化時に確保されたメモリ領域が解放されます。
		...
		///  動作時パラメーターを設定する際は、本関数呼び出し前にあらかじめ <see cref="CriAtomEx.AttachDspBusSetting"/> 関数でバスが構築されている必要があります。
		...
		/// <param name="effectName">エフェクト名 </param>
		/// <param name="parameterIndex">エフェクト動作時パラメーターインデックス </param>
		/// <returns>指定したパラメーターインデックスのエフェクト動作時パラメーター値を返します。 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// デフォルト設定を使用する場合、 <see cref="CriAtomExAsr.SetDefaultConfigForBusAnalyzer"/> メソッドで 構造体にデフォルトパラメーターをセットした後、 <see cref="CriAtomExAsr.AttachBusAnalyzerByName"/> 関数 に構造体を指定してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExAsr.SetDefaultConfigForBusAnalyzer"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomExAsr.AttachBusAnalyzerByName"/>
		...
		///  以下の条件に該当する場合は、ユーザ定義エフェクトインターフェースの登録に失敗し、エラーコールバックが返ります:<list type="bullet">
		/// <item><description>同一のエフェクト名を持つユーザ定義エフェクトインターフェースが既に登録されている</description></item>
		/// <item><description>Atomが使用しているユーザ定義エフェクトインターフェースと異なる</description></item>
		/// <item><description>ユーザ定義エフェクトインターフェースの登録数上限（ <see cref="CriAtomExAsr.MaxNumUserEffectInterfaces"/> ）に達した </description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		{
			return NativeMethods.criAtomExAsr_GetNumBufferedSamples();
		}

		/// <summary>PCMバッファーサイズの指定 </summary>
		/// <param name="numSamples">PCMバッファーサイズ（サンプル数単位） </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  本関数にて設定した値は <see cref="CriAtomExAsr.GetPcmBufferSize"/> 関数にて取得することができます。 
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <para>
		/// 注意:
		/// PCMバッファーの確保は <see cref="CriAtomEx.InitializeForUserPcmOutput"/> 関数内で行われます。
		///  そのため、本関数は <see cref="CriAtomEx.InitializeForUserPcmOutput"/> 関数よりも先に実行する必要があります。
		...
		{
			NativeMethods.criAtomExAsr_SetPcmBufferSize(numSamples);
		}
		...
		/// <returns>バイノーラライザーの有効か？（true:有効, false:無効） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
