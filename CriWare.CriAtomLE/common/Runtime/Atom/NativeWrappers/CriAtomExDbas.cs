...
namespace CriWare
{
	...
	public partial class CriAtomExDbas : IDisposable
	{
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExDbas.CriAtomExDbas"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExDbas.Config"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExDbas_SetDefaultConfig_(CriAtomExDbasConfig *p_config)"/>
		/// </remarks>
		...
		public unsafe partial struct Config
		{
			...
			///  例えば、シーンAではオーディオデータを２本、 シーンBではオーディオデータを１本とムービーデータを２本、ストリーミング再生するとします。
			...
			///  ADX内部ではバッファーをブロック単位で管理しているため、 ファイル終端やループ先頭等の半端なデータサイズに対しても、 バッファーブロックを１つ分割り当てます。
			///  ワンショットのストリーミング再生では、 ダブルバッファリングでデータを読むだけの単純な処理であっても、 ストリームが途切れる事はありません。
			///  一方、ループ付きデータの場合では、ループ終端のわずかなデータに１ブロック、 ループ先頭のわずかなデータにも１ブロックを使ってしまうと、
			///  バッファリング済みのデータが極端に少ないにもかかわらず、 次のデータを読み込むバッファーが空かないためストリームが途切れてしまいます。
			///  本パラメーターは、D-BASが確保するストリーミングバッファーサイズに影響を与えます。
			///  音が途切れない事を十分に確認できていれば、 本パラメーターの下限値は <see cref="CriAtomDbas.MinimumNumSecurementBuffers"/> です。
			/// </para>
			/// </remarks>
		...
		/// <returns>CriSint32 D-BAS作成用ワークサイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// </para>
		/// <nativeinfo declaration="CriSint32 criAtomExDbas_CalculateWorkSize_(CriAtomExDbasConfig *config)"/>
		/// </remarks>
		/// <seealso cref="CriAtomExDbas.CriAtomExDbas"/>
		...
		/// <param name="config">D-BAS作成用コンフィグ構造体へのポインタ </param>
		/// <returns><see cref="CriAtomExDbas"/> D-BAS管理用ID </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// </para>
		/// <nativeinfo declaration="CriAtomExDbasId criAtomExDbas_Create_(CriAtomExDbasConfig *config, void *work, CriSint32 work_size)"/>
		/// </remarks>
		/// <seealso cref="CriAtomExDbas.CalculateWorkSize"/>
		/// <seealso cref="CriAtomExDbas.Dispose"/>
		public unsafe CriAtomExDbas(in CriAtomExDbas.Config config)
		{
			fixed (CriAtomExDbas.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomExDbas_Create_(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomExDbas()
		{
		...
		/// </para>
		/// <nativeinfo declaration="void criAtomExDbas_Destroy_(CriAtomExDbasId atom_dbas_id)"/>
		/// </remarks>
		/// <seealso cref="CriAtomExDbas.CriAtomExDbas"/>
		public void Dispose()
		{
...
#pragma warning restore 1591
		...
		/// <summary>ストリーム再生中のAtomプレーヤーハンドルを取得 </summary>
		/// <param name="players">プレーヤーハンドル受け取り用配列 </param>
		/// <param name="length">プレーヤーハンドル受け取り用配列要素数 </param>
		/// <returns>CriSint32 プレーヤー数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ストリーム再生中のAtomプレーヤーオブジェクトを取得します。
		///  プレーヤーオブジェクトの取得に成功すると、 第3引数（players配列）にプレーヤーオブジェクトのアドレスが保存され、 プレーヤーオブジェクト数が戻り値として返されます。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <para>
		/// 注意:
		/// プレーヤー数を取得してからオブジェクトを取得する場合、 プレーヤー数取得とオブジェクト取得の間にサーバー処理が割り込まないよう、 <see cref="CriAtom.Lock"/> 関数で排他制御を行う必要があります。
		///  （サーバー処理のタイミングで、プレーヤー数が変わる可能性があります。）
		///  配列要素数がストリーム再生中のプレーヤー数に満たない場合、 本関数はエラー値（-1）を返します。
		/// </para>
		/// <nativeinfo declaration="CriSint32 criAtomExDbas_GetStreamingPlayerHandles_(CriAtomExDbasId dbas_id, CriAtomPlayerHn *players, CriSint32 length)"/>
		/// </remarks>
