...
namespace CriWare
{
	...
	/// <list type="bullet">
	/// <item><description><see cref="CriAtomExOutputPort.CriAtomExOutputPort"/> 関数で新しいオブジェクトを作成</description></item>
	/// <item><description><see cref="CriAtomExAcf.GetOutputPortHnByName"/> 関数でACF内の出力ポートオブジェクトを取得 
	///  ACF内の出力ポートオブジェクトは、ACFファイルを登録したとき自動的に作成されます。 プレーヤーに対して <see cref="CriAtomExPlayer.AddOutputPort"/> 関数または <see cref="CriAtomExPlayer.AddPreferredOutputPort"/> 関数にて 別の出力ポートオブジェクトが指定されていない場合、出力ポートが指定された音声の再生にはACF内の出力ポートが使用されます。 </description></item>
	/// </list>
	/// </para>
	/// <para>
	/// 備考:
	/// 出力ポートは、音声の出力先を抽象化した概念です。
	///  音声トラックに予め出力先の名前を指定すると、再生するとき自動的に同じ名前の出力ポートオブジェクトが割り当てられます。 出力ポートはASRラックと紐付けられており、出力ポートオブジェクトが割り当てられた音声は、そのASRラックを介して再生されます。 
	///  そのため、ACF内の出力ポートオブジェクトを含め、全ての出力ポートオブジェクトは使用する前に必ず <see cref="CriAtomExOutputPort.SetAsrRackId"/> 関数で適切なASRラックと紐付ける必要があります。 
	/// </para>
	/// </remarks>
	...
	{
		/// <summary>出力ポートハンドルの破棄可能の判定 </summary>
		/// <returns>CriBool 破棄可能か？（true = 可能, false = 不可能） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// <see cref="CriAtomExPlayer.AddOutputPort"/> 関数または <see cref="CriAtomExPlayer.AddPreferredOutputPort"/> 関数を使用して プレーヤーに追加中の出力ポートオブジェクトは破棄することができません。
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExOutputPort.CriAtomExOutputPort"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExOutputPort.Config"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExOutputPort_SetDefaultConfig_(CriAtomExOutputPortConfig *p_config, CriChar8 *outputport_name)"/>
		/// </remarks>
		...
		/// <summary>出力ポートハンドル作成用ワーク領域サイズの計算 </summary>
		/// <param name="config">出力ポートハンドル作成用コンフィグ構造体 </param>
		/// <returns>CriSint32 ワーク領域サイズ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 出力ポートオブジェクトの作成に必要なワーク領域のサイズを計算します。
		/// <see cref="CriAtomEx.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomExOutputPort.CriAtomExOutputPort"/> 関数で出力ポートオブジェクトを作成する際には、 <see cref="CriAtomExOutputPort.CriAtomExOutputPort"/> 関数に本関数が返すサイズ分のメモリをワーク 領域として渡す必要があります。
		...
		/// <summary>出力ポートハンドルの作成 </summary>
		/// <param name="config">ASR初期化用コンフィグ構造体 </param>
		/// <returns><see cref="CriAtomExOutputPort"/> 出力ポートオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 出力ポートオブジェクトの作成を行います。
		...
		/// <para>
		/// 備考:
		/// ACFファイルに設定された出力ポートオブジェクトは <see cref="CriAtomEx.RegisterAcfFile"/> 関数などを用いて ACFファイルを登録したとき、ACF内に自動的に作成されるため、 本関数で新たに作成する必要はありません。
		...
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。
		///  出力ポートオブジェクトの生成に成功した場合は、本関数は生成した出力ポートオブジェクトを返します。 生成に失敗した場合は null を返します。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数で作成された出力ポートオブジェクトには、<see cref="CriAtomExOutputPort.Config"/>::type で指定したタイプによって 以下のASRラックIDが初期値としてセットされています。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExOutputPort.Type.Audio"/> を指定：<see cref="CriAtomExAsr.RackDefaultId"/></description></item>
		/// <item><description><see cref="CriAtomExOutputPort.Type.Vibration"/> を指定：<see cref="CriAtomExAsr.RackIllegalId"/> </description></item>
		/// </list>
		/// </para>
		...
		public unsafe CriAtomExOutputPort(in CriAtomExOutputPort.Config config)
		{
			fixed (CriAtomExOutputPort.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomExOutputPort_Create(configPtr, default, default);
		}

		/// <summary>出力ポートハンドルの破棄 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 出力ポートオブジェクトの破棄を行います。
...
#pragma warning restore 1591
		...
		/// <para>
		/// 備考:
		/// ACFファイル登録時に作成された出力ポートオブジェクトや <see cref="CriAtomExOutputPort.CriAtomExOutputPort"/> 関数で作成された 出力ポートオブジェクトには、必ず本関数で適切なASRラックを指定する必要があります。 
		...
		/// <para>
		/// 説明:
		/// 振動タイプの出力ポートは２チャンネルで動作するため、入力されるボイスがステレオ以上の音声データか、 3Dパンが設定されている場合、その結果が振動デバイスの左右に伝わります。 モノラルミックスを有効にすると、振動デバイスへ出力する前に一度モノラルにダウンミックスを行うことでそれらの影響をなくすことができます。 <see cref="CriAtomExOutputPort.SetVibrationChannelLevel"/> 関数を使用して、モノラルミックス後振動デバイスへ送られるレベルを設定することも可能です。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <param name="ignoreParameters">無視設定パラメータ （CRI_TRUE:無視する, CRI_FALSE:無視しない） </param>
		/// <remarks>
		/// <para>
		/// 説明:
