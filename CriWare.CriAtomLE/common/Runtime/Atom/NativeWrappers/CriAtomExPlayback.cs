...
namespace CriWare
{
	...
	///  プレーヤー単位ではなく、 <see cref="CriAtomExPlayer.Start"/> 関数で再生した個々の音声に対して パラメーター変更や状態取得を行いたい場合、本IDを使用して制御を行う必要があります。
	...
	public readonly partial struct CriAtomExPlayback
	{
		...
		/// <para>
		/// 備考:
		/// AtomEx プレーヤーによって再生された全ての音声を停止したい場合、 本関数ではなく <see cref="CriAtomExPlayer.Stop"/> 関数をご利用ください。
		...
		/// <para>
		/// 備考:
		/// プレーヤーによって再生された全ての音声を停止したい場合、 本関数ではなく <see cref="CriAtomExPlayer.StopWithoutReleaseTime"/> 関数をご利用ください。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数で再生音の停止を行うと、再生中の音声のステータスは <see cref="CriAtomExPlayback.Status.Removed"/> に遷移します。
		...
		///  sw に true を指定して本関数を実行すると、指定したIDの音声がポーズ （一時停止）されます。
		///  sw に false を指定して本関数を実行すると、指定したIDの音声のポーズが 解除され、一時停止していた音声の再生が再開されます。
		///  本関数を使用することで、プレーヤーによって再生された音声を、プレーヤー単位ではなく、 個別にポーズ／ポーズ解除させることが可能です。
		/// </para>
		/// <para>
		/// 備考:
		/// プレーヤーによって再生された全ての音声をポーズ／ポーズ解除したい場合、 本関数ではなく <see cref="CriAtomExPlayer.Pause"/> 関数をご利用ください。
		///  フェーダーをアタッチしたプレーヤーで再生した音声に対して本関数で個別にポーズ／ポーズ解除の操作を行った場合、 クロスフェード処理はフェードイン側の音声のポーズ状態に同期して行われます。
		///  例えば、クロスフェード中に<see cref="CriAtomExPlayer.Pause"/> 関数で両方の音声をポーズした場合、 フェードイン側の音声のポーズを解除すればクロスフェード処理が再開されますが、 フェードアウト側の音声のポーズを解除してもクロスフェード処理は再開されません。
		/// </para>
		/// <para>
		/// 注意:
		...
		///  本関数でポーズをかけた音声についてのみポーズを解除したい場合、 本関数を使用せず、 <see cref="CriAtomExPlayback.Resume"/>(id, <see cref="CriAtomEx.ResumeMode.PausedPlayback"/>); を実行してポーズ解除を行ってください。
		...
		/// <see cref="CriAtomExPlayback.Pause"/> 関数と異なり、 <see cref="CriAtomExPlayer.Prepare"/> 関数で再生開始待ちの音声と、 <see cref="CriAtomExPlayer.Pause"/> 関数（または <see cref="CriAtomExPlayback.Pause"/> 関数でポーズをかけた音声とを、 個別に再開させることが可能です。
		...
		/// <see cref="CriAtomExPlayback.Pause"/> 関数でポーズ状態のプレーヤーに対して <see cref="CriAtomExPlayer.Prepare"/> 関数で再生準備を行った場合、 その音声は <see cref="CriAtomEx.ResumeMode.PausedPlayback"/> 指定のポーズ解除処理と、 <see cref="CriAtomEx.ResumeMode.PreparedPlayback"/> 指定のポーズ解除処理の両方が行われるまで、再生が開始されません。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <returns><see cref="CriAtomExPlayback.Status"/> 再生ステータス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// <see cref="CriAtomExPlayer.GetStatus"/> 関数がAtomExプレーヤーのステータスを返すのに対し、 本関数は再生済みの個々の音声のステータスを取得します。
		///  再生中の音声のボイスリソースは、以下の場合に削除されます。
		/// <list type="bullet">
		/// <item><description>再生が完了した場合。</description></item>
		/// <item><description><see cref="CriAtomExPlayback.Stop"/> 関数で再生中の音声を停止した場合。</description></item>
		/// <item><description>高プライオリティの発音リクエストにより再生中のボイスが奪い取られた場合。</description></item>
		/// <item><description>再生中にエラーが発生した場合。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayback.Stop"/></see>
		/// <see><see cref="CriAtomExPlayback.Status.Removed"/></see>
		/// <para>
		/// そのため、 
		///  関数を使用して明示的に再生を停止したか、 その他の要因によって再生が停止されたかの違いに関係なく、 再生音のステータスはいずれの場合も 
		///  に遷移します。
		...
		{
			return (CriAtomExPlayback.Status)NativeMethods.criAtomExPlayback_GetStatus(NativeHandle);
		}
		...
		///  （ <see cref="CriAtomExPlayer.GetTime"/> 関数と異なり、本関数は再生中の音声ごとに時刻を 取得可能ですが、再生終了時刻を取ることができません。）
		...
		/// <para>
		/// 備考:
		/// <see cref="CriAtomExPlayback.GetTime"/> 関数が返す「再生開始後からの経過時間」とは 異なり、本関数からは再生中の音声に同期した再生時刻を取得することが 可能です。
		///  デバイスのリードリトライ処理等により音声データの供給が途切れて 再生が停止した場合、またはシステムの割り込みにより音声出力が妨げられた 場合には、再生時刻のカウントアップが一時的に停止します。
		///  再生された音声に厳密に同期した処理を行いたい場合は、本関数で 取得した再生時刻を用いてください。
		///  ただし、ループ再生時や、シームレス連結再生時に行った場合でも、 再生位置に応じて時刻が巻き戻ることはありません。
		///  また、波形の詰まっていないシーケンスキューや 再生波形が切り替わるブロックシーケンスキューに対しては、正常に再生時刻を 取得することができません。
		/// <see cref="CriAtomExPlayer.Pause"/> 関数でポーズをかけた場合、 再生時刻のカウントアップも停止します。
		///  （ポーズを解除すれば再度カウントアップが再開されます。）
		///  本関数による再生時刻の取得を行う場合は、対応するAtomExプレーヤー作成時に、 <see cref="CriAtomExPlayer.Config"/> 構造体の enable_audio_synced_timer を true に 設定してください。
		///  デフォルトでは無効になっています。
		...
		/// <summary>再生時刻の取得（再生音声に同期した補正込み） </summary>
		/// <returns>Sint64 再生時刻（マイクロ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で再生された音声の再生時刻を取得します。
		...
		/// <returns>Sint64 シーケンス再生位置（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <see cref="CriAtomExPlayer.Pause"/> 関数でポーズをかけた場合、 再生位置の更新も停止します。
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で再生された音声の再生サンプル数、 およびサンプリングレートを返します。
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で再生された音声について、 サウンドバッファーへの書き込み済みサンプル数、およびサンプリングレートを返します。
		///  本関数は <see cref="CriAtomExPlayback.GetNumPlayedSamples"/> 関数と異なり、 サウンドバッファーに書き込まれた未出力の音声データのサンプル数を含む値を返します。 
		...
		/// <returns>パラメーターが取得できたかどうか（ true = 取得できた、 false = 取得できなかった） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>AISACコントロール値が取得できたかどうか（ true = 取得できた、 false = 取得できなかった） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns><see cref="Int32"/> カレントブロックインデックス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で再生されたブロックシーケンスの カレントブロックインデックスを取得します。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <list type="bullet">
		/// <item><description>キュー以外のデータを再生している。（トラック情報が存在しないため）
		/// </description></item>
		/// <item><description>再生中のキューがポリフォニックタイプ、またはセレクター参照のスイッチタイプである。（トラック情報が複数存在する可能性があるため）
		/// </description></item>
		/// <item><description>再生中のキューがトラック遷移タイプである。（遷移により再生トラックが変わるため）
		/// </description></item>
		/// </list>
		/// </para>
		...
		/// <list type="bullet">
		/// <item><description>キュー以外のデータを再生している。（ビート同期情報が存在しないため）
		/// </description></item>
		/// <item><description>ビート同期情報が設定されていないキューを再生している。
		/// </description></item>
		/// <item><description>ビート同期情報が設定されているキューを"間接的"に再生している。（キューリンクやスタートアクションで再生している）
		/// </description></item>
		/// </list>
		/// </para>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <see cref="CriAtomExPlayer.EnumeratePlaybacks"/> 関数に本関数型のコールバック関数を登録することで、 プレーヤーで再生中のプレイバックIDをコールバックで受け取ることが可能となります。
		///  本コールバック関数内で長時間処理をブロックすると、音切れ等の問題が発生しますので、 ご注意ください。
		/// </para>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		public enum Event
		{
			...
			/// <list type="bullet">
			/// <item><description>キューに含まれる波形データを終端まで再生したため、ボイスが不要になった</description></item>
			/// <item><description><see cref="CriAtomExPlayer.Stop"/> 関数等の呼び出しにより、再生中の波形データが停止された</description></item>
			/// <item><description>プライオリティ制御により、再生中の波形データが停止され、ボイスが奪い取られた</description></item>
			/// </list>
			/// </para>
			/// <para>
			/// 備考:
			...
			/// <para>
			/// 備考:
			/// キューに含まれる波形データが再生されている場合、 本イベント発生前に、必ず <see cref="CriAtomExPlayback.Event.FromNormalToVirtual"/> イベントが発生します。 
			/// </para>
			/// </remarks>
			Remove = 3,
		}
		...
		///  再生状態は、通常以下の順序で遷移します。
		/// <list type="number">
		/// <item><description><see cref="CriAtomExPlayback.Status.Prep"/></description></item>
		/// <item><description><see cref="CriAtomExPlayback.Status.Playing"/></description></item>
		/// <item><description><see cref="CriAtomExPlayback.Status.Removed"/></description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考
		/// <see cref="CriAtomExPlayback.Status"/>はAtomExプレーヤーのステータスではなく、 プレーヤーで再生を行った（ <see cref="CriAtomExPlayer.Start"/> 関数を実行した） 音声のステータスです。
		///  再生中の音声リソースは、発音が停止された時点で破棄されます。
		///  そのため、以下のケースで再生音のステータスが <see cref="CriAtomExPlayback.Status.Removed"/> に遷移します。
		/// <list type="bullet">
		/// <item><description>再生が完了した場合。</description></item>
		/// <item><description><see cref="CriAtomExPlayback.Stop"/> 関数で再生中の音声を停止した場合。</description></item>
		/// <item><description>高プライオリティの発音リクエストにより再生中のボイスが奪い取られた場合。</description></item>
