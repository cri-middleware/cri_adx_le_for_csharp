...
namespace CriWare
{
	/// <summary>プレーヤーハンドル </summary>
	/// <remarks>
	/// <para>
	/// 説明:
	/// <see cref="CriAtomExPlayer"/> は、音声再生用に作られたプレーヤーを操作するためのオブジェクトです。
	/// <see cref="CriAtomExPlayer.CriAtomExPlayer"/> 関数で音声再生用のプレーヤーを作成すると、 関数はプレーヤー操作用に、この"AtomExプレーヤーオブジェクト"を返します。 
	///  データのセットや再生の開始、ステータスの取得等、プレーヤーに対して行う操作は、 全てAtomExプレーヤーオブジェクトを介して実行されます。
	/// </para>
	/// </remarks>
	...
	public partial class CriAtomExPlayer : IDisposable
	{
		...
		///  例えば、 start_time_us に 10000000 をセットして本関数を実行すると、 次に再生する音声データは 10 秒目の位置から再生されます。 
		/// </para>
		/// <para>
		/// 備考:
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のパンニング3D角度を更新することができます。
		///  角度は度単位で指定します。
		...
		/// <returns>CriBool 独自のパンニング処理を行ったかどうか </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// パンニング処理用のコールバック関数の型です。
		/// <see cref="CriAtomExPlayer.OverrideDefaultPanMethod"/> 関数を実行することで、 コールバック関数の登録が可能です。
		...
		///  アプリケーション側でセンドレベルマトリクスを操作した場合には、 関数の戻り値で true を返す必要があります。
		///  false を返した場合、コールバック内で指定したセンドレベルは無視され、 Atomライブラリのデフォルトのパンニング処理が適用されます。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  PCM データのフォーマットが 16 bit 整数型の場合、 format は <see cref="CriAtom.PcmFormat.Sint16"/> となり、 PCM データのフォーマットが 32 bit 浮動小数点数型の場合、 format は <see cref="CriAtom.PcmFormat.Float32"/> となります。
		///  それぞれのケースで PCM データの値域は異なりますのでご注意ください。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtom.PcmFormat.Sint16"/> 時は -32768 ～ +32767</description></item>
		/// <item><description><see cref="CriAtom.PcmFormat.Float32"/> 時は -1.0f ～ +1.0f</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// </para>
		/// <para>（デコード時点ではクリッピングが行われていないため、 <see cref="CriAtom.PcmFormat.Float32"/> 時は上記範囲をわずかに超えた値が出る可能性があります。）</para>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <see cref="CriAtomExPlayer.SetWideness"/> 関数と異なり、モノラル音源にも適用可能です。 
		/// </para>
		/// <para>
		/// 備考：
		/// 本関数でパンニング時の角度タイプを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、設定された広がりでパンニング計算されます。
		...
		/// <seealso cref="CriAtomExPlayer.UpdateAll"/>
		/// <seealso cref="CriAtomExPlayer.ResetParameters"/>
		/// <seealso cref="CriAtomExPlayer.SetWideness"/>
		public void SetSpread(Single spread)
		{
			NativeMethods.criAtomExPlayer_SetSpread(NativeHandle, spread);
		}
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExPlayer.CriAtomExPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExPlayer.Config"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExPlayer_SetDefaultConfig_(CriAtomExPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <summary>出力ポートハンドルの追加 </summary>
		/// <param name="outputPort">出力ポートハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// プレーヤーに出力ポートを追加します。
		...
		/// <para>
		/// 備考:
		/// 本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数または <see cref="CriAtomExPlayer.ClearOutputPorts"/> 関数にてクリアされます。 また、<see cref="CriAtomExPlayer.RemoveOutputPort"/> 関数で特定のオブジェクトのみを取り外すことも可能です。 
		///  キュー再生時に本関数を呼び出すと、以下の設定は全て<b>無視</b>され、キューは全て追加した出力ポートを通して再生されます。<list type="bullet">
		/// <item><description>データ側に設定されているパラメーターパレットのASRラックID設定</description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetAsrRackId"/> 関数及び <see cref="CriAtomExPlayer.SetAsrRackIdArray"/> 関数で指定したASRラックID</description></item>
		/// <item><description>データ側に設定されているトラックの出力ポート名 
		...
		/// <see cref="CriAtomExPlayer.SetData"/> 関数等を使用したキュー再生以外の再生時では、本関数にて指定した複数の出力ポートの内、 1つ目に設定した出力ポートのみが適用されます。
		///  HCA-MX用にエンコードされた音声データには、本関数の設定が適用されません。
		///  HCA-MX用にエンコードされた音声データについて出力先設定する場合、 </description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <summary>出力ポートハンドルの取り外し </summary>
		/// <param name="outputPort">出力ポートハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// プレーヤーに <see cref="CriAtomExPlayer.AddOutputPort"/> 関数で追加した特定の出力ポートオブジェクトを取り外します。 プレーヤーに設定されている出力ポートを全てを取り外すには <see cref="CriAtomExPlayer.ClearOutputPorts"/> 関数を使用してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 既に再生が開始された音声に対し、後から出力ポートを変更することはできません。
		...
		/// <para>
		/// 備考:
		/// プレーヤーに <see cref="CriAtomExPlayer.AddOutputPort"/> 関数で追加した特定の出力ポートオブジェクトを全てクリアします。 特定のオブジェクトを取り外すためには <see cref="CriAtomExPlayer.RemoveOutputPort"/> 関数を使用してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 既に再生が開始された音声に対し、後から出力ポートを変更することはできません。
		...
		/// <summary>優先出力ポートハンドルの追加 </summary>
		/// <param name="outputPort">出力ポートハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// 本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数または <see cref="CriAtomExPlayer.ClearPreferredOutputPorts"/> にてクリアされます。 また、<see cref="CriAtomExPlayer.RemovePreferredOutputPort"/> 関数や <see cref="CriAtomExPlayer.RemovePreferredOutputPortByName"/> 関数にて特定のオブジェクトのみを取り外すことも可能です。 
		...
		/// <summary>優先出力ポートハンドルの取り外し </summary>
		/// <param name="outputPort">出力ポートハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// プレーヤーに <see cref="CriAtomExPlayer.AddPreferredOutputPort"/> 関数で追加した特定の優先出力ポートオブジェクトを取り外します。 プレーヤーに設定されている出力ポートを全て取り外すには <see cref="CriAtomExPlayer.ClearOutputPorts"/> 関数を使用してください。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <summary>優先出力ポートハンドルの取り外し（名前指定） </summary>
		...
		/// <para>
		/// 備考:
		/// プレーヤーに <see cref="CriAtomExPlayer.AddPreferredOutputPort"/> 関数で追加した特定の優先出力ポートオブジェクトを取り外します。 プレーヤーに設定されている出力ポートを全てクリアするためには <see cref="CriAtomExPlayer.ClearOutputPorts"/> 関数を使用してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 優先出力ポートを取り外しても、既に再生が開始された音声には影響しません。
		...
		/// <summary>優先出力ポートハンドルのクリア </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// プレーヤーに <see cref="CriAtomExPlayer.AddPreferredOutputPort"/> 関数で追加した優先出力ポートオブジェクトを全てクリアします。 特定の優先出力ポートを取り外すためには、<see cref="CriAtomExPlayer.RemovePreferredOutputPort"/> 関数または <see cref="CriAtomExPlayer.RemovePreferredOutputPortByName"/> 関数を使用してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 優先出力ポートを取り外しても、既に再生が開始された音声には影響しません。
		...
		///  アロケーターを登録せずにAtomExプレーヤーを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomExPlayer.CriAtomExPlayer"/> 関数にセットする必要があります。
		///  プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtomExPlayer.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomExPlayer.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomExPlayer.SetDefaultConfig"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		...
		public unsafe partial struct Config
		{
			...
			///  voice_allocation_method に <see cref="CriAtomEx.VoiceAllocationMethod.AllocateVoiceOnce"/> を指定した場合、 AtomExプレーヤーはボイスの確保を発音開始のタイミングでのみ行います。
			...
			///  これに対し、 voice_allocation_method に <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定した場合、AtomExプレーヤーはボイスの確保を必要な限り何度も繰り返します。
			///  ボイスが確保できない場合やボイスを奪い取られた場合でも、 発音を管理するリソース（バーチャルボイス）は解放しないので、 再度ボイスに空きができた時点で、発音処理が再開されます。
			/// </para>
			/// <para>
			/// 備考:
			/// <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定した場合、発音中のボイスの処理に加え、 発音を行っていないバーチャルボイスについても定期的にボイスの再取得処理等が行われるため、 <see cref="CriAtomEx.VoiceAllocationMethod.AllocateVoiceOnce"/> を指定した場合に比べ、 処理負荷が高くなる可能性があります。
			///  <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> 指定時、 ボイスの再確保に成功すると、波形データは<b>再生時刻を考慮した位置から</b>シーク再生されます。
			/// </para>
			/// <para>
			/// 注意:
			/// <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定して AtomEx プレーヤーを作成した場合でも、 Atom ライブラリ初期化時に指定する max_virtual_voices の数を超える再生要求があった場合、発音は再開されなくなります。
			///  （エラーコールバック関数に警告が返され、バーチャルボイスも削除されます。）
			///  <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定する際には、 初期化時に必要充分なバーチャルボイスを確保してください。
			///  （ max_virtual_voices に大きめの値を指定してください。）
			///  <see cref="CriAtomEx.VoiceAllocationMethod.RetryVoiceAllocation"/> を指定した場合、 再生されなかった波形データやボイスが奪い取られた波形データが、 いつどこから再生再開されるか、厳密に制御することはできません。
			...
			/// <para>
			/// 備考:
			/// <see cref="CriAtomExPlayer.SetFile"/> 関数を実行すると、 指定したパス文字列がAtomExプレーヤー内に保持されます。
			...
			/// <para>
			/// 注意:
			/// <see cref="CriAtomExPlayer.SetDefaultConfig"/> メソッドで <see cref="CriAtomExPlayer.Config"/> 構造体に デフォルト値を設定した場合、 max_path には 0 がセットされます。
			///  ファイル名を指定して再生を行う場合、 <see cref="CriAtomExPlayer.SetDefaultConfig"/> メソッドを使用しないか、または <see cref="CriAtomExPlayer.SetDefaultConfig"/> メソッド実行後 に再度パスの最大長をセットする必要があります。
			/// </para>
			/// </remarks>
			/// <seealso cref="CriAtomExPlayer.SetDefaultConfig"/>
			/// <seealso cref="CriAtomExPlayer.SetFile"/>
			...
			/// <para>
			/// 備考:
			/// updates_time に false を指定した場合、 作成されたAtomExプレーヤーは再生時刻の更新を行いません。
			///  その結果、 <see cref="CriAtomExPlayer.GetTime"/> 関数による再生時刻の取得は行えなくなりますが、 音声再生時の処理負荷をわずかに下げることが可能となります。
			/// </para>
			/// </remarks>
			...
			/// <para>
			/// 備考:
			/// enable_audio_synced_timer に true を指定した場合、 作成されたAtomExプレーヤーを用いて再生された音声に対して、 再生済みサンプル数に同期するように補正した再生時刻の更新を行うようになります。
			///  補正された再生時刻は <see cref="CriAtomExPlayback.GetTimeSyncedWithAudio"/> 関数によって取得できます。
			///  音声再生時の処理負荷が上がるため、音声に同期した正確な再生時刻を取得したいプレーヤーの 作成時のみ true を指定するようにしてください。
			/// </para>
			/// <para>
			/// 注意:
		...
		/// <returns><see cref="CriAtomExPlayer"/> AtomExプレーヤーオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// AtomExプレーヤーを作成します。
		...
		///  AtomExプレーヤー作成時に確保されたメモリは、AtomExプレーヤー破棄時（ <see cref="CriAtomExPlayer.Dispose"/> 関数実行時）に解放されます。
		...
		///  AtomExプレーヤー作成前に <see cref="CriAtomExPlayer.CalculateWorkSize"/> 関数で取得した サイズ分のメモリを予め確保しておき、本関数に設定してください。
		///  尚、Fixed Memory方式を用いた場合、ワーク領域はAtomExプレーヤーの破棄 （ <see cref="CriAtomExPlayer.Dispose"/> 関数）を行うまでの間、ライブラリ内で利用され続けます。
		...
		/// <list type="number">
		/// <item><description>AtomExプレーヤー作成前に、 <see cref="CriAtomEx.SetUserAllocator"/> 関数を用いてメモリ確保／解放関数を登録する。
		/// </description></item>
		/// <item><description>AtomExプレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description><see cref="CriAtomExPlayer.CriAtomExPlayer"/> 関数でAtomExプレーヤーを作成する。
		///  （workにはnull、work_sizeには0を指定する。）
		/// </description></item>
		/// <item><description>オブジェクトが不要になったら <see cref="CriAtomExPlayer.Dispose"/> 関数でAtomExプレーヤーを破棄する。
		/// </description></item>
		/// </list>
		/// </para>
		/// <list>
		...
		/// <item><description>AtomExプレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description>AtomExプレーヤーの作成に必要なワーク領域のサイズを、 <see cref="CriAtomExPlayer.CalculateWorkSize"/> 関数を使って計算する。
		/// </description></item>
		...
		public unsafe CriAtomExPlayer(in CriAtomExPlayer.Config config)
		{
			fixed (CriAtomExPlayer.Config* configPtr = &config)
				...
				NativeHandle = NativeMethods.criAtomExPlayer_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomExPlayer()
		{
...
#pragma warning restore 1591
		...
		/// <param name="acbHn">ACBハンドル </param>
		/// <param name="id">キューID </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// キューIDを、AtomExプレーヤーに関連付けます。
		///  本関数でキューIDを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定されたキューが再生されます。 
		/// </para>
		/// <para>
		...
		/// <see cref="CriAtomExPlayer.SetCueId"/> 関数でキューをセットした場合、以下の関数で設定された パラメーターは無視されます。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		...
		/// <returns><see cref="CriAtomExPlayback"/> 再生ID </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 音声データの再生処理を開始します。
		///  本関数を実行する前に、事前に <see cref="CriAtomExPlayer.SetData"/> 関数等を使用し、再生する 音声データをAtomExプレーヤーにセットしておく必要があります。
		///  例えば、オンメモリの音声データを再生する場合には、以下のように事前に <see cref="CriAtomExPlayer.SetData"/> 関数を使って音声データをセットした後、本関数を実行する 必要があります。
		...
		/// <list type="number">
		/// <item><description><see cref="CriAtomExPlayer.Status.Stop"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.Status.Prep"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.Status.Playing"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.Status.Playend"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.Status.Error"/></description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		/// 関数実行時に発音リソースが確保できない場合（全てのボイスが使用中で、なおかつ 他のボイスを奪い取れない場合等）、本関数は <see cref="CriAtomEx.InvalidPlaybackId"/> を返します。
		///  しかし、戻り値を元にエラーチェックを行わなくても、ほとんどのケースで問題は発生しません。
		///  再生ID（ <see cref="CriAtomExPlayback"/> ）を使用する API に対し、 <see cref="CriAtomEx.InvalidPlaybackId"/> をセットしたとしても、Atomライブラリは特に何も処理しません。
		///  そのため、デバッグ目的で発音が行われたかどうかをチェックしたい場合を除き、 本関数の結果に応じてアプリケーション側で処理を切り分ける必要はありません。
		///  （ <see cref="CriAtomEx.InvalidPlaybackId"/> が返された際に、有効な再生IDが返された場合と 同様の処理を行っても、エラーコールバック等は発生しません。） 
		/// </para>
		/// <para>
		/// AtomExプレーヤーを作成した時点では、AtomExプレーヤーのステータスは停止状態 （ <see cref="CriAtomExPlayer.Status.Stop"/> ）です。
		///  再生する音声データをセット後、本関数を実行することで、AtomExプレーヤーのステータスが 準備状態（ <see cref="CriAtomExPlayer.Status.Prep"/> ）に変更されます。
		///  （<see cref="CriAtomExPlayer.Status.Prep"/> は、データ供給やデコードの開始を待っている状態です。）
		///  再生の開始に充分なデータが供給された時点で、AtomExプレーヤーはステータスを 再生状態（ <see cref="CriAtomExPlayer.Status.Playing"/> ）に変更し、音声の出力を開始します。
		///  セットされたデータを全て再生し終えると、AtomExプレーヤーはステータスを再生終了状態 （ <see cref="CriAtomExPlayer.Status.Playend"/> ）に変更します。
		///  尚、再生中にエラーが発生した場合には、AtomExプレーヤーはステータスをエラー状態 （ <see cref="CriAtomExPlayer.Status.Error"/> ）に変更します。
		...
		/// <param name="acbHn">ACBハンドル </param>
		/// <param name="cueName">
		/// キュー名 キュー名を、AtomExプレーヤーに関連付けます。
		///  本関数でキュー名を指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定されたキューが再生されます。 
		/// </param>
		/// <remarks>
		/// <para>
		/// 尚、一旦セットしたデータの情報は、他のデータがセットされるまでAtomExプレーヤー内に保持 されます。
		...
		///  この際、検索の順序は、ACBデータのロード順とは逆順で行われます。
		///  （後からロードされたデータから優先的に検索が行われます。）
		/// <see cref="CriAtomExPlayer.SetCueName"/> 関数でキューをセットした場合、以下の関数で設定された パラメーターは無視されます。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetSamplingRate"/></description></item>
		...
		/// <param name="acbHn">ACBハンドル </param>
		/// <param name="index">
		/// キューインデックス キューインデックスを、AtomExプレーヤーに関連付けます。
		///  本関数でキューインデックスを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定されたキューが再生されます。 
		/// </param>
		/// <remarks>
		/// <para>
		/// 尚、一旦セットしたデータの情報は、他のデータがセットされるまでAtomExプレーヤー内に保持 されます。
		...
		///  この際、検索の順序は、ACBデータのロード順とは逆順で行われます。
		///  （後からロードされたデータから優先的に検索が行われます。）
		/// <see cref="CriAtomExPlayer.SetCueIndex"/> 関数でキューをセットした場合、以下の関数で設定された パラメーターは無視されます。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetSamplingRate"/></description></item>
		...
		///  本関数でメモリアドレスとサイズを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定されたデータが再生されます。 
		/// </para>
		/// <para>
		/// 尚、一旦セットしたデータの情報は、他のデータがセットされるまでAtomExプレーヤー内に保持 されます。
		///  そのため、同じデータを何度も再生する場合には、再生毎にデータをセットしなおす必要 はありません。 
		...
		///  本関数でセットしたメモリ領域を解放する際には、事前に <see cref="CriAtomEx.IsDataPlaying"/> 関数を実行し、当該メモリ領域への参照が行われていないことを確認してください。 
		/// <see cref="CriAtomExPlayer.SetData"/> 関数で音声データをセットする場合、以下の関数を使用して 再生する音声データの情報を別途指定する必要があります。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetSamplingRate"/></description></item>
		...
		///  この関数は、<see cref="CriAtomExPlayer.Start"/> 関数でボイスプールからボイスを 取得する際の、取得対象を絞り込む関数のひとつです。
		///  この関数では、取得対象ボイスを、 指定したフォーマットのデータを再生可能なボイスに絞り込みます。
		///  絞り込みを行う関数は、この関数のほかに、<see cref="CriAtomExPlayer.SetSamplingRate"/> 関数と <see cref="CriAtomExPlayer.SetNumChannels"/> 関数があります。
		...
		///  絞り込みを行う関数は、この関数のほかに、<see cref="CriAtomExPlayer.SetFormat"/> 関数と <see cref="CriAtomExPlayer.SetSamplingRate"/> 関数があります。
		...
		///  絞り込みを行う関数は、この関数のほかに、<see cref="CriAtomExPlayer.SetFormat"/> 関数と <see cref="CriAtomExPlayer.SetNumChannels"/> 関数があります。
		...
		/// <param name="binder">バインダーハンドル </param>
		/// <param name="path">ファイルパス </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 音声ファイルをAtomExプレーヤーに関連付けます。
		///  本関数でファイルを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を開始すると、 指定されたファイルがストリーミング再生されます。
		...
		///  （max_path_strings に2以上の値を指定することで、 1つのプレーヤーで複数のファイルを同時にパス指定で再生することが可能となります。）
		///  ただし、 max_path_strings の値に応じて必要なワーク領域のサイズは増加します。
		/// <see cref="CriAtomExPlayer.SetFile"/> 関数で音声データをセットする場合、以下の関数を使用して 再生する音声データの情報を別途指定する必要があります。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetSamplingRate"/></description></item>
		...
		///  本関数にバインダーとコンテンツIDを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定されたコンテンツファイルがストリーミング再生されます。
		///  尚、本関数を実行した時点では、ファイルの読み込みは開始されません。
		///  ファイルの読み込みが開始されるのは、 <see cref="CriAtomExPlayer.Start"/> 関数実行後です。
		/// </para>
		/// <para>
		...
		/// <para>
		/// 注意:
		/// <see cref="CriAtomExPlayer.SetContentId"/> 関数で音声データをセットする場合、以下の関数を使用して 再生する音声データの情報を別途指定する必要があります。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExPlayer.SetFormat"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetNumChannels"/></description></item>
		/// <item><description><see cref="CriAtomExPlayer.SetSamplingRate"/></description></item>
		...
		///  本関数にAWBオブジェクトと波形データIDを指定後、 <see cref="CriAtomExPlayer.Start"/> 関数で再生を 開始すると、指定した波形データがストリーミング再生されます。
		///  尚、本関数を実行した時点では、ファイルの読み込みは開始されません。
		///  ファイルの読み込みが開始されるのは、 <see cref="CriAtomExPlayer.Start"/> 関数実行後です。
		/// </para>
		/// <para>
		...
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomAwb.Dispose"/></see>
		/// <see><see cref="CriAtomAwb.Dispose"/></see>
		/// <para>
		///  本関数でセットした音声を再生中に、 
		///  関数でデータを破棄しないでください。
		...
		/// <returns><see cref="CriAtomExPlayer.Status"/> ステータス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// AtomExプレーヤーを作成した時点では、AtomExプレーヤーのステータスは停止状態 （ <see cref="CriAtomExPlayer.Status.Stop"/> ）です。
		///  再生する音声データをセット後、<see cref="CriAtomExPlayer.Start"/> 関数を実行することで、 AtomExプレーヤーのステータスが準備状態（ <see cref="CriAtomExPlayer.Status.Prep"/> ）に変更されます。
		///  （<see cref="CriAtomExPlayer.Status.Prep"/> は、データ供給やデコードの開始を待っている状態です。）
		///  再生の開始に充分なデータが供給された時点で、AtomExプレーヤーはステータスを 再生状態（ <see cref="CriAtomExPlayer.Status.Playing"/> ）に変更し、音声の出力を開始します。
		///  セットされたデータを全て再生し終えると、AtomExプレーヤーはステータスを再生終了状態 （ <see cref="CriAtomExPlayer.Status.Playend"/> ）に変更します。
		///  尚、再生中にエラーが発生した場合には、AtomExプレーヤーはステータスをエラー状態 （ <see cref="CriAtomExPlayer.Status.Error"/> ）に変更します。
		...
		{
			return (CriAtomExPlayer.Status)NativeMethods.criAtomExPlayer_GetStatus(NativeHandle);
		}
		...
		/// 説明:
		/// 再生のポーズ／ポーズ解除を行います。
		///  sw に true を指定して本関数を実行すると、AtomExプレーヤーは再生中の 音声をポーズ（一時停止）します。
		///  sw に false を指定して本関数を実行すると、AtomExプレーヤーはポーズを 解除し、一時停止していた音声の再生を再開します。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <para>
		/// 注意:
		/// 第2引数（sw）に false を指定してポーズ解除の操作を行った場合、 本関数でポーズをかけた音声だけでなく、<see cref="CriAtomExPlayer.Prepare"/> 関数で再生準備中の音声についても再生が開始されてしまいます。
		///  （旧バージョンとの互換性維持のための仕様です。）
		///  本関数でポーズをかけた音声についてのみポーズを解除したい場合、 本関数を使用せず、 <see cref="CriAtomExPlayer.Resume"/>(player, <see cref="CriAtomEx.ResumeMode.PausedPlayback"/>); を実行してポーズ解除を行ってください。
		...
		/// 説明:
		/// 音声データの再生を準備します。
		///  本関数を実行する前に、事前に <see cref="CriAtomExPlayer.SetData"/> 関数等を使用し、 再生すべき音声データをAtomExプレーヤーにセットしておく必要があります。
		...
		///  本関数で再生準備を行った音声を発音するには、 本関数が返す再生 ID （ <see cref="CriAtomExPlayback"/> ）に対し、 <see cref="CriAtomExPlayback.Pause"/> (id, false); の操作を行う必要があります。
		/// </para>
		/// <para>
		/// 備考:
		/// ストリーミング再生の音声を再生する際、本関数を使用せず <see cref="CriAtomExPlayer.Start"/> 関数を使用した場合、 実際に音声の再生が開始されるまでにはタイムラグがあります。
		...
		/// <see><see cref="CriAtomExPlayer.Pause"/></see>
		/// <see><see cref="CriAtomExPlayer.Resume"/></see>
		/// <see><see cref="CriAtomExPlayback.Resume"/></see>
		/// <para>
		///  ポーズ解除処理に 
		///  関数を使用した場合、 本関数による再生準備のためのポーズと、 
		///  関数による一時停止処理の両方が解除されます。
		///  関数でポーズした音声を停止したまま 本関数で再生準備を行った音声を再生したい場合、ポーズの解除に 
		///  関数（または 
		///  関数）をご利用ください。
		/// </para>
		...
		/// 説明:
		/// 再生の停止要求を発行します。
		///  音声再生中のAtomExプレーヤーに対して本関数を実行すると、 AtomExプレーヤーは再生を停止（ファイルの読み込みや、発音を停止）し、 ステータスを停止状態（ <see cref="CriAtomExPlayer.Status.Stop"/> ）に遷移します。
		/// </para>
		/// <para>
		/// 備考:
		/// 既に停止しているAtomExプレーヤー（ステータスが <see cref="CriAtomExPlayer.Status.Playend"/> や <see cref="CriAtomExPlayer.Status.Error"/> のAtomExプレーヤー） に対して本関数を実行すると、 AtomExプレーヤーのステータスを <see cref="CriAtomExPlayer.Status.Stop"/> に変更します。 
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数は完了復帰型の関数ではありません。
		...
		///  本関数を実行すると、AtomExプレーヤーは再生を停止（ファイルの読み込みや、発音を停止）し、 ステータスを停止状態（ <see cref="CriAtomExPlayer.Status.Stop"/> ）に遷移します。
		/// </para>
		/// <para>
		/// 備考:
		/// 既に停止しているAtomExプレーヤー（ステータスが <see cref="CriAtomExPlayer.Status.Playend"/> や <see cref="CriAtomExPlayer.Status.Error"/> のAtomExプレーヤー） についても、 本関数が実行されるとステータスが <see cref="CriAtomExPlayer.Status.Stop"/> に変更されます。 
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数は完了復帰型の関数ではありません。
		...
		///  本関数を実行すると、第 1 引数（ func ） でセットされたコールバック関数がAtomExプレーヤーの数分だけ呼び出されます。
		...
		/// <see cref="CriAtomExPlayer.EnumeratePlayers"/> 関数に本関数型のコールバック関数を登録することで、 アプリケーション中で作成したプレーヤーをコールバックで受け取ることが可能となります。
		/// </para>
		/// <para>
		/// 注意:
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// 説明:
		/// 一時停止状態の解除を行います。
		/// <see cref="CriAtomExPlayer.Pause"/> 関数と異なり、 <see cref="CriAtomExPlayer.Prepare"/> 関数で再生開始待ちの音声と、 <see cref="CriAtomExPlayer.Pause"/> 関数（または <see cref="CriAtomExPlayback.Pause"/> 関数でポーズをかけた音声とを、 個別に再開させることが可能です。
		///  第2引数（mode）に <see cref="CriAtomEx.ResumeMode.PausedPlayback"/> を指定して本関数を実行すると、 ユーザが <see cref="CriAtomExPlayer.Pause"/> 関数（または <see cref="CriAtomExPlayback.Pause"/> 関数）で一時停止状態になった音声の再生が再開されます。
		///  第2引数（mode）に <see cref="CriAtomEx.ResumeMode.PreparedPlayback"/> を指定して本関数を実行すると、 ユーザが <see cref="CriAtomExPlayer.Prepare"/> 関数で再生準備を指示した音声の再生が開始されます。
		/// <see cref="CriAtomExPlayer.Pause"/> 関数でポーズ状態のプレーヤーに対して <see cref="CriAtomExPlayer.Prepare"/> 関数で再生準備を行った場合、その音声は <see cref="CriAtomEx.ResumeMode.PausedPlayback"/> 指定のポーズ解除処理と、 <see cref="CriAtomEx.ResumeMode.PreparedPlayback"/> 指定のポーズ解除処理の両方が行われるまで、再生が開始されません。
		/// </para>
		/// <para>
		/// 備考:
		/// <see cref="CriAtomExPlayer.Pause"/> 関数か <see cref="CriAtomExPlayer.Prepare"/> 関数かに関係なく、 常に再生を開始したい場合には、第2引数（mode）に <see cref="CriAtomEx.ResumeMode.AllPlayback"/> を指定して本関数を実行するか、または <see cref="CriAtomExPlayer.Pause"/>(player, false); を実行してください。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriBool ポーズ中かどうか（false = ポーズされていない、true = ポーズ中） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// プレーヤーがポーズ中かどうかを返します。
		...
		/// <see cref="CriAtomExPlayer.Pause"/> 関数実行後、再生ID指定で個々の音声のポーズを解除 （ <see cref="CriAtomExPlayback.Pause"/> 関数を実行）した場合、本関数は false を 返します。
		...
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.SetData"/></see>
		/// <see><see cref="CriAtomExPlayer.Start"/></see>
		/// <para>
		/// 備考
		/// AtomExプレーヤーは、Atomプレーヤーと異なり、1つのプレーヤーで複数音の再生が可能です。
		///  そのため、再生中のAtomExプレーヤーに対して <see cref="CriAtomExPlayer.Start"/> 関数を実行すると、 2つの音が重なって再生されます。
		///  再生中に <see cref="CriAtomExPlayer.Stop"/> 関数を実行した場合、AtomExプレーヤーで再生中の全ての音声 が停止し、ステータスは <see cref="CriAtomExPlayer.Status.Stop"/> に戻ります。
		///  （ <see cref="CriAtomExPlayer.Stop"/> 関数の呼び出しタイミングによっては、 <see cref="CriAtomExPlayer.Status.Stop"/> に遷移するまでに時間がかかる場合があります。）
		///  1つのAtomExプレーヤーで複数回 <see cref="CriAtomExPlayer.Start"/> 関数を実行した場合、 1つでも再生準備中の音があれば、ステータスは <see cref="CriAtomExPlayer.Status.Prep"/> 状態になります。
		///  （全ての音声が再生中の状態になるまで、ステータスは <see cref="CriAtomExPlayer.Status.Playing"/> 状態に 遷移しません。）
		///  また、 <see cref="CriAtomExPlayer.Status.Playing"/> 状態のプレーヤーに対し、再度 <see cref="CriAtomExPlayer.Start"/> 関数を実行した場合、ステータスは一時的に <see cref="CriAtomExPlayer.Status.Prep"/> に戻ります。
		///  再生中に不正なデータを読み込んだ場合や、ファイルアクセスに失敗した場合、 ステータスは <see cref="CriAtomExPlayer.Status.Error"/> に遷移します。
		///  複数の音声を再生中にある音声でエラーが発生した場合、プレーヤーのステータスは 他の音声の状態に関係なく、 <see cref="CriAtomExPlayer.Status.Error"/> に遷移します。
		/// </para>
		/// <para>
		/// AtomExプレーヤー作成直後の状態は、停止状態（ <see cref="CriAtomExPlayer.Status.Stop"/> ）です。
		///  関数等でデータをセットし、 
		///  関数を 実行すると、再生準備状態（ <see cref="CriAtomExPlayer.Status.Prep"/> ）に遷移し、再生準備を始めます。
		///  データが充分供給され、再生準備が整うと、ステータスは再生中（ <see cref="CriAtomExPlayer.Status.Playing"/> ） に変わり、音声の出力が開始されます。
		///  セットされたデータを全て再生し終えた時点で、ステータスは再生完了 （ <see cref="CriAtomExPlayer.Status.Playend"/> ）に変わります。 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomExPlayer.GetStatus"/>
		/// <seealso cref="CriAtomExPlayer.SetData"/>
		...
		/// <returns>CriSint32 再生音数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriSint32 再生ID </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>Sint64 再生時刻（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  複数の音声に対して再生時刻をチェックする必要がある場合には、 再生する音声の数分だけプレーヤーを作成するか、または <see cref="CriAtomExPlayback.GetTime"/> 関数をご利用ください。
		///  本関数が返す再生時刻は「再生開始後からの経過時間」です。
		///  ループ再生時や、シームレス連結再生時を行った場合でも、 再生位置に応じて時刻が巻き戻ることはありません。
		/// <see cref="CriAtomExPlayer.Pause"/> 関数でポーズをかけた場合、 再生時刻のカウントアップも停止します。
		///  （ポーズを解除すれば再度カウントアップが再開されます。） 
		///  本関数で取得可能な時刻の精度は、サーバー処理の周波数に依存します。
		///  （時刻の更新はサーバー処理単位で行われます。）
		///  より精度の高い時刻を取得する必要がある場合には、本関数の代わりに <see cref="CriAtomExPlayback.GetNumPlayedSamples"/> 関数を使用し、 再生済みサンプル数を取得してください。
		/// </para>
		/// <para>
		/// 注意:
		...
		///  AtomExプレーヤー作成時、 <see cref="CriAtomExPlayer.Config"/> 構造体の updates_time を false に設定した場合、当該プレーヤーから再生時刻を取得することはできなくなります。
		...
		/// <see cref="CriAtomExPlayer.Start"/> 関数で音声を再生した際、AtomExプレーヤーは本関数で 指定されたサウンドレンダラから出力するボイスを、ボイスプールから取得します。
		...
		///  group_no に <see cref="CriAtomExPlayer.NoGroupLimitation"/> を指定した場合、 プレーヤーはボイスリミットグループによる制限を受けなくなります。
		///  （空きボイスがあるか、または自身より低プライオリティのボイスがあれば、 ボイスリミットグループに関係なくボイスを取得します。）
		/// </para>
		/// <para>
		/// 備考:
		/// <see cref="CriAtomExPlayer.Start"/> 関数で再生を開始した際、 指定したボイスリミットグループのボイスが全て使用中だった場合、 再生した音声が発音されるかどうかは、ボイスプライオリティ制御によって決まります。
		...
		///  本関数でボイス制御方法をセット後、 <see cref="CriAtomExPlayer.Start"/> 関数で音声を再生すると、 当該プレーヤーで再生する波形データには、本関数で指定した制御方式が適用されます。
		///  ボイス制御方法（ method ）には、以下のいずれかが指定可能です。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomEx.VoiceControlMethod.PreferLast"/></description></item>
		/// <item><description><see cref="CriAtomEx.VoiceControlMethod.PreferFirst"/></description></item>
		/// <item><description><see cref="CriAtomEx.VoiceControlMethod.PreferData"/></description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		...
		///  （ボイスプライオリティによる発音制御の詳細は <see cref="CriAtomExPlayer.SetVoicePriority"/> 関数の説明をご参照ください。）
		/// </para>
		/// <para>
		///  空きボイスがない状態で再生中のボイスと同プライオリティの音声を再生した場合、 ボイス制御方式に <see cref="CriAtomEx.VoiceControlMethod.PreferLast"/> が指定されていれば、 再生中のボイスを停止して新規に音声の再生を開始します。
		///  同条件で <see cref="CriAtomEx.VoiceControlMethod.PreferFirst"/> が指定されている場合、 新規の再生リクエストがキャンセルされ、既存のボイスが再生を続けます。
		///  <see cref="CriAtomEx.VoiceControlMethod.PreferData"/> が指定されている場合、 データにあらかじめ設定されているボイス制御方式（オーサリングツール上で設定した値） が使用されます。
		...
		///  playback_id に <see cref="CriAtomEx.InvalidPlaybackId"/> を指定すると、 再生IDの登録がクリアされます。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description>ADX</description></item>
		/// <item><description>HCA</description></item>
		/// <item><description>Wave</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.SetStartTime"/></see>
		/// <see><see cref="CriAtomExPlayer.SetStartTime"/></see>
		/// <see><see cref="CriAtomExPlayer.SetStartTime"/></see>
		/// <see><see cref="CriAtomExPlayer.GetTime"/></see>
		/// <see><see cref="CriAtomExPlayback.GetTime"/></see>
		/// <see><see cref="CriAtomExPlayback.GetNumPlayedSamples"/></see>
		/// <para>
		...
		///  ループ回数を一旦制限した後、ループ回数を再度無限回に戻したい場合には、 count に <see cref="CriAtomExPlayer.NoLoopLimitation"/> を指定してください。
		///  count に <see cref="CriAtomExPlayer.IgnoreLoop"/> を指定することで、 ループポイント付きの音声データをループさせずに再生することも可能です。
		///  本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数にてクリアされます。 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description>criatomencd.exe で -nodelterm を指定してデータをエンコードする。</description></item>
		/// <item><description>本関数に <see cref="CriAtomExPlayer.IgnoreLoop"/> を指定してから再生を行う。</description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		/// 説明:
		/// 出力音声のボリュームを指定します。
		///  本関数でボリュームを設定後、<see cref="CriAtomExPlayer.Start"/> 関数で再生を開始すると、 設定されたボリュームで音声が再生されます。
		///  またボリューム設定後に <see cref="CriAtomExPlayer.Update"/> 関数や <see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことで、すでに再生された音声のボリュームを更新することも可能です。
		...
		///  1.0fを超える値をセットした場合、<b>プラットフォームによっては</b>、 波形データを元素材よりも大きな音量で再生可能です。
		///  ボリューム値に0.0f未満の値を指定した場合、値は0.0fにクリップされます。
		///  （ボリューム値に負の値を設定した場合でも、 波形データの位相が反転されることはありません。）
		///  キュー再生時、データ側にボリュームが設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>乗算</b>した値が適用されます。
		...
		/// <list type="bullet">
		/// <item><description>プラットフォームごとに挙動が異なる可能性がある。</description></item>
		/// <item><description>音割れが発生する可能性がある。</description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		/// 説明:
		/// 出力音声のピッチを指定します。
		///  本関数でピッチを設定後、<see cref="CriAtomExPlayer.Start"/> 関数で再生を開始すると、 設定されたピッチで音声が再生されます。
		///  またピッチ後に <see cref="CriAtomExPlayer.Update"/> 関数や <see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、すでに再生された音声のピッチを更新することが可能です。
		///  ピッチはセント単位で指定します。
		...
		/// <para>
		/// 備考:
		/// キュー再生時、データ側にピッチが設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>加算</b>した値が適用されます。
		...
		/// <para>
		/// 備考:
		/// キュー再生時、データ側にパンニング3D角度が設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>加算</b>した値が適用されます。
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のパンニング3D距離を更新することができます。
		///  距離は、リスナー位置を0.0f、スピーカーの配置されている円周上を1.0fとして、-1.0f～1.0fの範囲で指定します。
		///  負値を指定すると、パンニング3D角度が180度反転し、逆方向に定位します。 
		/// </para>
		/// <para>
		/// 備考:
		/// キュー再生時、データ側にパンニング3D距離が設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>乗算</b>した値が適用されます。
		...
		/// 説明:
		/// パンニング3Dのボリュームを指定します。
		///  本関数でパンニング3Dボリュームを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたパンニング3Dボリュームで再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のパンニング3Dボリュームを更新することができます。
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のパンタイプを更新することができます。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// <returns><see cref="CriAtomEx.PanType"/> プレーヤー再生時のパンタイプ </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// <see cref="CriAtomEx.PanType.Auto"/> を設定している場合、以下に従って返却されるパンタイプが変化します。<list type="bullet">
		/// <item><description><see cref="CriAtomEx.Config"/>::enable_auto_matching_in_pan_type_auto の設定</description></item>
		/// <item><description><see cref="CriAtomEx3dListener"/> が設定されているかどうか</description></item>
		/// <item><description><see cref="CriAtomEx3dSource"/> が設定されているかどうか</description></item>
		...
		{
			return (CriAtomEx.PanType)NativeMethods.criAtomExPlayer_GetPanTypeOnPlayback(NativeHandle);
		}
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声の出力スピーカータイプを更新することができます。
		/// </para>
		/// <para>
		/// 備考:
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声の角度タイプを更新することができます。
		/// </para>
		/// <para>
		/// 備考:
		/// 本関数の設定はパン3Dと3Dポジショニングにおけるパンニング計算に影響します。
		///  デフォルト値はオフセット（<see cref="CriAtomEx.PanAngleType.Offset"/>）です。
		///  本関数は、主にはCRI Audioとの互換用に使用します。 本関数で <see cref="CriAtomEx.PanAngleType.Fix"/> を設定することで、CRI Audioでのパン3D計算と同じ挙動になります。
		///  本パラメーターはデータ側には設定できないため、常に本関数の設定値が適用されます。
		///  本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数にてクリアされます。 
		/// </para>
		...
		///  本関数でセンドレベルを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたセンドレベルで再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のセンドレベルを更新することができます。
		...
		///  なお、センタースピーカーのあるプラットフォームで、モノラル音をセンタースピーカーのみから出力したい場合、 本関数ではなく<see cref="CriAtomExPlayer.SetPanSpeakerType"/> 関数で<see cref="CriAtomEx.PanSpeakerType._5ch"/> を設定することをお薦めします。
		/// </para>
		/// <para>
		/// 備考:
		/// センドレベルの設定には「自動設定」「手動設定」の2通りが存在します。
		///  AtomExプレーヤーを作成した直後や、 <see cref="CriAtomExPlayer.ResetParameters"/> 関数で パラメーターをクリアした場合、センドレベルの設定は「自動設定」となります。
		...
		///  過去に指定したセンドレベルをクリアし、ルーティングを「自動設定」の状態に戻したい場合は、 <see cref="CriAtomExPlayer.ResetParameters"/> 関数を実行してください。
		///  本パラメーターはデータ側には設定できないため、常に本関数の設定値が適用されます。
		/// </para>
		/// <para>
		/// 注意:
		...
		///  本関数でバスセンドレベルを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたバスセンドレベルで再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のバスセンドレベルを更新することができます。
		///  キュー再生時、データ側にバスセンドレベルが設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>乗算</b>した値が適用されます。
		...
		/// <returns>CriBool バスセンドレベル値が取得できたかどうか？（取得できた：true／取得できない：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 指定した AtomEx プレーヤーに設定されている特定のバスセンドレベルを取得します。
		///  第2引数にはDSPバス設定内のバス名を指定します。
		///  なお、以下のケースに該当する場合、バスセンドレベルの取得に失敗します。<list type="bullet">
		/// <item><description>第2引数のバス名で指定したバスが適用中のDSPバス設定に存在しない</description></item>
		...
		///  キュー再生時、データ側にバスセンドレベルが設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを<b>加算</b>した値が適用されます。
		...
		/// <returns>CriBool バスセンドレベルのオフセット値が取得できたかどうか？（取得できた：true／取得できない：false） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 指定した AtomEx プレーヤーに設定されている特定のバスセンドレベルのオフセットを取得します。
		///  第2引数にはDSPバス設定内のバス名を指定します。
		///  なお、以下のケースに該当する場合、バスセンドレベルのオフセットの取得に失敗します。<list type="bullet">
		/// <item><description>第2引数のバス名で指定したバスが適用中のDSPバス設定に存在しない</description></item>
		/// <item><description>第1引数にて指定した AtomEx プレーヤーに第2引数のバス名に関するバスセンドレベルの設定を行っていない</description></item>
		/// </list>
		/// </para>
		...
		///  ADX環境で新規にパン操作を行うアプリケーションを作成する場合、 <see cref="CriAtomExPlayer.SetPan3dAngle"/> 関数を使用してください。
		///  本関数でパンを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、設定されたパンで再生されます。
		///  すでに再生された音声のパンを変更する場合、本関数で新たなパン設定をプレーヤーに指定し、 <see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数で再生中の音声にそのパラメーターを適用する必要があります。
		///  本関数でパンをコントロール可能なのは、モノラル音声とステレオ音声のみです。 また、左右の定位のみコントロールできます。
		///  3ch以上の音声に対してパンをコントロールしたい場合や、前後を含めた定位をコントロールしたい場合には、 <see cref="CriAtomExPlayer.SetPan3dAngle"/> 関数や<see cref="CriAtomExPlayer.SetSendLevel"/> 関数を使用する必要があります。
		...
		///  本関数でカットオフ周波数を設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたカットオフ周波数でバンドパスフィルターが動作します。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声に対してバンドパスフィルターのカットオフ周波数を更新することができます。
		...
		/// <para>
		/// 備考:
		/// キュー再生時、データ側にバンドパスフィルターのパラメーターが設定されている場合に本関数を呼び出すと、 以下のように設定されます。<list type="bullet">
		/// <item><description>cof_low
		///  データに設定された値に対し、「cof_low_rev = 1.0f - cof_low」としてから乗算し、最終的にまた「cof_low = 1.0f - cof_low_rev」と元に戻して適用されます。
		///  つまり、0.0fを「低域側に最もフィルターを開く」として、開き具合を乗算して適用していく形になります。</description></item>
		/// <item><description>cof_high
		///  データに設定された値に対し、乗算して適用されます。
		///  つまり、1.0fを「高域側に最もフィルターを開く」として、開き具合を乗算して適用していく形になります。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.ResetParameters"/></see>
		/// <para>
		...
		///  本関数でパラメーターを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたパラメーターでバイクアッドフィルターが動作します。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声に対してバイクアッドフィルターのパラメーターを更新することができます。
		///  正規化周波数は、対数軸上の24Hz～24000Hzを、0.0f～1.0fに正規化した値です。
		///  ゲインはデシベルで指定します。
		///  ゲインはフィルタータイプが以下の場合のみ有効です。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomEx.BiquadFilterType.Lowshelf"/>：ローシェルフフィルター</description></item>
		/// <item><description><see cref="CriAtomEx.BiquadFilterType.Highshelf"/>：ハイシェルフフィルター</description></item>
		/// <item><description><see cref="CriAtomEx.BiquadFilterType.Peaking"/>：ピーキングフィルター</description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		...
		///  データに設定された値を上書きします。</description></item>
		/// <item><description>frequency
		///  データに設定された値に加算されます。</description></item>
		/// <item><description>gain
		///  データに設定された値に乗算されます。</description></item>
		/// <item><description>q_value
		///  データに設定された値に加算されます。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.ResetParameters"/></see>
		...
		///  本関数でプライオリティをセット後、 <see cref="CriAtomExPlayer.Start"/> 関数で音声を再生すると、 再生された音声は本関数でセットしたプライオリティで発音されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のプライオリティを更新することができます。
		...
		/// <list type="bullet">
		/// <item><description>先着優先時は、再生中の波形データを優先し、リクエストされた波形データを再生しません。</description></item>
		/// <item><description>後着優先時は、リクエストされた波形データを優先し、ボイスを奪い取ります。</description></item>
		/// </list>
		/// </para>
		/// <b><b>加算</b></b>
		/// <see><see cref="CriAtomExPlayer.ResetParameters"/></see>
		...
		///  Atom Craft上でキューに対して設定された<b>カテゴリキュープライオリティ</b>には影響を与えませんので、 ご注意ください。 
		/// </para>
		/// <para>
		///  キュー再生時、データ側にボイスプライオリティが設定されている場合に本関数を呼び出すと、 データ側に設定されている値と本関数による設定値とを
		/// した値が適用されます。
		...
		///  本関数でAISACコントロール値を設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたAISACコントロール値で再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声のAISACコントロール値を更新することができます。
		///  設定したコントロール値を削除するには、<see cref="CriAtomExPlayer.ClearAisacControls"/> 関数を使用してください。
		///  AISACコントロール値には、0.0f～1.0fの範囲で実数値を指定します。
		/// </para>
		/// <para>
		/// 備考:
		/// AISACのコントロールタイプによって、以下のように挙動が変わります。<list type="bullet">
		/// <item><description>オフ<list type="bullet">
		/// <item><description>本関数等によるAISACコントロール値が未設定の場合はそのAISACは動作しません。</description></item>
		/// </list></description></item>
		/// <item><description>オートモジュレーション<list type="bullet">
		/// <item><description>本関数の設定値には影響されず、時間経過とともに自動的にAISACコントロール値が変化します。</description></item>
		/// </list></description></item>
		/// <item><description>ランダム<list type="bullet">
		/// <item><description>本関数等によって設定されたAISACコントロール値を中央値として、データに設定されたランダム幅でランダマイズし、最終的なAISACコントロール値を決定します。</description></item>
		/// <item><description>ランダマイズ処理は再生開始時のパラメーター適用でのみ行われ、再生中の音声に対するAISACコントロール値変更はできません。</description></item>
		/// <item><description>再生開始時にAISACコントロール値が設定されていなかった場合、0.0fを中央値としてランダマイズ処理を行います。</description></item>
		/// </list></description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.ResetParameters"/></see>
		...
		/// <summary>3Dリスナーハンドルの設定 </summary>
		/// <param name="listener">3Dリスナーハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dポジショニングを実現するための3Dリスナーオブジェクトを設定します。
		///  3Dリスナーオブジェクトと3D音源オブジェクトまたは3D音源オブジェクトリストを設定することで、3Dリスナーと3D音源オブジェクトまたは 3D音源オブジェクトリスト内の全ての3D音源オブジェクトの位置関係等から定位や音量、ピッチ等が自動的に適用されます。
		///  本関数で3Dリスナーオブジェクトを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定された3Dリスナーオブジェクトを参照して再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声が参照する3Dリスナーオブジェクトを変更することができます。
		...
		/// <list type="bullet">
		/// <item><description>3D音源オブジェクトまたは3D音源オブジェクトリストが設定されている</description></item>
		/// <item><description>3Dリスナーが<see cref="CriAtomEx3dListener.CriAtomEx3dListener"/>によって作成されている</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.ResetParameters"/></see>
		/// <para>
		...
		///  右手座標系で各種ベクトルを設定する場合は、ライブラリ初期化時（ <see cref="CriAtomEx.Initialize"/> 関数 ）の設定で <see cref="CriAtomEx.Config"/>::coordinate_system に<see cref="CriAtomEx.CoordinateSystem.RightHanded"/> を指定してください。
		/// </para>
		/// <para>
		///  本パラメーターは 
		///  関数にてクリアされます。 
		...
		/// <summary>3D音源ハンドルの設定 </summary>
		/// <param name="source">3D音源ハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 3Dポジショニングを実現するための3D音源オブジェクトを設定します。
		///  3Dリスナーオブジェクトと3D音源オブジェクトを設定することで、3Dリスナーオブジェクトと3D音源オブジェクトの位置関係等から定位や音量、ピッチ等が自動的に適用されます。
		///  本関数で3D音源オブジェクトを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定された3D音源オブジェクトを参照して再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声が参照する3D音源オブジェクトを変更することができます。
		...
		/// <summary>3D音源ハンドルリストの設定 </summary>
		/// <param name="sourceList">3D音源ハンドルリスト </param>
		/// <remarks>
		/// <para>
		/// 説明:
		/// マルチポジショニング再生を実現するための3D音源オブジェクトリストを設定します。
		///  3Dリスナーオブジェクトと3D音源オブジェクトリストを設定することで、3Dリスナーオブジェクトと3D音源オブジェクトリスト内の 全ての3D音源オブジェクトの位置関係等から定位や音量、ピッチ等が自動的に適用されます。
		///  本関数で3D音源オブジェクトリストを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定された3D音源オブジェクトリストを参照して再生されます。
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声が参照する3D音源オブジェクトリストを変更することができます。
		...
		/// <param name="controlId">コントロールID </param>
		/// <returns>CriFloat32 コントロール値（0.0f～1.0f）、未設定時は-1.0f </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <para>
		/// 備考:
		/// キュー再生時に本関数を呼び出すと、データ側に設定されているカテゴリ設定とマージされます。 その際、カテゴリグループが競合している場合には本関数の設定が有効になります。 CRI Atomライブラリ Ver.2.20.31未満では データ側に設定されているカテゴリ設定を<b>上書き</b>していました。（データ側の設定値は無視されていた）。
		///  従来の仕様で動作させたい場合はライブラリ初期化時（ <see cref="CriAtomEx.Initialize"/> 関数 ）に <see cref="CriAtomEx.Config"/>::enable_category_override_by_ex_player にtrueを設定してください。 本関数で設定したカテゴリ情報は、ACFのレジスト、アンレジストを行うとクリアされます。
		///  本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数にてクリアされます。 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <para>
		/// 備考:
		/// カテゴリ指定を名前で行うことを除き、基本的な仕様は<see cref="CriAtomExPlayer.SetCategoryById"/> 関数と同様です。 デフォルトカテゴリ名での指定を行う場合は CRIATOMEXCATEGORY_DEFAULT_NAME_??? を使用してください。
		///  本パラメーターは <see cref="CriAtomExPlayer.ResetParameters"/> 関数にてクリアされます。 
		/// </para>
		/// <para>
		/// 注意:
		...
		///  そのため、本関数実行後に <see cref="CriAtomExPlayer.SetSendLevel"/> 関数を使用した場合、 音声の出力位置や出力ボリュームが意図しない結果になる可能性があります。
		///  （同様に、 <see cref="CriAtomExPlayer.SetPan3dAngle"/> 関数や <see cref="CriAtomExPlayer.SetSendLevel"/> 関数も併用できません。）
		...
		/// <para>
		/// 注意:
		/// 本関数は <see cref="CriAtomExPlayer.SetTrackInfo"/> 関数でトラック情報を設定した プレーヤーに対してのみ実行可能です。
		///  本関数は内部的に <see cref="CriAtomExPlayer.SetSendLevel"/> 関数を使用しています。
		///  そのため、本関数実行後に <see cref="CriAtomExPlayer.SetSendLevel"/> 関数を使用した場合、 音声の出力位置や出力ボリュームが意図しない結果になる可能性があります。
		///  （同様に、 <see cref="CriAtomExPlayer.SetPan3dAngle"/> 関数や <see cref="CriAtomExPlayer.SetSendLevel"/> 関数も併用できません。）
		///  本関数は、３チャンネル以上の入力に対応した機種でしか利用できません。
		...
		///  また設定後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声の無音時処理モードを更新することができます。
		...
		///  本関数でキュープライオリティをセット後、 <see cref="CriAtomExPlayer.Start"/> 関数で音声を再生すると、 再生された音声は本関数でセットしたキュープライオリティで発音されます。
		///  関数実行前のデフォルト設定値は0です。
		/// </para>
		/// <para>
		/// 備考:
		...
		/// 説明:
		/// プリディレイタイムを設定します。
		///  本関数でプリディレイタイムを設定後、<see cref="CriAtomExPlayer.Start"/> 関数により再生開始すると、 設定されたプリディレイタイム発音を待ちます。
		...
		///  登録したコールバック関数内で <see cref="CriAtomPlayer.SetData"/> 関数等を用いて Atom プレーヤーに データをセットすると、セットされたデータは現在再生中のデータに続いてシームレスに 連結されて再生されます。
		///  また、コールバック関数内で <see cref="CriAtomPlayer.SetPreviousDataAgain"/> 関数を実行することで、 同一データを繰り返し再生し続けることも可能です。
		/// </para>
		/// <para>
		/// 備考:
		/// 登録したコールバック関数内でデータを指定しなかった場合、現在のデータを再生し 終えた時点で、AtomEx プレーヤーのステータスが <see cref="CriAtomExPlayer.Status.Playend"/> に遷移します。
		///  タイミング等の問題により、データを指定することができないが、ステータスを <see cref="CriAtomExPlayer.Status.Playend"/> に遷移させたくない場合には、コールバック関数内で <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行してください。
		/// <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行することで、約1V後に再度データ要求 コールバック関数が呼び出されます。（コールバック処理をリトライ可能。）
		///  ただし、 <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行した場合、再生が途切れる （連結箇所に一定時間無音が入る）可能性があります。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomExAcb.GetWaveformInfoById"/>（引数のnull指定は不可）</description></item>
		/// <item><description><see cref="CriAtomExAcb.GetWaveformInfoByName"/>（引数のnull指定は不可）</description></item>
		/// <item><description><see cref="CriAtomExAcb.GetOnMemoryAwbHandle"/></description></item>
		/// <item><description><see cref="CriAtomExAcb.GetStreamingAwbHandle"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetData"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetFile"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetContentId"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetWaveId"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetPreviousDataAgain"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.DeferCallback"/></description></item>
		/// </list>
		/// </para>
		/// <list>
		/// <list type="bullet">
		/// <item><description>HCA-MXコーデックを使用する場合、データがシームレスには連結されず、 再生中の音声と次に再生する音声との継ぎ目に無音が入ります。</description></item>
		/// <item><description>プラットフォーム固有の音声圧縮コーデックを使用している場合、 エラー等が発生する可能性があります。</description></item>
		/// </list>
		/// </list>
		/// <para>
		...
		///  登録したコールバック関数は、ボイスが内部的に使用している Atom プレーヤーが 連結再生用のデータを要求するタイミングで実行されます。
		///  （前回のデータを読み込み終えて、次に再生すべきデータを要求するタイミングで コールバック関数が実行されます。）
		///  コールバック関数内で <see cref="CriAtomPlayer.SetData"/> 関数等を用いて Atom プレーヤーにデータをセットすると、 セットされたデータは現在再生中のデータに続いてシームレスに連結されて再生されます。
		///  また、本関数内で <see cref="CriAtomPlayer.SetPreviousDataAgain"/> 関数を実行することで、 同一データを繰り返し再生し続けることも可能です。 
		/// </para>
		/// <para>
		/// 備考:
		/// 本関数内でデータを指定しなかった場合、現在のデータを再生し終えた時点で、 AtomEx プレーヤーのステータスが <see cref="CriAtomExPlayer.Status.Playend"/> に遷移します。
		///  タイミング等の問題により、データを指定することができないが、ステータスを <see cref="CriAtomExPlayer.Status.Playend"/> に遷移させたくない場合には、コールバック関数内で <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行してください。
		/// <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行することで、約1V後に再度データ要求 コールバック関数が呼び出されます。（コールバック処理をリトライ可能。）
		///  ただし、 <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行した場合、再生が途切れる （連結箇所に一定時間無音が入る）可能性があります。
		/// </para>
		...
		/// </list>
		/// </para>
		/// <para> 本コールバック関数内で長時間処理をブロックすると、音切れ等の問題が発生しますので、 ご注意ください。</para>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  またアタッチ後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声に対しても、アタッチしたAISACによる各種パラメーター設定を適用することができます。
		...
		/// <returns>プレーヤーにアタッチされているAISAC数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <param name="tween">トゥイーンハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  またアタッチ後、<see cref="CriAtomExPlayer.Update"/> 関数、<see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことにより、 すでに再生された音声に対しても、アタッチしたトゥイーンによる各種パラメーター設定を適用することができます。
		/// </para>
		/// <para>
		/// 備考:
		/// トゥイーンによって変化したパラメーターは、AtomExプレーヤーに設定されているパラメーターに対し、加算／乗算／上書きされます。
		///  加算／乗算／上書きのどれに該当するかは、AtomExプレーヤーへの設定関数（<see cref="CriAtomExPlayer.SetVolume"/> 関数等）と同様です。 例えば、ボリュームであれば乗算され、AISACコントロール値であれば上書きします。
		...
		/// <param name="tween">取り外すトゥイーンハンドル </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  本関数で再生開始ブロックインデックスを指定後、ブロックシーケンスキューを <see cref="CriAtomExPlayer.Start"/> 関数で再生開始すると指定したブロックから再生を 開始します。 
		/// </para>
		/// <para>
		/// 備考:
		/// AtomExプレーヤーのデフォルトブロックインデックスは 0 です。
		/// <see cref="CriAtomExPlayer.Start"/> 関数による再生開始時にプレーヤーに設定されているキューが ブロックシーケンスでない場合は、本関数で設定した値は利用されません。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  本関数でドライセンドレベルを設定後、<see cref="CriAtomExPlayer.Start"/> 関数で再生を開始すると、 設定されたドライセンドレベルで音声が再生されます。
		///  またドライセンドレベル設定後に <see cref="CriAtomExPlayer.Update"/> 関数や <see cref="CriAtomExPlayer.UpdateAll"/> 関数を呼び出すことで、すでに再生された音声のドライセンドレベルを更新することも可能です。
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// 備考:
		/// 第 2 引数（ obj ）にセットした値は、コールバック関数の引数として渡されます。
		///  コールバック関数のその他の引数については、 別途 <see cref="CriAtomExPlayback.EventCbFunc"/> の説明をご参照ください。
		///  funcにnullを指定することで登録済み関数の登録解除が行えます。
		/// </para>
		/// <para>
		/// 注意:
		...
		///  アロケーターを登録せずにフェーダーをアタッチする場合、あらかじめ本関数で計算した ワーク領域サイズ分のメモリをワーク領域として <see cref="CriAtomExPlayer.AttachFader"/> 関数に セットする必要があります。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomExFader.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。
		///  ワーク領域サイズ計算時に失敗した場合、戻り値は -1 になります。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックの メッセージで確認可能です。
		/// </para>
		/// <para>
		...
		///  本関数でフェーダーをアタッチしたプレーヤーは、以降音声再生開始毎 （ <see cref="CriAtomExPlayer.Start"/> 関数や <see cref="CriAtomExPlayer.Prepare"/> 関数を実行する毎）に、 以下の制御を行います。
		/// <list type="bullet">
		/// <item><description>既にフェードアウト中の音があれば強制停止。</description></item>
		/// <item><description>現在再生中（またはフェードイン中）の音声をフェードアウト。</description></item>
		/// <item><description>新規に再生を開始する音声をフェードイン。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExPlayer.Stop"/></see>
		/// <list>
		/// <list type="bullet">
		/// <item><description>既にフェードアウト中の音があれば強制停止。</description></item>
		/// <item><description>現在再生中（またはフェードイン中）の音声をフェードアウト。</description></item>
		...
		{
			fixed (CriAtomExFader.Config* configPtr = &config)
				NativeMethods.criAtomExPlayer_AttachFader(NativeHandle, configPtr, default, default);
		}
		...
		///  次回音声再生時（ <see cref="CriAtomExPlayer.Start"/> 関数実行時）には、本関数で設定された 時間で新規に音声がフェードイン再生されます。
		///  フェードイン時間のデフォルト値は 0 秒です。
		///  そのため、本関数を使用しない場合フェードインは行われず、即座にフルボリューム で音声の再生が開始されます。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行する前に、 <see cref="CriAtomExPlayer.AttachFader"/> 関数を使用して あらかじめプレーヤーにフェーダーをアタッチしておく必要があります。
		...
		///  次回音声再生時（ <see cref="CriAtomExPlayer.Start"/> 関数実行時）には、本関数で設定された 時間で再生中の音声がフェードアウトします。
		...
		/// <list type="number">
		/// <item><description>指定された時間で音声のボリュームを 0 まで落とす。</description></item>
		/// <item><description>ボリュームが 0 の状態でディレイ時間が経過するまで再生を続ける。</description></item>
		/// <item><description>ディレイ時間経過後に再生を停止する。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomEx.IgnoreFadeOut"/></see>
		/// <list>
		/// <list type="bullet">
		/// <item><description>0指定時：即座にボリュームが 0 に落とされ、停止処理が行われる。</description></item>
		/// <item><description><see cref="CriAtomEx.IgnoreFadeOut"/>指定時：ボリューム変更は行われず、停止処理が行われる。</description></item>
		/// </list>
		/// </list>
		/// <para>
		...
		/// <returns>CriSint32 フェードアウト時間（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// フェードアウト時間を取得します。
		...
		/// <returns>CriSint32 フェードイン時間（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// フェードイン時間を取得します。
		...
		/// <returns>CriSint32 フェードイン開始オフセット（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// フェードイン開始オフセットを取得します。
		...
		/// <returns>CriSint32 フェードアウト後のディレイ時間（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriBool フェード処理中かどうか（true = フェード処理中、false = フェード処理中ではない） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <list type="bullet">
		/// <item><description>クロスフェード開始のための同期待ち中。</description></item>
		/// <item><description>フェードイン／フェードアウト処理中（ボリューム変更中）。</description></item>
		...
		/// <see cref="CriAtomExPlayer.SetGroupNumber"/> 関数に対してこの値を指定すると、 指定されたプレーヤーはボイスリミットグループによる制限を受けなくなります。
		///  （空きボイスがあるか、または自身より低プライオリティのボイスがあれば、 ボイスリミットグループに関係なくボイスを取得します。） 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomExPlayer.SetGroupNumber"/>
		public const Int32 NoGroupLimitation = (-1);
		/// <summary>ループ回数制御用 </summary>
		/// <remarks>
		/// <para>
		/// 説明:
		/// ループ回数制限なし 
