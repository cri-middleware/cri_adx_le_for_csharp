...
namespace CriWare
{
	/// <summary>サウンドオブジェクトハンドル </summary>
	/// <remarks>
	/// <para>
	/// 説明:
	/// <see cref="CriAtomExSoundObject"/> は、サウンドオブジェクトを操作するためのオブジェクトです。
	/// <see cref="CriAtomExSoundObject.CriAtomExSoundObject"/> 関数でサウンドオブジェクトを作成すると、 関数はサウンドオブジェクト操作用に、この"サウンドオブジェクトオブジェクト"を返します。 
	...
	public partial class CriAtomExSoundObject : IDisposable
	{
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomExSoundObject.CriAtomExSoundObject"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomExSoundObject.Config"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomExSoundObject_SetDefaultConfig_(CriAtomExSoundObjectConfig *p_config)"/>
		/// </remarks>
		...
		///  アロケーターを登録せずにサウンドオブジェクトを作成する場合、 あらかじめ本関数で計算したワーク領域サイズ分のメモリを ワーク領域として <see cref="CriAtomExSoundObject.CriAtomExSoundObject"/> 関数にセットする必要があります。
		///  プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtomExSoundObject.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomExSoundObject.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		///  作成されるサウンドオブジェクトは、オブジェクト作成時に本構造体で指定された設定に応じて、 内部リソースを必要なだけ確保します。
		///  サウンドオブジェクトが必要とするワーク領域のサイズは、本構造体で指定されたパラメーターに応じて変化します。 
		/// </para>
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、各メンバの設定前に必ず <see cref="CriAtomExSoundObject.SetDefaultConfig"/> メソッドを使用してデフォルト値をセットしてください。
		...
		/// <param name="config">サウンドオブジェクト作成用コンフィグ構造体 </param>
		/// <returns><see cref="CriAtomExSoundObject"/> サウンドオブジェクトオブジェクト </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// サウンドオブジェクトを作成します。
		...
		public unsafe CriAtomExSoundObject(in CriAtomExSoundObject.Config config)
		{
			fixed (CriAtomExSoundObject.Config* configPtr = &config)

				NativeHandle = NativeMethods.criAtomExSoundObject_Create(configPtr, default, default);
		}
		/// <summary>デフォルト設定でのインスタンス作成</summary>
		public unsafe CriAtomExSoundObject()
		{
...
#pragma warning restore 1591
		...
		/// <list type="bullet">
		/// <item><description>発音数制限やイベント機能が影響する範囲（スコープ）の限定</description></item>
		/// <item><description>再生コントロール（停止、ポーズ等）</description></item>
		/// <item><description>パラメーターコントロール</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomExSoundObject.DeletePlayer"/></see>
		/// <para>
		/// 注意:
		/// 本関数の呼び出しは、追加しようとしているAtomExプレーヤーで音声を再生していない状態で行ってください。
		///  ステータスが <see cref="CriAtomExPlayer.Status.Stop"/> ではないAtomExプレーヤーが指定された場合、 追加時に <see cref="CriAtomExPlayer.StopWithoutReleaseTime"/> 関数にて再生停止が行われます。 
		/// </para>
		/// <para>
		/// 追加したAtomExプレーヤーをサウンドオブジェクトから削除する場合は、 
		///  関数を呼び出してください。
