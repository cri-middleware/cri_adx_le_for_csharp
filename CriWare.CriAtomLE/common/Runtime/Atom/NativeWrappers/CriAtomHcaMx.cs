...
#pragma warning disable 0465
	...
	public static partial class CriAtomHcaMx
	{
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomHcaMx.Initialize"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomHcaMx.Config"/> ）に、デフォルト値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomHcaMx_SetDefaultConfig_(CriAtomHcaMxConfig *p_config)"/>
		/// </remarks>
		...
		/// <see cref="CriAtom.SetUserAllocator"/> メソッドによるアロケーター登録を行わずに <see cref="CriAtomHcaMx.Initialize"/> 関数でHCA-MXの初期化を行う場合、 本関数で計算したサイズ分のメモリをワーク領域として渡す必要があります。
		///  ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。
		///  ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。
		/// </para>
		/// <para>
		/// 備考:
		/// HCA-MXの初期化に必要なワークメモリのサイズは、HCA-MX初期化用コンフィグ 構造体（ <see cref="CriAtomHcaMx.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomHcaMx.SetDefaultConfig"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		...
		public unsafe partial struct Config
		{
			...
			/// <para>
			/// 注意:
			/// Atomライブラリ初期化時に指定した値（ <see cref="CriAtom.Config"/> 構造体の server_frequency ）と、同じ値をセットする必要があります。
			/// </para>
			/// </remarks>
			...
			///  ミキサーを複数作成することで、 ミキサーごとに異なるバスエフェクトを適用することが可能になります。
			/// </para>
			/// <para>
			/// 注意:
			/// HCA-MXのデコード処理、および定常状態の処理負荷は、 ミキサーの数に比例して重くなります。
			/// </para>
			/// </remarks>
			...
			/// <para>
			/// 備考:
			/// 例えば<see cref="CriAtomHcaMx.SetFrequencyRatio"/> 関数に 2.0f を指定する場合は output_sampling_rate * 2 を指定してください。
			/// </para>
			/// </remarks>
		...
		/// <para>
		/// 説明:
		/// ワーク領域サイズの計算用に、HCA-MX初期化用コンフィグ構造体 （ <see cref="CriAtomHcaMx.Config"/> 構造体）を仮登録します。
		///  HCA-MXプレーヤーの作成に必要なワーク領域のサイズは、 HCA-MX初期化時（ <see cref="CriAtomHcaMx.Initialize"/> 関数実行時） に設定する構造体のパラメーターによって変化します。
		///  そのため、通常はプレーヤーの作成に必要なワーク領域サイズを計算する前に、 HCA-MXを初期化する必要があります。
		///  本関数を使用してHCA-MX初期化用コンフィグ構造体を登録した場合、 <see cref="CriAtomPlayer.CalculateWorkSizeForHcaMxPlayer"/> 関数が初期化処理なしに使用可能となります。
		/// </para>
		/// <para>
		/// 備考:
		/// 引数（ config ）に null を指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForHcaMxPlayer"/> 適用時と同じパラメーター）でワーク領域サイズを計算します。
		...
		/// 備考:
		/// HCA-MXの初期化に必要なワークメモリのサイズは、HCA-MX初期化用コンフィグ 構造体（ <see cref="CriAtomHcaMx.Config"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomHcaMx.SetDefaultConfig"/> 適用時と同じパラメーター）で初期化処理を行います。
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		public static unsafe void Initialize(in CriAtomHcaMx.Config config)
		{
			fixed (CriAtomHcaMx.Config* configPtr = &config)
				NativeMethods.criAtomHcaMx_Initialize(configPtr, default, default);
		}
		...
		/// <see cref="CriAtom.SetUserAllocator"/> メソッドを使用してアロケーターを登録済みの場合、 HCA-MX初期化時に確保されたメモリ領域が解放されます。
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomPlayer.SetDefaultConfigForHcaMxPlayer"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// <para>HCA-MXプレーヤー作成用コンフィグ構造体 </para>
		/// </remarks>
		...
		public unsafe partial struct PlayerConfig
		{
			...
			/// <see cref="CriAtomPlayer.CreateHcaMxPlayer"/> 関数で作成されたAtomプレーヤーは、max_channelsで指定した チャンネル数"以下の"音声データを再生可能です。
			/// </para>
			/// </remarks>
			...
			/// <see cref="CriAtomPlayer.CreateHcaMxPlayer"/> 関数で作成されたAtomプレーヤーは、max_sampling_rateで指定した サンプリングレートと一致する音声データのみを再生可能です。
			/// </para>
			/// <para>
			/// 備考:
