...
#pragma warning disable 0465
	...
	public static partial class CriAtomInstrument
	{
		...
		///  以下の条件に該当する場合は、ユーザ定義インストゥルメントインターフェースの登録に失敗し、エラーコールバックが返ります:<list type="bullet">
		/// <item><description>同一のエフェクト名を持つユーザ定義インストゥルメントインターフェースが既に登録されている</description></item>
		/// <item><description>Atomが使用しているユーザ定義インストゥルメントインターフェースと異なる</description></item>
		...
		/// <para>
		/// 注意:
		/// 将来的にメンバが増える可能性があるため、 <see cref="CriAtomPlayer.SetDefaultConfigForInstrumentPlayer"/> メソッドを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。
		///  （構造体のメンバに不定値が入らないようご注意ください。） 
		/// </para>
		/// <para>インストゥルメントプレーヤー作成用コンフィグ構造体 </para>
		/// </remarks>
		[Serializable]
		public unsafe partial struct PlayerConfig
		{
			...
			/// <summary></summary>
			/// <remarks>
			/// <para>最大サンプリングレート </para>
			/// </remarks>
			public Int32 maxSamplingRate;

			/// <summary></summary>
			/// <remarks>
			/// <para>出力先のサウンドレンダラタイプ </para>
			/// </remarks>
