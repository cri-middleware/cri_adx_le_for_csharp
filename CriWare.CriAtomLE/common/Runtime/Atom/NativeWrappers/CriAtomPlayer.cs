...
namespace CriWare
{
	...
	public partial class CriAtomPlayer : IDisposable
	{
		...
		{
			NativeMethods.criAtomPlayer_SetHcaFormat(NativeHandle, numChannels, samplingRate, bitrate);
		}
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateRawPcmPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.RawPcmPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForInstrumentPlayer_(CriAtomInstrumentPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// プレーヤーを作成する際に、動作仕様を指定するためのコンフィグ構造体 （ <see cref="CriAtomPlayer.ConfigASR"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfig_ASR_(CriAtomPlayerConfig_ASR *p_config)"/>
		/// </remarks>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.AdxPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForAdxPlayer_(CriAtomAdxPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateAiffPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.AiffPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForAiffPlayer_(CriAtomAiffPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateHcaMxPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtomHcaMx.PlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForHcaMxPlayer_(CriAtomHcaMxPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateHcaPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.HcaPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForHcaPlayer_(CriAtomHcaPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <see cref="CriAtomPlayer.CreateRawPcmPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.RawPcmPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForRawPcmPlayer_(CriAtomRawPcmPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateStandardPlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.StandardPlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForStandardPlayer_(CriAtomStandardPlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 説明:
		/// <see cref="CriAtomPlayer.CreateWavePlayer"/> 関数に設定するコンフィグ構造体 （ <see cref="CriAtom.WavePlayerConfig"/> ）に、デフォルトの値をセットします。
		/// </para>
		/// <nativeinfo declaration="void criAtomPlayer_SetDefaultConfigForWavePlayer_(CriAtomWavePlayerConfig *p_config)"/>
		/// </remarks>
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.StandardPlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForStandardPlayer"/> 適用時と同じパラメーター） でワーク領域サイズを計算します。 
		...
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForStandardPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		...
		///  標準プレーヤー作成時に確保されたメモリは、標準プレーヤー破棄時（ <see cref="CriAtomPlayer.Dispose"/> 関数実行時）に解放されます。
		...
		///  標準プレーヤー作成前に <see cref="CriAtomPlayer.CalculateWorkSizeForStandardPlayer"/> 関数で取得した サイズ分のメモリを予め確保しておき、本関数に設定してください。
		///  尚、Fixed Memory方式を用いた場合、ワーク領域は標準プレーヤーの破棄 （ <see cref="CriAtomPlayer.Dispose"/> 関数）を行うまでの間、ライブラリ内で利用され続けます。
		...
		/// <list type="number">
		/// <item><description>標準プレーヤー作成前に、 <see cref="CriAtom.SetUserMallocFunction"/> 関数と <see cref="CriAtom.SetUserFreeFunction"/> 関数を用いてメモリ確保／解放関数を登録する。
		/// </description></item>
		/// <item><description>標準プレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description><see cref="CriAtomPlayer.CreateStandardPlayer"/> 関数で標準プレーヤーを作成する。
		///  （workにはnull、work_sizeには0を指定する。）
		/// </description></item>
		/// <item><description>オブジェクトが不要になったら <see cref="CriAtomPlayer.Dispose"/> 関数で標準プレーヤーを破棄する。
		/// </description></item>
		/// </list>
		/// </para>
		/// <list>
		...
		/// <item><description>標準プレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description>標準プレーヤーの作成に必要なワーク領域のサイズを、 <see cref="CriAtomPlayer.CalculateWorkSizeForStandardPlayer"/> 関数を使って計算する。
		/// </description></item>
...
#pragma warning restore 1591
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.AdxPlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForAdxPlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForAdxPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  User Allocator方式を用いる場合、ユーザがワーク領域を用意する必要はありません。
		///  workにnull、work_sizeに0を指定するだけで、必要なメモリを登録済みのメモリ確保関数から確保します。
		///  ADXプレーヤー作成時に確保されたメモリは、ADXプレーヤー破棄時（ <see cref="CriAtomPlayer.Dispose"/> 関数実行時）に解放されます。
		///  Fixed Memory方式を用いる場合、ワーク領域として別途確保済みのメモリ領域を本関数に 設定する必要があります。
		///  ワーク領域のサイズは <see cref="CriAtomPlayer.CalculateWorkSizeForAdxPlayer"/> 関数で取得可能です。
		///  ADXプレーヤー作成前に <see cref="CriAtomPlayer.CalculateWorkSizeForAdxPlayer"/> 関数で取得した サイズ分のメモリを予め確保しておき、本関数に設定してください。
		///  尚、Fixed Memory方式を用いた場合、ワーク領域はADXプレーヤーの破棄 （ <see cref="CriAtomPlayer.Dispose"/> 関数）を行うまでの間、ライブラリ内で利用され続けます。
		...
		/// <list type="number">
		/// <item><description>ADXプレーヤー作成前に、 <see cref="CriAtom.SetUserMallocFunction"/> 関数と <see cref="CriAtom.SetUserFreeFunction"/> 関数を用いてメモリ確保／解放関数を登録する。
		/// </description></item>
		/// <item><description>ADXプレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description><see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数でADXプレーヤーを作成する。
		///  （workにはnull、work_sizeには0を指定する。）
		/// </description></item>
		/// <item><description>オブジェクトが不要になったら <see cref="CriAtomPlayer.Dispose"/> 関数でADXプレーヤーを破棄する。
		/// </description></item>
		/// </list>
		/// </para>
		/// <list>
		...
		/// <item><description>ADXプレーヤー作成用コンフィグ構造体にパラメーターをセットする。
		/// </description></item>
		/// <item><description>ADXプレーヤーの作成に必要なワーク領域のサイズを、 <see cref="CriAtomPlayer.CalculateWorkSizeForAdxPlayer"/> 関数を使って計算する。
		/// </description></item>
		/// <item><description>ワーク領域サイズ分のメモリを確保する。
		/// </description></item>
		/// <item><description><see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数でADXプレーヤーを作成する。
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.HcaPlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForHcaPlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForHcaPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  各方式の詳細については、別途 <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数の説明をご参照ください。
		/// <see cref="CriAtomPlayer.CreateHcaPlayer"/> 関数を実行すると、Atomプレーヤーが作成され、 プレーヤーを制御するためのオブジェクト（ <see cref="CriAtomPlayer"/> ）が返されます。
		///  データやデコーダーのセット、再生の開始、ステータスの取得等、Atomプレーヤーに対して 行う操作は、全てオブジェクトに対して行います。
		///  作成されたAtomプレーヤーオブジェクトを使用して音声データを再生する手順は以下のとおりです。
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.SetData"/> 関数を使用して、Atomプレーヤーに再生するデータをセットする。
		...
		/// <seealso cref="CriAtomPlayer.CreateHcaPlayer"/>
		public static unsafe CriAtomPlayer CreateHcaPlayer(in CriAtom.HcaPlayerConfig config)
		{
			IntPtr handle;
			fixed (CriAtom.HcaPlayerConfig* configPtr = &config)
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtomHcaMx.PlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForHcaMxPlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForHcaMxPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  各方式の詳細については、別途 <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数の説明をご参照ください。
		/// <see cref="CriAtomPlayer.CreateHcaMxPlayer"/> 関数を実行すると、Atomプレーヤーが作成され、 プレーヤーを制御するためのオブジェクト（ <see cref="CriAtomPlayer"/> ）が返されます。
		///  データやデコーダーのセット、再生の開始、ステータスの取得等、Atomプレーヤーに対して 行う操作は、全てオブジェクトに対して行います。
		///  作成されたAtomプレーヤーオブジェクトを使用して音声データを再生する手順は以下のとおりです。
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.SetData"/> 関数を使用して、Atomプレーヤーに再生するデータをセットする。
		...
		/// <seealso cref="CriAtomPlayer.Start"/>
		/// <seealso cref="CriAtomPlayer.CreateHcaMxPlayer"/>
		public static unsafe CriAtomPlayer CreateHcaMxPlayer(in CriAtomHcaMx.PlayerConfig config)
		{
			IntPtr handle;
			fixed (CriAtomHcaMx.PlayerConfig* configPtr = &config)
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.WavePlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForWavePlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForWavePlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  各方式の詳細については、別途 <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数の説明をご参照ください。
		/// <see cref="CriAtomPlayer.CreateWavePlayer"/> 関数を実行すると、Atomプレーヤーが作成され、 プレーヤーを制御するためのオブジェクト（ <see cref="CriAtomPlayer"/> ）が返されます。
		///  データやデコーダーのセット、再生の開始、ステータスの取得等、Atomプレーヤーに対して 行う操作は、全てオブジェクトに対して行います。
		///  作成されたAtomプレーヤーオブジェクトを使用して音声データを再生する手順は以下のとおりです。
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.SetData"/> 関数を使用して、Atomプレーヤーに再生するデータをセットする。
		...
		/// <seealso cref="CriAtomPlayer.Start"/>
		/// <seealso cref="CriAtomPlayer.CreateWavePlayer"/>
		public static unsafe CriAtomPlayer CreateWavePlayer(in CriAtom.WavePlayerConfig config)
		{
			IntPtr handle;
			fixed (CriAtom.WavePlayerConfig* configPtr = &config)
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.AiffPlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForAiffPlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForAiffPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  各方式の詳細については、別途 <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数の説明をご参照ください。
		/// <see cref="CriAtomPlayer.CreateAiffPlayer"/> 関数を実行すると、Atomプレーヤーが作成され、 プレーヤーを制御するためのオブジェクト（ <see cref="CriAtomPlayer"/> ）が返されます。
		///  データやデコーダーのセット、再生の開始、ステータスの取得等、Atomプレーヤーに対して 行う操作は、全てオブジェクトに対して行います。
		///  作成されたAtomプレーヤーオブジェクトを使用して音声データを再生する手順は以下のとおりです。
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.SetData"/> 関数を使用して、Atomプレーヤーに再生するデータをセットする。
		...
		/// <seealso cref="CriAtomPlayer.Start"/>
		/// <seealso cref="CriAtomPlayer.CreateAiffPlayer"/>
		public static unsafe CriAtomPlayer CreateAiffPlayer(in CriAtom.AiffPlayerConfig config)
		{
			IntPtr handle;
			fixed (CriAtom.AiffPlayerConfig* configPtr = &config)
		...
		/// <para>
		/// 備考:
		/// プレーヤーの作成に必要なワークメモリのサイズは、プレーヤー作成用コンフィグ 構造体（ <see cref="CriAtom.RawPcmPlayerConfig"/> ）の内容によって変化します。
		///  引数にnullを指定した場合、デフォルト設定 （ <see cref="CriAtomPlayer.SetDefaultConfigForRawPcmPlayer"/> 適用時と同じパラメーター）で ワーク領域サイズを計算します。 
		///  引数 config の情報は、関数内でのみ参照されます。
		///  関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても 問題ありません。 
		/// </para>
		/// <para>
		...
		///  作成されたプレーヤーで再生できる音声のフォーマットは、第一引数（config）に指定した パラメーターによって決まります。
		///  例えば、configのmax_sampling_rateに24000を設定した場合、作成されたプレーヤーでは 24kHzを超えるサンプリングレートの音声データは再生できなくなります。
		///  configにnullを指定した場合、デフォルト設定（ <see cref="CriAtomPlayer.SetDefaultConfigForRawPcmPlayer"/> 適用時と同じパラメーター）でプレーヤーを作成します。 
		///  プレーヤーを作成する際には、ライブラリが内部で利用するためのメモリ領域（ワーク領域） を確保する必要があります。
		///  ワーク領域を確保する方法には、以下の2通りの方法があります。
		/// <b>(a) User Allocator方式</b>：メモリの確保／解放に、ユーザが用意した関数を使用する方法。
		/// <b>(b) Fixed Memory方式</b>：必要なメモリ領域を直接ライブラリに渡す方法。
		///  各方式の詳細については、別途 <see cref="CriAtomPlayer.CreateAdxPlayer"/> 関数の説明をご参照ください。
		/// <see cref="CriAtomPlayer.CreateRawPcmPlayer"/> 関数を実行すると、Atomプレーヤーが作成され、 プレーヤーを制御するためのオブジェクト（ <see cref="CriAtomPlayer"/> ）が返されます。
		///  データやデコーダーのセット、再生の開始、ステータスの取得等、Atomプレーヤーに対して 行う操作は、全てオブジェクトに対して行います。
		///  作成されたAtomプレーヤーオブジェクトを使用して音声データを再生する手順は以下のとおりです。
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.SetData"/> 関数を使用して、Atomプレーヤーに再生するデータをセットする。
		...
		/// <seealso cref="CriAtomPlayer.Start"/>
		/// <seealso cref="CriAtomPlayer.CreateRawPcmPlayer"/>
		public static unsafe CriAtomPlayer CreateRawPcmPlayer(in CriAtom.RawPcmPlayerConfig config)
		{
			IntPtr handle;
			fixed (CriAtom.RawPcmPlayerConfig* configPtr = &config)
		...
		///  本関数でメモリアドレスとサイズを指定後、 <see cref="CriAtomPlayer.Start"/> 関数で再生を 開始すると、指定されたデータが再生されます。 
		/// </para>
		/// <para>
		...
		/// 説明:
		/// 音声データの再生処理を開始します。
		///  本関数を実行する前に、事前に <see cref="CriAtomPlayer.SetData"/> 関数等を使用し、再生する 音声データをAtomプレーヤーにセットしておく必要があります。
		///  例えば、オンメモリの音声データを再生する場合には、事前に <see cref="CriAtomPlayer.SetData"/> 関数を使って音声データをセットした後、本関数を実行する 必要があります。
		...
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.Status.Stop"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.Status.Prep"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.Status.Playing"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.Status.Playend"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.Status.Error"/></description></item>
		/// </list>
		/// </para>
		/// <para>
		/// 備考:
		/// 再生開始後、実際に音声が出力されるまでには、タイムラグがあります。
		///  オンメモリのデータ再生時（ <see cref="CriAtomPlayer.SetData"/> 関数でデータをセットした場合）は、 本関数実行後、最初にサーバー処理が実行されたタイミングでステータスが <see cref="CriAtomPlayer.Status.Playing"/> に遷移します。
		///  しかし、ストリーミング再生時は、再生を維持するために必要なデータが充分バッファリング されるまでの間、<see cref="CriAtomPlayer.Status.Prep"/> を維持し続けます。
		///  （必要充分なデータが供給された時点で、 <see cref="CriAtomPlayer.Status.Playing"/> に遷移します。）
		///  尚、ステータスが <see cref="CriAtomPlayer.Status.Playing"/> に遷移するタイミングは、 あくまで"サウンドライブラリに対して再生指示を発行する"タイミングになります。
		...
		///  ストリーミング再生時に意図としたタイミングで発音を開始させたい場合には、 <see cref="CriAtomPlayer.Pause"/> 関数で事前にポーズをかけておき、Atomプレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playing"/> に遷移した時点で、ポーズを解除してください。
		///  （ポーズをかけた状態でステータスが <see cref="CriAtomPlayer.Status.Playing"/> に遷移した場合、 ポーズ解除後最初のサーバー処理が実行されたタイミングで、発音が開始されます。
		/// </para>
		/// <para>
		/// 注意:
		/// 既に再生を開始したAtomプレーヤーに対して本関数を実行することはできません。
		///  （ADXライブラリとは異なり、再生中のAtomプレーヤーに対して再度再生の開始を指示した場合、 エラーになります。）
		///  Atomプレーヤーに対して再生を指示する場合には、必ず事前にステータスをチェックし、 ステータスが準備中（ <see cref="CriAtomPlayer.Status.Prep"/> ）や再生中（ <see cref="CriAtomPlayer.Status.Playing"/> ） になっていないことをご確認ください。 
		/// </para>
		/// <para>
		/// Atomプレーヤーを作成した時点では、Atomプレーヤーのステータスは停止状態 （ <see cref="CriAtomPlayer.Status.Stop"/> ）です。
		///  再生する音声データをセット後、本関数を実行することで、Atomプレーヤーのステータスが 準備状態（ <see cref="CriAtomPlayer.Status.Prep"/> ）に変更されます。
		///  （<see cref="CriAtomPlayer.Status.Prep"/> は、データ供給やデコードの開始を待っている状態です。）
		///  再生の開始に充分なデータが供給された時点で、Atomプレーヤーはステータスを 再生状態（ <see cref="CriAtomPlayer.Status.Playing"/> ）に変更し、音声の出力を開始します。
		///  セットされたデータを全て再生し終えると、Atomプレーヤーはステータスを再生終了状態 （ <see cref="CriAtomPlayer.Status.Playend"/> ）に変更します。
		///  尚、再生中にエラーが発生した場合には、Atomプレーヤーはステータスをエラー状態 （ <see cref="CriAtomPlayer.Status.Error"/> ）に変更します。
		...
		///  登録したコールバック関数内で <see cref="CriAtomPlayer.SetData"/> 関数等を用いてAtomプレーヤーに データをセットすると、セットされたデータは現在再生中のデータに続いてシームレスに 連結されて再生されます。
		///  また、コールバック関数内で <see cref="CriAtomPlayer.SetPreviousDataAgain"/> 関数を実行することで、 同一データを繰り返し再生し続けることも可能です。
		/// </para>
		/// <para>
		/// 備考:
		/// 登録したコールバック関数内でデータを指定しなかった場合、現在のデータを再生し 終えた時点で、Atomプレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playend"/> に遷移します。
		///  タイミング等の問題により、データを指定することができないが、ステータスを <see cref="CriAtomPlayer.Status.Playend"/> に遷移させたくない場合には、コールバック関数内で <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行してください。
		...
		/// <list type="bullet">
		/// <item><description>コーデック</description></item>
		/// <item><description>チャンネル数</description></item>
		/// <item><description>サンプリングレート</description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		///  本関数でファイルを指定後、 <see cref="CriAtomPlayer.Start"/> 関数で再生を開始すると、 指定されたファイルがストリーミング再生されます。
		...
		///  本関数にバインダーとコンテンツIDを指定後、 <see cref="CriAtomPlayer.Start"/> 関数で再生を 開始すると、指定されたコンテンツファイルがストリーミング再生されます。
		///  尚、本関数を実行した時点では、ファイルの読み込みは開始されません。
		///  ファイルの読み込みが開始されるのは、 <see cref="CriAtomPlayer.Start"/> 関数実行後です。
		/// </para>
		/// <para>
		...
		///  本関数にAWBオブジェクトと波形データIDを指定後、 <see cref="CriAtomPlayer.Start"/> 関数で再生を 開始すると、指定した波形データがストリーミング再生されます。
		///  尚、本関数を実行した時点では、ファイルの読み込みは開始されません。
		///  ファイルの読み込みが開始されるのは、 <see cref="CriAtomPlayer.Start"/> 関数実行後です。
		/// </para>
		/// <para>
		...
		/// <para>
		/// 備考:
		/// データ要求コールバック関数内で何もしなかった場合、Atomプレーヤーのステータスは <see cref="CriAtomPlayer.Status.Playend"/> に遷移します。
		///  しかし、データ要求コールバック関数内で本関数を実行した場合、Atomプレーヤーの ステータスは <see cref="CriAtomPlayer.Status.Playing"/> を維持し続けます。 
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数を実行した場合、Atomプレーヤーのステータスは <see cref="CriAtomPlayer.Status.Playing"/> を維持し続けますが、音声出力は途切れる可能性があります。
		...
		/// <returns><see cref="CriAtomPlayer.Status"/> ステータス </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomPlayer.Status.Stop"/></see>
		/// <see><see cref="CriAtomPlayer.Status.Prep"/></see>
		/// <see><see cref="CriAtomPlayer.Status.Prep"/></see>
		/// <see><see cref="CriAtomPlayer.Status.Playing"/></see>
		/// <see><see cref="CriAtomPlayer.Status.Playend"/></see>
		/// <see><see cref="CriAtomPlayer.Status.Error"/></see>
		/// <para>
		/// Atomプレーヤーを作成した時点では、Atomプレーヤーのステータスは停止状態 （ 
		///  ）です。
		///  再生する音声データをセット後、<see cref="CriAtomPlayer.Start"/> 関数を実行することで、 Atomプレーヤーのステータスが準備状態（ 
		///  ）に変更されます。
		///  （ 
		///  は、データ供給やデコードの開始を待っている状態です。）
		///  再生の開始に充分なデータが供給された時点で、Atomプレーヤーはステータスを 再生状態（ 
		///  ）に変更し、音声の出力を開始します。
		...
		{
			return (CriAtomPlayer.Status)NativeMethods.criAtomPlayer_GetStatus(NativeHandle);
		}

		/// <summary>再生のポーズ／ポーズ解除 </summary>
		/// <param name="flag">動作フラグ（CRI_TRUE = ポーズ、CRI_FALSE = ポーズ解除） </param>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  Atomプレーヤーがポーズされているかどうかは <see cref="CriAtomPlayer.IsPaused"/> 関数を使用する ことで確認が可能です。
		///  本関数は主に以下の2通りの用途に利用します。
		/// <list type="bullet">
		/// <item><description>音声出力の一時停止／一時停止解除。</description></item>
		/// <item><description>ストリーミング再生の頭出し。</description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomPlayer.Start"/></see>
		/// <list>
		/// <list type="number">
		/// <item><description><see cref="CriAtomPlayer.Pause"/> 関数でAtomプレーヤーをポーズさせる。</description></item>
		/// <item><description><see cref="CriAtomPlayer.Start"/> 関数でAtomプレーヤーに再生開始を指示する。</description></item>
		/// <item><description>Atomプレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playing"/> になるのを待つ。</description></item>
		/// <item><description>発音を開始したいタイミングで <see cref="CriAtomPlayer.Pause"/> 関数を実行し、ポーズを解除する。</description></item>
		/// </list>
		/// </list>
		/// <para>
		...
		///  関数で再生指示しても、音声の出力は行われません。
		///  しかし、再生準備は行われるため、データが充分に供給されていれば、ステータスは <see cref="CriAtomPlayer.Status.Playing"/> まで遷移します。
		...
		/// 説明:
		/// 再生の停止要求を発行します。
		///  音声再生中のAtomプレーヤーに対して本関数を実行すると、Atomプレーヤーは再生を停止 （ファイルの読み込みや、発音を止める）し、ステータスを停止状態 （ <see cref="CriAtomPlayer.Status.Stop"/> ）に遷移します。
		///  既に停止しているAtomプレーヤー（ステータスが <see cref="CriAtomPlayer.Status.Playend"/> や <see cref="CriAtomPlayer.Status.Error"/> のAtomプレーヤー） に対して本関数を実行すると、 Atomプレーヤーのステータスを <see cref="CriAtomPlayer.Status.Stop"/> に変更します。 
		/// </para>
		/// <para>
		/// 注意:
		/// 音声再生中のAtomプレーヤーに対して本関数を実行した場合、ステータスが即座に <see cref="CriAtomPlayer.Status.Stop"/> になるとは限りません。
		...
		/// <returns>CriBool ポーズ状態（true = ポーズされている、false = ポーズされていない） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomPlayer.SetData"/></see>
		/// <see><see cref="CriAtomPlayer.Start"/></see>
		/// <para>
		/// 備考
		/// 再生中に <see cref="CriAtomPlayer.Stop"/> 関数を実行した場合、上記の流れに関係なく、 最終的にステータスは <see cref="CriAtomPlayer.Status.Stop"/> に戻ります。
		///  （ <see cref="CriAtomPlayer.Stop"/> 関数の呼び出しタイミングによっては、 <see cref="CriAtomPlayer.Status.Stop"/> に遷移するまでに時間がかかる場合があります。）
		///  また、再生中に不正なデータを読み込んだ場合や、ファイルアクセスに失敗した場合も、 上記の流れに関係なく、ステータスは <see cref="CriAtomPlayer.Status.Error"/> に遷移します。 
		/// </para>
		/// <para>
		/// 注意:
		/// ステータスが <see cref="CriAtomPlayer.Status.Prep"/> や <see cref="CriAtomPlayer.Status.Playing"/> のタイミングでは、 データのセット（ <see cref="CriAtomPlayer.SetData"/> 関数）や、再生の開始（ <see cref="CriAtomPlayer.Start"/> 関数） は行えません。
		///  現在再生中のAtomプレーヤーを停止して別のデータを再生したい場合は、一旦 <see cref="CriAtomPlayer.Stop"/> 関数で再生を停止させ、ステータスが <see cref="CriAtomPlayer.Status.Stop"/> に遷移してから次のデータを セット／再生する必要があります。 
		/// </para>
		/// <para>
		/// Atomプレーヤー作成直後の状態は、停止状態（ <see cref="CriAtomPlayer.Status.Stop"/> ）です。
		///  関数等でデータをセットし、 
		///  関数を 実行すると、再生準備状態（ <see cref="CriAtomPlayer.Status.Prep"/> ）に遷移し、再生準備を始めます。
		///  データが充分供給され、再生準備が整うと、ステータスは再生中（ <see cref="CriAtomPlayer.Status.Playing"/> ） に変わり、音声の出力が開始されます。
		///  セットされたデータを全て再生し終えた時点で、ステータスは再生完了 （ <see cref="CriAtomPlayer.Status.Playend"/> ）に変わります。 
		/// </para>
		/// </remarks>
		/// <seealso cref="CriAtomPlayer.GetStatus"/>
		/// <seealso cref="CriAtomPlayer.SetData"/>
		...
		/// <returns>CriSint32 チャンネル数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriBool サンプル数が取得できたかどうか（ true = 取得できた、 false = 取得できなかった） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  また、 <see cref="CriAtomPlayer.Pause"/> 関数でポーズをかけた場合、 再生済みサンプル数のカウントアップも停止します。
		///  （ポーズを解除すればカウントアップが再開されます。） 
		/// </para>
		/// <para>
		/// 注意:
		/// 再生済みサンプル数は、プレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playing"/> になるまで取得できません。
		///  （ <see cref="CriAtomPlayer.Status.Prep"/> 時に本関数を実行した場合、エラー値が返されます。）
		...
		/// <see cref="CriAtomPlayer.GetNumPlayedSamples"/> 関数と異なり、 サウンドバッファに書き込まれた未出力の音声データのサンプル数を含む値を返します。 
		///  サンプル数が正しく取得できた場合、戻り値は true になります。
		...
		///  また、 <see cref="CriAtomPlayer.Pause"/> 関数でポーズをかけた場合、 書き込み済みサンプル数のカウントアップも停止します。
		///  （ポーズを解除すればカウントアップが再開されます。） 
		/// </para>
		/// <para>
		/// 注意:
		/// 書き込み済みサンプル数は、プレーヤのステータスが <see cref="CriAtomPlayer.Status.Playing"/> になるまで取得できません。
		///  （ <see cref="CriAtomPlayer.Status.Prep"/> 時に本関数を実行した場合、エラー値が返されます。）
		...
		/// <returns>CriSint64 デコードデータ量（単位はバイト） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  また、 <see cref="CriAtomPlayer.Pause"/> 関数でポーズをかけた場合、 デコード量のカウントアップも停止します。
		///  （ポーズを解除すればカウントアップが再開されます。） 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriSint64 デコードサンプル数 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// Atomプレーヤー内でデコードした音声データのバイト数を返します。
		...
		///  また、 <see cref="CriAtomPlayer.Pause"/> 関数でポーズをかけた場合、 デコードサンプル数のカウントアップも停止します。
		///  （ポーズを解除すればカウントアップが再開されます。） 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriSint64 再生時刻（ミリ秒単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  そのため、 <see cref="CriAtomPlayer.SetFrequencyRatio"/> 関数を用いてピッチを上げた場合、 再生時刻は実時間よりも早く進みます。
		///  （ピッチを下げた場合、再生時刻は実時刻よりも遅く進みます。）
		///  本関数が返す再生時刻は、出力済み音声データの累積値です。
		///  そのため、ループ再生時や、シームレス連結再生時を行った場合でも、 再生位置に応じて時刻が巻き戻ることはありません。
		///  また、 <see cref="CriAtomPlayer.Pause"/> 関数でポーズをかけた場合、 再生時刻のカウントアップも停止します。
		///  （ポーズを解除すればカウントアップが再開されます。） 
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <returns>CriBool 情報が取得できたかどうか（ true = 取得できた、 false = 取得できなかった） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriSint32 入力バッファー内のデータ残量（Byte単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		/// <returns>CriSint32 出力バッファー内のデータ残量（サンプル数単位） </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		...
		///  プレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playing"/> にもかかわらず、 データ残量が長時間 0 の場合、何らかの異常によりデコード処理が 行われていない可能性があります。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomPlayer.SetChannelVolume"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetSendLevel"/></description></item>
		/// </list>
		/// </para>
		/// <see><see cref="CriAtomPlayer.SetChannelVolume"/></see>
		/// <para>
		/// 注意:
		...
		/// <para>
		/// 例えば、本関数に0.5fを、 
		///  関数にも0.5fを設定した場合、 出力音声のボリュームは原音を0.25f倍したボリュームで出力されます。
		...
		/// <summary>ボリュームの指定 </summary>
		/// <returns>CriFloat32 ボリューム値 </returns>
		/// <remarks>
		/// <para>
		/// 説明:
		/// 出力音声のボリュームを取得します。
		...
		///  （ <see cref="CriAtomPlayer.SetSendLevel"/> 関数を併用している場合は、 必ずしもレフトスピーカーから出力される音量のボリュームが変更される とは限りません。）
		...
		///  本関数の設定値と、以下の関数のボリューム設定値は独立して制御されます。
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomPlayer.SetVolume"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetSendLevel"/> 例えば、本関数に0.5fを、 <see cref="CriAtomPlayer.SetVolume"/> 関数にも0.5fを設定した場合、 出力音声のボリュームは原音を0.25f倍したボリュームで出力されます。
		...
		///  チャンネル単位のボリューム指定（ <see cref="CriAtomPlayer.SetChannelVolume"/> 関数） と異なり、本関数では1つのチャンネルのデータを複数のスピーカーから異なる ボリュームで出力することが可能です。
		///  尚、セットされたセンドレベルの値は <see cref="CriAtomPlayer.ResetSendLevel"/> 関数で リセットすることが可能です。
		///  \備考: センドレベルの設定には「自動設定」「手動設定」の2通りが存在します。
		///  Atomプレーヤーを作成した直後や、 <see cref="CriAtomPlayer.ResetSendLevel"/> 関数で センドレベルをクリアした場合、センドレベルの設定は「自動設定」となります。
		...
		///  センドレベルの設定をクリアし、ルーティングを「自動設定」の状態に戻したい場合は、 <see cref="CriAtomPlayer.ResetSendLevel"/> 関数を実行してください。
		...
		/// <list type="bullet">
		/// <item><description><see cref="CriAtomPlayer.SetVolume"/></description></item>
		/// <item><description><see cref="CriAtomPlayer.SetChannelVolume"/> 例えば、本関数に0.5fを、 <see cref="CriAtomPlayer.SetVolume"/> 関数にも0.5fを設定した場合、 出力音声のボリュームは原音を0.25f倍したボリュームで出力されます。
		///  （0.5f×0.5f＝0.25fの演算が行われます。） </description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		/// <see cref="CriAtomPlayer.SetPanAdx1Compatible"/> 関数が、内部的に本関数を呼び出すため、 両者を併用した場合、後から実行した関数により設定が上書きされる可能性があります。
		///  音源の定位をコントロールする際には、本関数かまたは <see cref="CriAtomPlayer.SetPanAdx1Compatible"/> 関数 のいずれか一方のみをご利用ください。
		///  （3Dパンを利用する場合は本関数を、2Dパンのみを行う場合は <see cref="CriAtomPlayer.SetPanAdx1Compatible"/> 関数をご利用ください。） 
		///  本関数は一部の機種でのみ利用が可能です。
		...
		///  （自動設定時のルーティングについては、 <see cref="CriAtomPlayer.SetSendLevel"/> 関数の説明を参照してください。）
		/// </para>
		/// <para>
		/// 注意:
		...
		///  Atomプレーヤーを作成した直後や、 <see cref="CriAtomPlayer.ResetPan"/> 関数で パンをクリアした場合、パンの設定は「自動設定」となります。
		...
		///  チャンネル0の音声をレフトスピーカーから、 チャンネル1の音声をライトスピーカーから出力します。
		///  これに対し、本関数を用いて「手動設定」を行った場合、音声データのチャンネル数に 関係なく、指定されたルーティングで音声が出力されます。
		///  パンの設定をクリアし、ルーティングを「自動設定」の状態に戻したい場合は、 <see cref="CriAtomPlayer.ResetPan"/> 関数を実行してください。
		/// </para>
		/// <para>
		/// 注意:
		/// 本関数でパンをコントロール可能なのは、モノラル音声とステレオ音声のみです。
		///  3ch以上の音声に対してパンをコントロールしたい場合には、 <see cref="CriAtomPlayer.SetSendLevel"/> 関数を使用する必要があります。
		///  再生する音声データがステレオの場合、チャンネル0番とチャンネル1番のそれぞれの について、独立してパンをコントロールすることが可能です。
		///  ただし、設定されたパンがモノラル音声向けなのか、ステレオ音声向けなのかは区別 されないため、ステレオ設定用にパン設定を行ったAtomプレーヤーでモノラル音声を再生 した場合、意図しない位置に音源が定位する可能性があります。
		...
		///  本関数が内部的に <see cref="CriAtomPlayer.SetSendLevel"/> 関数を呼び出すため、 両者を併用した場合、後から実行した関数により設定が上書きされる可能性があります。
		...
		///  （自動設定時のルーティングについては、 <see cref="CriAtomPlayer.SetPanAdx1Compatible"/> 関数の説明を参照してください。）
		/// </para>
		/// <para>
		/// 注意:
		...
		///  （Atomプレーヤー作成時に指定する <see cref="CriAtom.AdxPlayerConfig"/> 構造体 の max_sampling_rate の値に、「原音のサンプリングレート×周波数比」で 計算される値を指定する必要があります。）
		/// 本関数は一部の機種でのみ利用が可能です。
		///  （プラットフォームのサウンドライブラリの仕様によっては実装が困難な場合が あり、その場合は利用できません。）
		///  本関数が利用可能かどうかは、別途マニュアルの機種依存情報のページをご参照ください。 
		/// </para>
		...
		///  ループ回数を一旦制限した後、ループ回数を再度無限回に戻したい場合には、 count に <see cref="CriAtomPlayer.NoLoopLimitation"/> を指定してください。
		///  count に <see cref="CriAtomPlayer.IgnoreLoop"/> を指定することで、 ループポイント付きの音声データをループさせずに再生することも可能です。
		/// </para>
		/// <para>
		/// 注意:
		...
		/// <item><description>本関数に <see cref="CriAtomPlayer.IgnoreLoop"/> を指定してから再生を行う。</description></item>
		/// </list>
		/// </para>
		/// <para>
		...
		/// <para>
		/// 注意:
		/// 本関数は <see cref="CriAtomPlayer.CreateHcaMxPlayer"/> 関数で作成されたプレーヤーに対してのみ効果があります。
		...
		/// <para>
		/// 注意:
		/// 本関数は <see cref="CriAtomPlayer.CreateRawPcmPlayer"/> 関数で作成されたプレーヤーに対してのみ効果があります。
		///  （他の関数で作成されたプレーヤーに対しては、何の効果もありません。）
		///  本関数は停止中のプレーヤーに対してのみ実行可能です。
		/// </para>
		...
		///  登録したコールバック関数は、Atomプレーヤーが連結再生用のデータを要求するタイミングで 実行されます。
		///  （前回のデータを読み込み終えて、次に再生すべきデータを要求するタイミングで コールバック関数が実行されます。） 本関数内で <see cref="CriAtomPlayer.SetData"/> 関数等を用いてAtomプレーヤーにデータをセットすると、 セットされたデータは現在再生中のデータに続いてシームレスに連結されて再生されます。
		///  また、本関数内で <see cref="CriAtomPlayer.SetPreviousDataAgain"/> 関数を実行することで、 同一データを繰り返し再生し続けることも可能です。 
		/// </para>
		/// <para>
		/// 備考:
		/// 本関数内でデータを指定しなかった場合、現在のデータを再生し終えた時点で、 Atomプレーヤーのステータスが <see cref="CriAtomPlayer.Status.Playend"/> に遷移します。
		///  タイミング等の問題により、データを指定することができないが、ステータスを <see cref="CriAtomPlayer.Status.Playend"/> に遷移させたくない場合には、コールバック関数内で <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行してください。
		/// <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行することで、約1V後に再度データ要求 コールバック関数が呼び出されます。（コールバック処理をリトライ可能。）
		///  ただし、 <see cref="CriAtomPlayer.DeferCallback"/> 関数を実行した場合、再生が途切れる （連結箇所に一定時間無音が入る）可能性があります。
		/// </para>
	...
	[UnmanagedCallersOnly(CallConvs = new System.Type[]{typeof(CallConvCdecl)})]
		...
		///  変更されたステータスについては、コールバック関数の引数として渡されるAtomプレーヤー オブジェクトに対し、 <see cref="CriAtomPlayer.GetStatus"/> 関数を実行することで取得可能です。
		...
		///  コールバック関数内でAtomプレーヤーを破棄しないでください。
		///  コールバックを抜けた後も、しばらくの間はサーバー処理内で当該オブジェクトのリソース が参照されるため、アクセス違反等の重大な問題が発生する可能性があります。 
		///  コールバック関数は1つしか登録できません。
		///  登録操作を複数回行った場合、既に登録済みのコールバック関数が、 後から登録したコールバック関数により上書きされてしまいます。
		///  funcにnullを指定することで登録済み関数の登録解除が行えます。
		/// </para>
		...
		///  登録したコールバック関数は、Atomプレーヤーのステータスが更新されるタイミングで 実行されます。
		///  変更されたステータスについては、引数で渡されるAtomプレーヤーオブジェクト（player）に対し、 <see cref="CriAtomPlayer.GetStatus"/> 関数を実行することで取得可能です。 
		/// </para>
		/// <para>
		/// 備考:
		/// 厳密には、ステータス遷移～コールバック関数実行までの間に他の処理が割り込みで動作する 余地があるため、ステータス遷移とコールバック関数実行のタイミングがズレる可能性があります。
