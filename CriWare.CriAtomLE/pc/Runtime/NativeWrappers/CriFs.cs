...
#pragma warning disable 0465
	...
	public static partial class CriFs
	{
		...
		/// <para>
		/// 注意:
		/// : 本関数は、ライブラリ初期化時にスレッドモデルをマルチスレッドモデル （ <see cref="CriFs.ThreadModel.Multi"/> ）に設定した場合にのみ効果を発揮します。
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetServerThreadPriority_PC(prio);
		}
		...
		public static unsafe CriErr.Error GetServerThreadPriorityPC(out Int32 prio)
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetServerThreadPriority_PC(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetFileAccessThreadPriority_PC(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetFileAccessThreadPriority_PC(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetMemoryFileSystemThreadPriority_PC(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetMemoryFileSystemThreadPriority_PC(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetDataDecompressionThreadPriority_PC(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetDataDecompressionThreadPriority_PC(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetInstallerThreadPriority_PC(prio);
		}
		...
		{
			fixed (Int32* prioPtr = &prio)
				return (CriErr.Error)NativeMethods.criFs_GetInstallerThreadPriority_PC(prioPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetServerThreadAffinityMask_PC(mask);
		}
		...
		public static unsafe CriErr.Error GetServerThreadAffinityMaskPC(out IntPtr mask)
		{
			fixed (IntPtr* maskPtr = &mask)
				return (CriErr.Error)NativeMethods.criFs_GetServerThreadAffinityMask_PC(maskPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetFileAccessThreadAffinityMask_PC(mask);
		}
		...
		{
			fixed (IntPtr* maskPtr = &mask)
				return (CriErr.Error)NativeMethods.criFs_GetFileAccessThreadAffinityMask_PC(maskPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetMemoryFileSystemThreadAffinityMask_PC(mask);
		}
		...
		{
			fixed (IntPtr* maskPtr = &mask)
				return (CriErr.Error)NativeMethods.criFs_GetMemoryFileSystemThreadAffinityMask_PC(maskPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetDataDecompressionThreadAffinityMask_PC(mask);
		}
		...
		{
			fixed (IntPtr* maskPtr = &mask)
				return (CriErr.Error)NativeMethods.criFs_GetDataDecompressionThreadAffinityMask_PC(maskPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SetInstallerThreadAffinityMask_PC(mask);
		}
		...
		{
			fixed (IntPtr* maskPtr = &mask)
				return (CriErr.Error)NativeMethods.criFs_GetInstallerThreadAffinityMask_PC(maskPtr);
		}
		...
		{
			return (CriErr.Error)NativeMethods.criFs_SwitchPathUnicodeToUtf8_PC(sw);
		}
